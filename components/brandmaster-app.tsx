...
function OnlineBrandCleanup({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  const [step, setStep] = useState<1 | 2 | 3>(1); const [query, setQuery] = useState(""); const [selected, setSelected] = useState<string[]>([]); const [decisions, setDecisions] = useState<Record<string, "APPROVE" | "REJECT">>({}); const [online, setOnline] = useState<Record<string, boolean>>({});
  ...
  const visible = candidates.filter((item) => `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const reviewed = Object.keys(decisions).length; const approved = Object.values(decisions).filter((value) => value === "APPROVE").length; const changedRows = candidates.filter((item) => item.changed && decisions[item.brand.id] === "APPROVE");
  function decide(item: typeof candidates[number], decision: "APPROVE" | "REJECT") { setDecisions((current) => ({ ...current, [item.brand.id]: decision })); if (decision === "APPROVE" && item.changed) onSave({ ...item.brand, name: item.suggested, aliases: item.aliases }); }
  function exportChanges() { const headers = ["BrandID", "BeforeBrand", "AfterBrand", "BeforeAliases", "AfterAliases", "Decision", "Confidence", "ReviewedBy", "ReviewedAt"]; const rows = changedRows.map((item) => [item.brand.id, item.brand.name, item.suggested, item.brand.aliases.join(" | "), item.aliases.join(" | "), decisions[item.brand.id], item.confidence, currentUser, new Date().toISOString()]); const csv = [headers, ...rows].map((row) => row.map((value) => `"${String(value ?? "").replaceAll('"', '""')}"`).join(",")).join("\n"); download(`brand-cleanup-changes-${new Date().toISOString().slice(0, 10)}.csv`, csv); }
