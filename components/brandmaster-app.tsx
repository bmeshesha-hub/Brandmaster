  ...
  return <>{!cleanMode && <WorkflowStepper stage={1} onNavigate={onNavigate} onRestart={onRestart} hasImport={Boolean(currentBatch)} outputReady={currentOutputReady} owner={currentBatch?.owner || currentUser || "Shared team"} counts={currentCounts} basketRecords={currentBatch?.records || []} />}
    ...
    {cleanMode && <section className="step1-choice-grid" aria-label="Choose how to start Step 1">
      <section className="step1-choice-card add step1-add-tile">
        <div className="step1-add-tile-head"><span className="step1-choice-number">1</span><span><small>ADD NEW WORK</small><b>Add 1–10 brands</b><p>Paste one brand per line or add a CSV file.</p></span><strong className={pastedNames.length > MAX_WORKLIST_SIZE ? "limit-exceeded" : ""}>{pastedNames.length} / {MAX_WORKLIST_SIZE}</strong></div>
        <textarea aria-label="Brand names to add" disabled={Boolean(currentBatch)} value={brandNames} onChange={(event) => setBrandNames(event.target.value)} placeholder={"Paste brands here, one per line…\n\npegaso\nb & p rods\nvolkswagen oe"} />
        <div className="step1-add-tile-meta"><span className={currentUser ? "ready" : "missing"}><Users size={13} />{currentUser ? <>Assigned to <b>{currentUser}</b></> : "Choose Working as first"}</span>{ubqSource && <span className="ready"><Check size={13} />Brand IDs matched automatically</span>}</div>
...
function AiReviewAssist({ records, knownBrandIds, onUpdate, initiallyOpen = false, selectionMode = false, onClose }: { records: BrandRecord[]; knownBrandIds: Set<string>; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; initiallyOpen?: boolean; selectionMode?: boolean; onClose?: () => void }) {
  ...
  return <section className={`ai-review ${open ? "open" : ""}`}>
    <div className="ai-review-head"><div className="gpt-icon"><Sparkles size={18} /></div><div><span>OPTIONAL EXTERNAL AI REVIEW</span><b>{selectionMode ? `Review ${records.length} selected decision${records.length === 1 ? "" : "s"} with your validation GPT` : `Check all ${records.length} decisions with your validation GPT`}</b><p>Brandmaster generates the prompt and safely imports the returned JSON. No API key is stored here.</p></div><button className={open ? "secondary" : "primary"} onClick={() => { if (open && onClose) onClose(); else setOpen(!open); }}>{open ? <X size={15} /> : <Sparkles size={15} />}{open ? "Close" : "Check with AI validator"}</button></div>
