...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  function updatePriorityItems(ids: string[], status: PriorityQueueStatus, assignee?: string) {
    ...
    const next = { ...data, priorityQueue: data.priorityQueue.map((item) => {
      ...
      return { ...item, status, assignedTo: release ? undefined : status === "ASSIGNED" ? assignmentOwner : item.assignedTo || assignmentOwner, assignedAt: release ? undefined : status === "ASSIGNED" ? now : item.assignedAt || now, completedAt: status === "COMPLETED" ? now : undefined, updatedAt: now };
    }) };
    setData(next); markPriorityPending();
...
function PriorityQueue({ items, currentUser, teamMembers, onChooseTeamMember, syncConnected, savePending, saveBusy, saveCountdown, onSave, onUpdate, onReset, onRemove, onStart, onNavigate }: { items: PriorityQueueItem[]; currentUser: string; teamMembers: string[]; onChooseTeamMember: (member: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; onSave: () => void; onUpdate: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onReset: (ids: string[]) => void; onRemove: (ids: string[]) => void; onStart: (ids: string[]) => void; onNavigate: (view: View) => void }) {
  ...
  const assignable = selectedItems.filter((item) => item.status !== "COMPLETED").map((item) => item.id);
