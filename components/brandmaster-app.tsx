...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  function reopenRecordsForReview(recordIds: string[]) {
    ...
    setData((prev) => ({ ...prev, batches: prev.batches.map((batch) => ({ ...batch, records: batch.records.map((record) => selectedIds.has(record.id) ? { ...record, status: "needs-review", excludedFromExport: false, reason: `Returned from Step 3 for another review: ${record.reason}` } : record) })) }));
  ...
  return <div className={`app-shell ebay-theme ${experienceMode}-mode`}>
    ...
    <main>
      ...
      <div className="page">
        ...
        {view === "output" && <BulkOutput records={current?.records || []} batch={current} data={data} onUpdate={updateRecord} onSetExcluded={setRecordExportExcluded} onReopen={reopenRecordsForReview} onCompletePriority={completePriorityBatch} onNavigate={navigate} onRestart={requestFreshTriage} />}
...
function PriorityQueue({ items, currentUser, teamMembers, onChooseTeamMember, syncConnected, savePending, saveBusy, saveCountdown, onSave, onUpdate, onReset, onRemove, onStart, onNavigate }: { items: PriorityQueueItem[]; currentUser: string; teamMembers: string[]; onChooseTeamMember: (member: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; onSave: () => void; onUpdate: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onReset: (ids: string[]) => void; onRemove: (ids: string[]) => void; onStart: (ids: string[]) => void; onNavigate: (view: View) => void }) {
  ...
  return <section className="priority-queue"><div className="priority-hero"><span><Activity size={23} /></span><div><small>TEAM TRIAGE</small><h2>High Priority Brand Queue</h2><p>Claim brands before working so teammates do not review the same records.</p></div><div className="priority-progress"><b>{progress}%</b><small>{completed} of {items.length} completed</small><i><em style={{ width: `${progress}%` }} /></i></div></div>
    ...
    <div className="priority-stats"><div><b>{items.length}</b><span>Total urgent brands</span></div><div><b>{available.length}</b><span>Available</span></div><div><b>{open.filter((item) => item.status === "IN_REVIEW").length}</b><span>In progress</span></div><div><b>{assigned}</b><span>Assigned</span></div><div><b>{completed}</b><span>Completed</span></div></div>
    {!items.length ? <div className="priority-empty"><Users size={25} /><div><b>No urgent team work yet</b><p>Use the High Priority Queue tab below, or send selected Root/UBQ records here from Brand management.</p></div></div> : <><div className={`priority-save-bar ${savePending ? "pending" : "saved"}`}><span>{saveBusy ? <RefreshCw className="spinning" size={19} /> : savePending ? <CircleHelp size={19} /> : <Check size={19} />}</span><div><b>{saveBusy ? "Saving team changes…" : savePending ? "Unsaved team changes" : "Team queue saved"}</b><small>{saveBusy ? "Merging your work with the latest shared workspace." : savePending ? `Click Save now, or Brandmaster will auto-save in ${saveCountdown} seconds.` : `Automatic team check in ${saveCountdown} seconds. You can also save and pull now.`}</small></div><button className="primary" disabled={saveBusy || !syncConnected} onClick={onSave}>{saveBusy ? "Saving…" : savePending ? "Save team changes" : "Save & pull now"}</button></div><div className="priority-toolbar"><div><b>Team assignments</b><small>Use “Assigned to” below to see each person’s list.</small></div><div className="priority-toolbar-actions">{completed > 0 && <button className="secondary" title="Select completed work to restart or remove" onClick={() => { setQueueOwner("ALL"); setQueueStatus("COMPLETED"); setSelected(items.filter((item) => item.status === "COMPLETED").map((item) => item.id)); }}><RotateCcw size={14} />Clean completed ({completed})</button>}</div></div>
      <div className="record-filters queue-filters"><label className="filter-search"><Search size={14} /><input value={queueQuery} onChange={(event) => setQueueQuery(event.target.value)} placeholder="Find brand, ID, owner, or target…" /></label><label className="queue-owner-filter"><span>Assigned to</span><select value={queueOwner} onChange={(event) => { setQueueOwner(event.target.value); setSelected([]); }}><option value="ALL">Everyone</option><option value="UNASSIGNED">Available / unassigned</option>{teamMembers.map((member) => <option key={member} value={member}>{member}{member === currentUser ? " (you)" : ""}</option>)}</select></label><label><span>Source</span><select value={queueSource} onChange={(event) => setQueueSource(event.target.value as "ALL" | PriorityQueueSource)}><option value="ALL">All sources</option>{queueSources.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label><span>Status</span><select value={queueStatus} onChange={(event) => setQueueStatus(event.target.value as "ALL" | PriorityQueueStatus)}><option value="ALL">All statuses</option><option value="UNASSIGNED">Available</option><option value="ASSIGNED">Assigned</option><option value="IN_REVIEW">In progress</option><option value="BLOCKED">Blocked</option><option value="COMPLETED">Completed</option></select></label><strong>{visible.length.toLocaleString()} shown</strong>{(queueQuery || queueOwner !== "ALL" || queueSource !== "ALL" || queueStatus !== "ALL") && <button className="text-button" onClick={() => { setQueueQuery(""); setQueueOwner("ALL"); setQueueSource("ALL"); setQueueStatus("ALL"); }}>Clear filters</button>}</div>
      {selected.length > 0 && <div className="priority-actions"><b>{selected.length} selected</b>{assignable.length > 0 && <div className="priority-assign-control"><label>Assign to<select value={assignmentTarget} onChange={(event) => setAssignmentTarget(event.target.value)}><option value="" disabled>Choose teammate</option>{teamMembers.map((member) => <option key={member} value={member}>{member}</option>)}</select></label><button className="primary" disabled={!assignmentTarget} onClick={() => { onUpdate(assignable, "ASSIGNED", assignmentTarget); setSelected(assignable); setQueueOwner(assignmentTarget); setQueueStatus("ALL"); }}><Users size={14} />Assign {assignable.length}</button></div>}{mineSelected.length > 0 && <button className="primary" onClick={() => onStart(mineSelected)}><WandSparkles size={14} />Start as {currentUser}</button>}<button className="secondary priority-restart" title="Clear owner, progress, completion, and final result" onClick={() => { onReset(selected); setSelected([]); setQueueOwner("UNASSIGNED"); setQueueStatus("ALL"); }}><RotateCcw size={14} />Start over</button><button className={`secondary priority-remove ${removeArmed ? "armed" : ""}`} title="Permanently remove these items from the High Priority Queue" onClick={() => { if (!removeArmed) { setRemoveArmed(true); return; } onRemove(selected); setSelected([]); }}><Trash2 size={14} />{removeArmed ? "Confirm remove" : "Remove from queue"}</button><button className="icon-button" onClick={() => setSelected([])}><X size={14} /></button></div>}
      <div className="priority-table"><div><input type="checkbox" checked={visible.length > 0 && visible.every((item) => selected.includes(item.id))} onChange={(event) => setSelected(event.target.checked ? visible.map((item) => item.id) : [])} /><b>Brand</b><b>Source</b><b>Owner</b><b>Status</b><b>Final result</b></div>{visible.slice(0, 100).map((item) => <div key={item.id}><input type="checkbox" checked={selected.includes(item.id)} onChange={(event) => setSelected(event.target.checked ? [...new Set([...selected, item.id])] : selected.filter((id) => id !== item.id))} /><span><b>{item.name}</b><small>{item.brandId}</small></span><em>{item.source}</em><span>{item.assignedTo || (item.status === "COMPLETED" ? "Unassigned" : "Available to claim")}</span>{item.assignedTo === currentUser && item.status !== "COMPLETED" ? <select value={item.status} onChange={(event) => onUpdate([item.id], event.target.value as PriorityQueueStatus)}><option value="ASSIGNED">Assigned</option><option value="IN_REVIEW">In progress</option><option value="BLOCKED">Blocked</option><option value="COMPLETED">Completed</option></select> : <strong className={`queue-status ${item.status.toLowerCase()}`}>{item.status.replace("_", " ")}</strong>}<span className="queue-result">{item.finalAction ? <><ActionPill action={item.finalAction} /><small>{item.finalAction === "MERGE" ? `${item.finalTargetName || "Target"} · ${item.finalTargetId || "Missing ID"}` : item.finalAction === "CREATE" ? item.finalTargetName || item.name : "No target brand"}</small></> : <small>{item.status === "COMPLETED" ? "Completed result recorded" : "Pending Step 3 export"}</small>}</span></div>)}</div>{visible.length > 100 && <p className="preview-more">Showing the first 100 of {visible.length.toLocaleString()} rows</p>}</>}
  </section>;
}

function Imports({ batches, priorityQueue, currentUser, teamMembers, onChooseTeamMember, syncConnected, savePending, saveBusy, saveCountdown, onSave, onImport, onAddPriority, onUpdatePriority, onResetPriority, onRemovePriority, onStartPriority, onNavigate, onRestart, ubqSource }: { batches: ImportBatch[]; priorityQueue: PriorityQueueItem[]; currentUser: string; teamMembers: string[]; onChooseTeamMember: (member: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; onSave: () => void; onImport: (name: string, rows: ReturnType<typeof parseCsv>) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onUpdatePriority: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onResetPriority: (ids: string[]) => void; onRemovePriority: (ids: string[]) => void; onStartPriority: (ids: string[]) => void; onNavigate: (v: View) => void; onRestart: () => void; ubqSource: UbqSource | null }) {
...
function BulkOutput({ records, batch, data, onUpdate, onSetExcluded, onReopen, onCompletePriority, onNavigate, onRestart }: { records: BrandRecord[]; batch?: ImportBatch; data: AppData; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onSetExcluded: (id: string, excluded: boolean) => void; onReopen: (ids: string[]) => void; onCompletePriority: (batch?: ImportBatch) => void; onNavigate: (view: View) => void; onRestart: () => void }) {
