...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const initialSync = protectedTriageRef.current
      ? getGitHubWorkspaceStatus(githubSession.token).then((status) => {
      ...
      : githubLiveSyncRef.current("connect").then(() => setSavePending(false));
  ...
  useEffect(() => {
    if (online && githubSession && storageHydrated && githubInitialSyncDoneRef.current && !USE_SYNC_SERVICE && !protectedTriageRef.current) void githubLiveSyncRef.current("online").then(() => setSavePending(false)).catch(() => undefined);
  ...
  async function prepareProtectedExport() {
    if (!githubSession || USE_SYNC_SERVICE || !navigator.onLine) return true;
    setSyncBusy(true);
    try {
      ...
      setToast("The final team safety sync failed. Your CSV was not downloaded; reconnect and try again.");
      return false;
    } finally { setSyncBusy(false); }
  }
  ...
  function applyAdminUploadResults(batchId: string | undefined, attemptedIds: string[], rows: AdminUploadResultRow[], exportFilename: string, resultFilename: string, moveFailuresToReview: boolean) {
    if (!batchId) return;
    const now = new Date().toISOString();
    setData((prev) => {
      ...
      const batches = prev.batches.map((item) => {
        ...
        const applied = applyAdminUploadResultsToRecords(item.records, attemptedIds, rows, resultFilename, now, moveFailuresToReview);
    ...
    setToast(`${successfulCount} successful · ${failedCount} failed${moveFailuresToReview && failedCount ? " · failures returned to Step 2" : ""}`);
  }
  ...
  return <div className={`app-shell ebay-theme ${experienceMode}-mode`}>
    ...
    <main>
      <header className="topbar">
        ...
        <button className={`top-sync-state ${syncBusy ? "syncing" : protectedTriage ? "protected" : savePending ? "pending" : githubRemoteUpdate ? "update" : teamConnected ? "connected" : "offline"}`} disabled={syncBusy} onClick={() => void syncAndPullNow()} title={protectedTriage ? "Protected triage mode: team updates are detected but not applied until export" : githubSession ? "Save local changes and pull team changes now" : "Connect Team Sync in settings"}><RefreshCw className={syncBusy ? "spinning" : ""} size={14} /><span aria-live="polite">{syncBusy ? "Saving & pulling…" : protectedTriage ? `Protected triage · ${githubRemoteUpdate ? "team update waiting" : `checking in ${syncCountdown}s`}` : githubSession ? savePending ? `Save team changes · auto-save in ${syncCountdown}s` : `Saved · next check in ${syncCountdown}s` : "Connect Team Sync"}</span></button>
...
function SmartTargetPicker({ brands, query, selectedId, onQuery, onSelect }: { brands: CatalogBrand[]; query: string; selectedId: string; onQuery: (query: string) => void; onSelect: (brand: CatalogBrand) => void }) {
  ...
  return <div className="smart-target-picker"><label className="field"><span>Find existing merge target</span><div className="smart-target-search"><Search size={15} /><input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="Search brand name, alias, or brand_ ID…" /></div><small>Only the best 10 matches are loaded. Name, aliases, BrandID, and fuzzy similarity are checked.</small></label>{selected && <div className="smart-target-selected"><Check size={15} /><span><b>{selected.name}</b><code>{selected.id}</code></span></div>}{query.trim() && <div className="smart-target-results">{results.length ? results.map(({ brand, score }) => <button type="button" className={brand.id === selectedId ? "selected" : ""} key={brand.id} onClick={() => onSelect(brand)}><span><b>{brand.name}</b><small>{brand.aliases?.slice(0, 2).join(" · ") || brand.source}</small></span><code>{brand.id}</code><em>{score >= 108 ? "Exact" : score >= 92 ? "Strong" : "Fuzzy"}</em></button>) : <p>No matching existing brand. Try the exact BrandID or fewer words.</p>}</div>}</div>;
}
...
function ReviewQueue({ records, batch, brands, ubqRows, knownBrandIds, focusIds, onClearFocus, onUpdate, onSelect, query, onNavigate, onRestart }: { records: BrandRecord[]; batch?: ImportBatch; brands: CatalogBrand[]; ubqRows: { id: string; name: string }[]; knownBrandIds: Set<string>; focusIds: string[]; onClearFocus: () => void; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onSelect: (r: BrandRecord) => void; query: string; onNavigate: (view: View) => void; onRestart: () => void }) {
    ...
    <div className="table-panel"><div className="data-table review-table research-enabled"><div className="table-row table-head-row"><div><input type="checkbox" checked={visible.length > 0 && visible.every((r) => checked.includes(r.id))} onChange={(e) => setChecked(e.target.checked ? visible.map((r) => r.id) : [])} /></div><div>{rootMode ? "Root brand" : "Unmapped brand"}</div><div>Normalized</div><div>Action</div><div>Source</div><div>Confidence</div><div>Status</div><div>Manual research</div><div>Edit</div></div>
      {visible.map((r) => <Fragment key={r.id}>
        <div className={`table-row ${inlineEditId === r.id ? "editing" : ""}`} onClick={() => onSelect(r)}>
          ...
          <div className="brand-cell"><b>{r.name}</b>{rootMode ? <><span>{r.id}</span><span className="ubq-badge"><Check size={10} />Root source</span></> : r.ubqVerified ? <><span>{r.id}</span><span className="ubq-badge"><Check size={10} />ID verified</span></> : <span className="missing-brand-id">Missing ID — click to {ubqRows.length ? "search UBQ" : "enter ID"}</span>}{!rootMode && r.relatedUbq?.length ? <span className="ubq-family-badge"><Boxes size={10} />{r.relatedUbq.length} related UBQ name{r.relatedUbq.length === 1 ? "" : "s"}</span> : null}{r.previouslyMergedStillPresent ? <span className="stale-merged-badge"><History size={10} />Previously merged · still in UBQ</span> : null}</div>
...
function BulkOutput({ records: allRecords, batch, data, currentUser, onUpdate, onSetExcluded, onReopen, onCompletePriority, onApplyAdminUploadResults, onRecordRootExport, onBeforeExport, onExported, onNavigate, onRestart }: { records: BrandRecord[]; batch?: ImportBatch; data: AppData; currentUser: string; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onSetExcluded: (id: string, excluded: boolean) => void; onReopen: (ids: string[]) => void; onCompletePriority: (batch?: ImportBatch) => void; onApplyAdminUploadResults: (batchId: string | undefined, attemptedIds: string[], rows: AdminUploadResultRow[], exportFilename: string, resultFilename: string, moveFailuresToReview: boolean) => void; onRecordRootExport: (changes: AppData["rootChanges"][string][], filename: string) => void; onBeforeExport: () => Promise<boolean>; onExported: () => void; onNavigate: (view: View) => void; onRestart: () => void }) {
  ...
  function finishAdminResult(rows: AdminUploadResultRow[], resultFilename: string, moveFailures: boolean) {
    if (!exportConfirmation) return;
    const failedIds = rows.filter((row) => row.status === "FAILED").map((row) => row.unmappedBrandId);
    onApplyAdminUploadResults(batch?.id, exportConfirmation.records.map((record) => record.id), rows, exportConfirmation.filename, resultFilename, moveFailures);
  ...
  return <><WorkflowStepper stage={3} onNavigate={onNavigate} onRestart={onRestart} hasImport={records.length > 0} outputReady={ready} rootMode={rootMode} counts={getTriageCounts(records, rootMode)} />
    ...
    {!records.length ? <div className="panel"><EmptyState icon={FileUp} title="No brands have reached Step 3" body="Start at Step 1 by adding brands, then confirm every required decision in Step 2." action={<button className="primary" onClick={() => onNavigate("imports")}>Go to Step 1</button>} /></div> : !ready ? <div className="output-blocked"><div className="output-status-icon"><FileClock size={24} /></div><h2>Your file needs attention</h2><p>Return to Step 2 and resolve every check before downloading the final file.</p><div className="output-checks">{!rootMode && <span className={invalidIds ? "bad" : "good"}>{invalidIds ? <X size={14} /> : <Check size={14} />}Valid unmapped IDs <b>{invalidIds ? `${invalidIds} missing` : "Complete"}</b></span>}<span className={needs ? "bad" : "good"}>{needs ? <X size={14} /> : <Check size={14} />}Review decisions <b>{needs ? `${needs} remaining` : "Complete"}</b></span><span className={(rootMode ? rootIncomplete : invalidMerges) ? "bad" : "good"}>{(rootMode ? rootIncomplete : invalidMerges) ? <X size={14} /> : <Check size={14} />}MERGE targets <b>{(rootMode ? rootIncomplete : invalidMerges) ? `${rootMode ? rootIncomplete : invalidMerges} incomplete` : "Complete"}</b></span>{!rootMode && <span className={invalidCreates ? "bad" : "good"}>{invalidCreates ? <X size={14} /> : <Check size={14} />}CREATE target names <b>{invalidCreates ? `${invalidCreates} incomplete` : "Complete"}</b></span>}</div><button className="primary" onClick={() => onNavigate("review")}>Return to Step 2 review</button></div> : rootMode ? <>
      ...
      <div className="output-success"><div className="output-status-icon"><Check size={25} /></div><div><span>THIRD STEP COMPLETE</span><h2>{includedRecords.length.toLocaleString()} brand mappings will be downloaded</h2><p>{preparingExport ? "Running the final protected team sync before creating your file…" : excludedRecords.length ? `${excludedRecords.length.toLocaleString()} brand${excludedRecords.length === 1 ? " is" : "s are"} intentionally excluded.` : "Download performs one final team safety check, then resumes normal synchronization."}</p></div><button className="primary output-download" disabled={!includedRecords.length || preparingExport} onClick={async () => { setPreparingExport(true); const safe = await onBeforeExport(); if (!safe) { setPreparingExport(false); return; } const filename = `brandmaster-${currentUser.toLowerCase()}-bulk-brand-mappings-${new Date().toISOString().slice(0, 10)}.csv`; download(filename, toCsv(includedRecords)); onCompletePriority(batch ? { ...batch, records: includedRecords } : batch); setExportConfirmation({ filename, records: includedRecords }); onExported(); setPreparingExport(false); }}><ArrowDownToLine size={17} />{preparingExport ? "Safety checking…" : `Download ${includedRecords.length.toLocaleString()} rows`}</button></div>
...
function DecisionDrawer({ record, records, brands, ubqRows, onClose, onSave, onApplyRelated }: { record: BrandRecord; records: BrandRecord[]; brands: CatalogBrand[]; ubqRows: { id: string; name: string }[]; onClose: () => void; onSave: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onApplyRelated: (ids: string[], targetId: string, targetName: string) => void }) {
  ...
  return <><div className="drawer-scrim" onClick={onClose} /><aside className="drawer"><div className="drawer-head"><div><span>BRAND DECISION</span><h2>{record.name}</h2></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div><div className="drawer-body">
    ...
    <section><h3>{rootMode ? "Root recommendation" : "Your decision"}</h3><div className="action-picker">{(["MERGE", "CREATE", "SKIP", "DELETE"] as Action[]).map((a) => <button key={a} className={`${a.toLowerCase()} ${action === a ? "active" : ""}`} onClick={() => changeAction(a)}><span>{a === "MERGE" ? "↗" : a === "CREATE" ? "+" : a === "SKIP" ? "–" : "×"}</span>{rootMode ? (a === "MERGE" ? "CONSOLIDATE" : a === "CREATE" ? "EDIT / KEEP" : a) : a}<Check size={14} /></button>)}</div>
      {action === "MERGE" && <div className="merge-fields"><SmartTargetPicker brands={brands} query={targetQuery} selectedId={target} onQuery={setTargetQuery} onSelect={(brand) => { setTarget(brand.id); setTargetName(brand.name); setTargetQuery(brand.id); }} /><label className="field"><span>TargetBrandID</span><input value={target} onChange={(e) => { const value = e.target.value.trim(); setTarget(value); setTargetQuery(value); const brand = brands.find((item) => item.id === value); if (brand) setTargetName(brand.name); }} placeholder="brand_xxxxxxxxxxxxxxxxxxxxxx" /></label><label className="field"><span>TargetBrandName</span><input value={targetName} onChange={(e) => setTargetName(e.target.value)} placeholder="Canonical brand name" /></label></div>}
...
function Analytics({ records, ledger, historicalMappings, priorityQueue, currentUser }: { records: BrandRecord[]; ledger: LedgerEntry[]; historicalMappings: HistoricalMappingEntry[]; priorityQueue: PriorityQueueItem[]; currentUser: string }) {
  ...
  const adminSuccessful = records.filter((record) => record.adminUploadStatus === "SUCCESS").length;
  const adminFailed = records.filter((record) => record.adminUploadStatus === "FAILED").length;
  const adminPending = records.filter((record) => !record.adminUploadStatus && record.status !== "needs-review" && !record.excludedFromExport).length;
  const safeUser = currentUser.toLowerCase().replace(/[^a-z0-9]+/g, "-");
  function downloadExcelReport() {
    download(`brandmaster-analytics-${safeUser}-${new Date().toISOString().slice(0, 10)}.xls`, analyticsExcelXml(allActivity, records, priorityQueue), "application/vnd.ms-excel;charset=utf-8");
  }
  async function downloadPdfReport() {
    const { jsPDF } = await import("jspdf");
    const doc = new jsPDF({ unit: "pt", format: "letter" });
    const actionCounts = { CREATE: 0, MERGE: 0, SKIP: 0, DELETE: 0 };
    allActivity.forEach((entry) => { actionCounts[entry.action] += 1; });
  ...
  return <><PageHead eyebrow="MAPPING PERFORMANCE" title="Progress & effort analytics" body="Historical imports, live reviews, and confirmed Admin upload results show both team effort and delivered mapping progress." actions={<><button className="secondary" onClick={downloadExcelReport}><ArrowDownToLine size={15} />Download Excel</button><button className="primary" onClick={() => void downloadPdfReport()}><ArrowDownToLine size={15} />Download PDF</button></>} />
    ...
    <section className="admin-outcome-analytics"><div><span><ShieldCheck size={21} /></span><div><small>ADMIN DELIVERY PROGRESS</small><h2>{adminSuccessful.toLocaleString()} mappings successfully applied</h2><p>Based on imported Admin result files and explicit all-success confirmations.</p></div></div><aside><b className="success">{adminSuccessful}<small>successful</small></b><b className={adminFailed ? "failed" : ""}>{adminFailed}<small>failed</small></b><b>{adminPending}<small>awaiting result</small></b></aside></section>
