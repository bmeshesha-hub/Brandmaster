...
import { CompletedBrandDetail, findCompletedBrandDetails } from "@/lib/completed-brands";
...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  function startSourceWorklist(source: Exclude<WorkflowSource, "IMPORT">, ids: string[], priorityItems: PriorityQueueItem[] = []) {
    ...
    if (ids.length > MAX_WORKLIST_SIZE) { setToast(`Choose no more than ${MAX_WORKLIST_SIZE} brands for one review run.`); return; }
    if (!queueUser) { setToast("Choose who is working before starting validation"); return; }
    const openBatch = activeTriageForUser(dataRef.current, queueUser);
    if (openBatch) {
      setView(resolveWorkflowCheckpoint(undefined, openBatch) || "review");
  ...
  function addPriorityRows(source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) {
    ...
    if (rows.length > MAX_WORKLIST_SIZE) {
      setToast(`Add no more than ${MAX_WORKLIST_SIZE} brands at a time.`);
      return;
    }
    const completedRows = findCompletedBrandDetails(dataRef.current, rows);
...
function CleanWorkflowHeader({ view, batch, owner, checkpointAt, savePending, saveBusy, connected, onNavigate, onSave, onRestart }: { view: "imports" | "review" | "output"; batch?: ImportBatch; owner: string; checkpointAt?: string; savePending: boolean; saveBusy: boolean; connected: boolean; onNavigate: (view: View) => void; onSave: () => void; onRestart: () => void }) {
  ...
  return <section className="clean-progress" aria-label="Triage progress">
    ...
    <div className="clean-progress-body"><div className="clean-progress-steps">{steps.map((step) => <button key={step.number} className={stage === step.number ? "active" : stage > step.number ? "done" : ""} disabled={!step.enabled} onClick={() => onNavigate(step.target)}><span>{stage > step.number ? <Check size={16} /> : step.number}</span><div><b>{step.title}</b><small>{step.detail}</small></div></button>)}</div><div className="clean-process-controls">
      ...
      <details className="process-actions"><summary><Settings size={16} /><span>Process actions</span><ChevronDown size={15} /></summary><div className="process-actions-menu"><header><small>PROCESS CONTROL</small><b>Manage this triage safely</b><p>The current step is saved locally. Team Save writes the same checkpoint to the shared workspace.</p></header><button disabled={!batch} onClick={() => onNavigate(resumeTarget)}><Play size={16} /><span><b>Resume triage</b><small>Open the latest valid step</small></span><ChevronRight size={15} /></button><button disabled={!connected || saveBusy} onClick={onSave}><UploadCloud size={16} /><span><b>Save progress now</b><small>Save Step {stage} and every decision</small></span><ChevronRight size={15} /></button><button disabled={!batch || stage === 1} onClick={() => onNavigate(previousTarget)}><ChevronLeft size={16} /><span><b>Return to previous step</b><small>Keep all saved work unchanged</small></span><ChevronRight size={15} /></button><button className="reset" disabled={!batch} onClick={onRestart}><RotateCcw size={16} /><span><b>Reset / restart triage</b><small>Confirmation is required</small></span><ChevronRight size={15} /></button></div></details>
    </div></div>
  </section>;
}
...
function Imports({ cleanMode, batches, activeBatchId, priorityQueue, currentUser, pinnedQueueIds, teamMembers, onChooseTeamMember, onTogglePin, syncConnected, savePending, saveBusy, saveCountdown, lastSavedAt, onSave, onImport, onAddPriority, onUpdatePriority, onResetPriority, onRemovePriority, onAdminDone, onStartPriority, onNavigate, onRestart, ubqSource }: { cleanMode?: boolean; batches: ImportBatch[]; activeBatchId?: string; priorityQueue: PriorityQueueItem[]; currentUser: string; pinnedQueueIds: string[]; teamMembers: string[]; onChooseTeamMember: (member: string) => void; onTogglePin: (id: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; lastSavedAt?: string; onSave: () => void; onImport: (name: string, rows: ReturnType<typeof parseCsv>) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onUpdatePriority: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onResetPriority: (ids: string[]) => void; onRemovePriority: (ids: string[]) => void; onAdminDone: (ids: string[]) => void; onStartPriority: (ids: string[]) => void; onNavigate: (v: View) => void; onRestart: () => void; ubqSource: UbqSource | null }) {
  ...
  function accept(file?: File, target = destination.current) { if (!file) return; if (!file.name.toLowerCase().endsWith(".csv")) { setError("Please choose a CSV file."); return; } const reader = new FileReader(); reader.onload = () => { const rows = parseCsv(String(reader.result)); if (!rows.length) setError("No brand rows found. Include UnmappedBrandID and UnmappedBrandName columns."); else if (rows.length > MAX_WORKLIST_SIZE) setError(`Add no more than ${MAX_WORKLIST_SIZE} brands at a time.`); else { setError(""); if (target === "queue") onAddPriority("CSV", rows); else onImport(file.name, rows); } }; reader.readAsText(file); }
  ...
  const currentBatch = storedBatch ? triageWorklistWindow(storedBatch, MAX_WORKLIST_SIZE) : undefined;
  ...
  return <>{!cleanMode && <WorkflowStepper stage={1} onNavigate={onNavigate} onRestart={onRestart} hasImport={Boolean(currentBatch)} outputReady={currentOutputReady} owner={currentBatch?.owner || currentUser || "Shared team"} counts={currentCounts} basketRecords={currentBatch?.records || []} />}
    <PageHead eyebrow={cleanMode ? "STEP 1 OF 3" : "FIRST STEP · TEAM VIEW"} title={cleanMode ? "Start by adding or picking brands" : "What would you like to review?"} body={cleanMode ? "Choose one clear path below. Every worklist is limited to 10 brands." : "Continue team work or add a new list. Choose one path below."} />
    {cleanMode && <section className="step1-choice-grid" aria-label="Choose how to start Step 1">
      <button className={`step1-choice-card add ${cleanChoice === "add" ? "selected" : ""}`} aria-pressed={cleanChoice === "add"} onClick={() => { if (currentBatch) { onRestart(); return; } setCleanChoice("add"); setQueueOpen(false); setInputMode("paste"); }}>
        <span className="step1-choice-number">1</span><span className="step1-choice-icon"><FileUp size={30} /></span>
        <span className="step1-choice-copy"><small>ADD NEW WORK</small><b>Add brands</b><p>Upload a CSV or paste brand names.</p><em>{currentBatch ? "A saved worklist is active" : "1–10 brands maximum"}</em></span>
        <span className="step1-choice-action">{currentBatch ? "Start over to add brands" : "Open add brands"} <ChevronRight size={18} /></span>
      </button>
    ...
    <section className={`compact-import ${inputMode === "priority" ? "priority-intake-open" : ""} ${cleanMode && cleanChoice === "add" ? "clean-add-open" : ""} ${cleanMode && cleanChoice !== "add" ? "clean-choice-hidden" : ""}`}>{cleanMode && <div className="step1-expanded-head"><span><FileUp size={20} /></span><div><small>OPTION 1 · ADD BRANDS</small><b>Add 1–10 brands</b><p>Paste names directly or add a CSV file.</p></div><button className="secondary" onClick={() => setCleanChoice(null)}><X size={15} />Close</button></div>}
      {cleanMode ? <div className="clean-add-composer">
        ...
        <div className="clean-add-guidance">
          ...
          <span className={ubqSource ? "ready" : "missing"}>{ubqSource ? <Check size={14} /> : <CircleHelp size={14} />}{ubqSource ? "Brand IDs will be matched automatically" : "Brand ID matching is not configured"}</span>
        </div>
        ...
        {inputMode === "csv" ? <div className={`dropzone compact ${drag ? "drag" : ""}`} onDragOver={(e) => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={drop} onClick={() => { destination.current = "validate"; input.current?.click(); }}><input ref={input} type="file" accept=".csv,text/csv" hidden onChange={(e: ChangeEvent<HTMLInputElement>) => { accept(e.target.files?.[0]); e.target.value = ""; }} /><div className="drop-icon"><UploadCloud size={23} /></div><div><h2>Drop CSV or click to browse</h2><p>Brand ID + Brand Name · maximum 10 brands</p></div><button className="primary" onClick={(event) => { event.stopPropagation(); destination.current = "validate"; input.current?.click(); }}>Choose & validate</button></div> : inputMode === "paste" ? <div className="compact-paste"><textarea value={brandNames} onChange={(e) => setBrandNames(e.target.value)} placeholder={"One brand per line…\npegaso\nb & p rods\nvolkswagen oe"} /><div className="compact-paste-footer"><div className={`id-mini ${ubqSource ? "ready" : ""}`}>{ubqSource ? <Check size={12} /> : <CircleHelp size={12} />}{ubqSource ? "UBQ IDs ready" : "UBQ IDs not configured"}</div>{!ubqSource && <button className="text-button" onClick={() => onNavigate("settings")}>Configure in Validation modules →</button>}<span>{pastedNames.length} / {MAX_WORKLIST_SIZE}</span><button className="primary" disabled={!pastedNames.length || pastedNames.length > MAX_WORKLIST_SIZE} onClick={validatePasted}><WandSparkles size={15} />Validate now</button></div></div> : <div className="priority-intake"><div className="priority-intake-head"><span><Activity size={20} /></span><div><small>SHARED TEAM INTAKE</small><h2>Add urgent brands for the team</h2><p>Add 1–10 brands. They enter the Available queue without starting validation or assigning an owner.</p></div></div><div className="priority-intake-grid"><button className="priority-upload-card" onClick={() => priorityInput.current?.click()}><input ref={priorityInput} type="file" accept=".csv,text/csv" hidden onChange={(event) => { accept(event.target.files?.[0], "queue"); event.target.value = ""; }} /><span><FileUp size={22} /></span><b>Upload urgent-brand CSV</b><small>Maximum 10 brands</small><em>Choose CSV</em></button><div className="priority-paste-card"><label><b>Paste urgent brand names</b><small>One per line · maximum 10</small></label><textarea value={priorityNames} onChange={(event) => setPriorityNames(event.target.value)} placeholder={"motocorse\npalloo-autoparts\nsteib\ninteva"} /><div><span className={priorityPastedNames.length > MAX_WORKLIST_SIZE ? "limit-exceeded" : ""}>{priorityPastedNames.length} / {MAX_WORKLIST_SIZE}</span><button className="priority" disabled={!priorityPastedNames.length || priorityPastedNames.length > MAX_WORKLIST_SIZE} onClick={() => { onAddPriority("PASTE", priorityPastedNames.map((name, index) => ({ id: `missing_id_${String(index + 1).padStart(5, "0")}`, name }))); setPriorityNames(""); }}><Activity size={14} />Add to High Priority Queue</button></div></div></div></div>}
      </>}
    </section>{error && (!cleanMode || cleanChoice === "add") && <div className="error-banner"><CircleHelp size={17} />{error}</div>}
...
function ReviewQueue({ cleanMode, records, batch, brands, ubqRows, knownBrandIds, focusIds, onClearFocus, onUpdate, onResolveWithoutMapping, onSelect, query, onNavigate, onRestart }: { cleanMode?: boolean; records: BrandRecord[]; batch?: ImportBatch; brands: CatalogBrand[]; ubqRows: { id: string; name: string }[]; knownBrandIds: Set<string>; focusIds: string[]; onClearFocus: () => void; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onResolveWithoutMapping: (ids: string[], resolution: NonNullable<BrandRecord["triageResolution"]>, note?: string) => void; onSelect: (r: BrandRecord) => void; query: string; onNavigate: (view: View) => void; onRestart: () => void }) {
  ...
  return <>{!cleanMode && <WorkflowStepper stage={2} onNavigate={onNavigate} onRestart={onRestart} hasImport outputReady={exportReady} rootMode={rootMode} counts={triageCounts} />}<PageHead eyebrow={rootMode ? "ROOT CLEANUP · SECOND STEP" : cleanMode ? undefined : focusedReview ? "SECOND STEP · FOCUSED REVIEW" : "SECOND STEP · REVIEW"} title={rootMode ? "Review Root cleanup decisions" : cleanMode ? `Review all ${focusedRecords.length} imported brand${focusedRecords.length === 1 ? "" : "s"}` : focusedReview ? `Review ${focusedRecords.length} selected brand${focusedRecords.length === 1 ? "" : "s"}` : "Review decisions"} body={cleanMode ? `${focusedNeeds} need${focusedNeeds === 1 ? "s" : ""} your decision and ${reviewReady} ${reviewReady === 1 ? "is" : "are"} already ready. All ${focusedRecords.length} imported brands are shown below.` : focusedReview ? "Only the rows returned from Step 3 are shown. The rest of the batch remains ready and unchanged." : needs ? `${needs} brand${needs === 1 ? " needs" : "s need"} your decision. You can return to Step 1 at any time without losing this work.` : rootMode ? "All Root cleanup decisions are ready." : "All decisions are ready. Continue to Step 3 when you are satisfied."} actions={<><button className="secondary" onClick={() => onNavigate("imports")}><ChevronLeft size={15} />Back</button>{focusedReview && <button className="secondary" onClick={onClearFocus}>Show all Step 2 brands</button>}{!cleanMode && unverified > 0 && <button className="secondary" onClick={() => onNavigate("settings")}><Database size={15} />Fix {unverified} missing IDs</button>}{rootMode ? <button className="primary clean-next-button" disabled={!exportReady} onClick={() => onNavigate("brands")}><Check size={15} />Finish</button> : <button className="primary clean-next-button" disabled={!exportReady} title={!exportReady ? "Resolve the remaining checks first" : "Continue to the output file"} onClick={() => onNavigate("output")}>Continue to Step 3 <ChevronRight size={15} /></button>}</>} />
    {cleanMode && <section className="review-accounting" aria-label="Step 2 worklist accounting"><div className="review-accounting-title"><span><ShieldCheck size={21} /></span><div><small>COMPLETE STEP 2 WORKLIST</small><b>All {focusedRecords.length} imported brands are accounted for</b><p>The table starts with All brands selected. Filters below only change what you see; they never remove work.</p></div></div><div className="review-accounting-counts"><button className={filter === "all" ? "active all" : "all"} onClick={() => setFilter("all")}><b>{focusedRecords.length}</b><small>Imported and shown</small></button><button className={filter === "needs-review" ? "active attention" : "attention"} onClick={() => setFilter("needs-review")}><b>{focusedNeeds}</b><small>Need your decision</small></button><button className={filter === "ready" ? "active ready" : "ready"} onClick={() => setFilter("ready")}><b>{focusedRecords.length - focusedNeeds}</b><small>Already ready</small></button></div><div className="review-accounting-visible"><span>Currently showing <b>{visible.length} of {focusedRecords.length}</b> brands</span>{visible.length !== focusedRecords.length && <button onClick={() => { setFilter("all"); setActionFilter("ALL"); }}>Show all {focusedRecords.length}</button>}</div></section>}
    {intakeDecisions.length > 0 && (intakeNotImported ? <details className="intake-summary has-exclusions" open><summary><span><ShieldCheck size={18} /><b>Step 1 intake: {intakeDecisions.length} submitted · {intakeImported} imported · {intakeNotImported} not imported</b></span><small>Review why some brands are not in the Step 2 worklist</small><ChevronDown size={16} /></summary><div className="intake-summary-table"><div><b>Brand</b><b>Outcome</b><b>Reason</b></div>{intakeDecisions.map((item, index) => <div key={`${item.id}:${item.brand}:${index}`}><strong>{item.brand}</strong><span className={item.outcome === "IMPORTED" ? "imported" : "not-imported"}>{item.outcome === "IMPORTED" ? "Imported" : "Not imported"}</span><p>{item.reason}{item.action ? <small>{item.action}{item.date ? ` · ${fmtDate(item.date)}` : ""}</small> : null}</p></div>)}</div></details> : <section className="intake-confirmed"><Check size={18} /><span><b>All {intakeImported} submitted brands were imported successfully</b><small>Nothing was skipped or removed. All {intakeImported} appear in the Step 2 worklist below.</small></span></section>)}
