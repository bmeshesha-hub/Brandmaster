...
type GitHubRemoteUpdate = { revision: string; workspace: SharedWorkspaceSnapshot };
...
export default function BrandmasterApp() {
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        const remote = await getGitHubWorkspace(githubSession!.token); if (!active) return;
        setGitHubTeamSync(remote.workspace?.sync);
        const lastRevision = localStorage.getItem("brandmaster-github-revision");
        setGitHubRemoteUpdate(remote.revision && remote.workspace && remote.revision !== lastRevision ? { revision: remote.revision, workspace: remote.workspace } : null);
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="page">
        {githubRemoteUpdate && view !== "settings" && <button className="global-sync-notice" onClick={() => navigate("settings")}><Bell size={16} /><span><b>New Brandmaster team update</b><small>{githubRemoteUpdate.workspace.sync?.lastSyncedBy ? `@${githubRemoteUpdate.workspace.sync.lastSyncedBy} saved a newer workspace.` : "A collaborator saved a newer workspace."} Pull and merge it safely.</small></span><ChevronRight size={17} /></button>}
