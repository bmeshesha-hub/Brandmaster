...
function ReviewQueue({ records, batch, catalogBrands, knownBrandIds, simpleMode, onUpdate, onAnnotate, onSelect, query, onNavigate, onRestart }: { records: BrandRecord[]; batch?: ImportBatch; catalogBrands: CatalogBrand[]; knownBrandIds: Set<string>; simpleMode: boolean; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onAnnotate: (id: string, changes: Partial<BrandRecord>) => void; onSelect: (r: BrandRecord) => void; query: string; onNavigate: (view: View) => void; onRestart: () => void }) {
  ...
  return <><WorkflowStepper stage={2} onNavigate={onNavigate} onRestart={onRestart} hasImport outputReady={exportReady} rootMode={rootMode} /><PageHead eyebrow={rootMode ? "ROOT CLEANUP · SECOND STEP" : "SECOND STEP · REVIEW"} title={rootMode ? "Review Root cleanup decisions" : "Review decisions"} body={needs ? `${needs} brand${needs === 1 ? " needs" : "s need"} your decision. Review one brand at a time, then continue to Step 3.` : rootMode ? "All Root cleanup decisions are ready." : "All decisions are ready. Continue to Step 3 when you are satisfied."} actions={<>{unverified > 0 && <button className="secondary" onClick={() => onNavigate("settings")}><Database size={15} />Fix {unverified} missing IDs</button>}{rootMode ? <button className="primary" disabled={!exportReady} onClick={() => onNavigate("brands")}><Check size={15} />Finish Root review</button> : <button className="primary" disabled={!exportReady} title={!exportReady ? "Resolve the remaining checks first" : "Continue to the output file"} onClick={() => onNavigate("output")}>Continue to Step 3 <ChevronRight size={15} /></button>}</>} />
    ...
    <details className="review-disclosure review-options"><summary><span><WandSparkles size={17} /><b>Review options</b></span><small>AI review and batch table</small><ChevronDown size={16} /></summary><div className="review-disclosure-body"><AiReviewAssist records={records} knownBrandIds={knownBrandIds} onUpdate={onUpdate} /><div className="review-view-switch"><div><b>Choose a review style</b><p>Guided review shows one brand at a time. Table view is for batch work.</p></div><span><button className={displayMode === "guided" ? "active" : ""} onClick={() => setDisplayMode("guided")}><WandSparkles size={14} />Guided review</button><button className={displayMode === "table" ? "active" : ""} onClick={() => setDisplayMode("table")}><Database size={14} />Batch table</button></span></div></div></details>
