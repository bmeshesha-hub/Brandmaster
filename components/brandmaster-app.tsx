...
function LearningRuleInspector({ rule, allRules, currentUser, onClose, onUpdate, onSendToQueue }: { rule: LearningRule; allRules: LearningRule[]; currentUser: string; onClose: () => void; onUpdate: (changes: LearningRuleChanges, type: LearningModerationEventType, note: string) => void; onSendToQueue: (changes: LearningRuleChanges, note: string) => void }) {
  ...
  function sendToQueue() {
    if (!valid) return;
    const message = note.trim() || `Review the proposed ${action}${targetName ? ` → ${targetName}` : ""} correction in Admin before this VLR rule is reactivated.`;
  ...
  return <><div className="fresh-dialog-scrim" onClick={onClose} /><section className="learning-inspector" role="dialog" aria-modal="true" aria-labelledby="learning-inspector-title">
    ...
    <footer><button className="secondary" onClick={onClose}>Cancel</button><button className="secondary learning-send-queue" disabled={!valid} onClick={sendToQueue}><Users size={14} />Disable &amp; send to Team Queue</button><button className="primary" disabled={!valid} onClick={save}><Check size={14} />Save moderated rule</button></footer>
  </section></>;
}
