...
function Imports({ batches, priorityQueue, currentUser, syncConnected, onImport, onAddPriority, onUpdatePriority, onResetPriority, onRemovePriority, onStartPriority, onNavigate, onRestart, ubqSource }: { batches: ImportBatch[]; priorityQueue: PriorityQueueItem[]; currentUser: string; syncConnected: boolean; onImport: (name: string, rows: ReturnType<typeof parseCsv>) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onUpdatePriority: (ids: string[], status: PriorityQueueStatus) => void; onResetPriority: (ids: string[]) => void; onRemovePriority: (ids: string[]) => void; onStartPriority: (ids: string[]) => void; onNavigate: (v: View) => void; onRestart: () => void; ubqSource: UbqSource | null }) {
  ...
  return <><WorkflowStepper stage={1} onNavigate={onNavigate} onRestart={onRestart} hasImport={batches.length > 0} />
    ...
    <PriorityQueue items={priorityQueue} currentUser={currentUser} syncConnected={syncConnected} onUpdate={onUpdatePriority} onReset={onResetPriority} onRemove={onRemovePriority} onStart={onStartPriority} onNavigate={onNavigate} />
...
function SmartCleanup({ data, ubqSource, onSaveRoot, onValidate, onAddPriority, onSetConfirmation, onNavigate }: { data: AppData; ubqSource: UbqSource | null; onSaveRoot: (brand: CatalogBrand) => void; onValidate: (source: "ROOT" | "UBQ", ids: string[]) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onSetConfirmation: (source: CleanupSource, rows: { brandId: string; name: string; fingerprint: string }[], status: "CONFIRMED" | "REOPENED") => void; onNavigate: (view: View) => void }) {
  ...
  return <><PageHead eyebrow="ADMIN TOOLS · DATA QUALITY" title="Smart brand cleanup" body="Scan the Root or UBQ table for likely junk, duplicates, alias conflicts, broken targets, and existing-brand matches. Work through 10–50 prioritized findings at a time." actions={<button className="primary cleanup-magic" disabled={scanning || !sourceCount} onClick={scan}><WandSparkles className={scanning ? "spinning" : ""} size={17} />{scanning ? "Analyzing data…" : issues.length ? "Scan again" : "Find cleanup opportunities"}</button>} />
    ...
    {!showConfirmed && selectedIssues.length === 1 && (selectedIssues[0].targetId || selectedIssues[0].related?.length) && <button className="cleanup-compare-float" onClick={() => setCompareIssue(selectedIssues[0])}><Boxes size={18} /><span><b>Compare these brands</b><small>{selectedIssues[0].name} and its suggested match</small></span><ChevronRight size={16} /></button>}
