...
import { buildMappingActivitySeries, cumulativeMappingSeries, MappingActivityEntry, MappingGranularity, summarizeMappingActivity } from "@/lib/analytics";
...
import { Action, AppData, BrandRecord, CatalogBrand, HistoricalMappingEntry, ImportBatch, LedgerEntry, SharedWorkspaceSnapshot, SourceMetadata, ValidationSettings, View, WorkflowSource } from "@/lib/types";
...
export default function BrandmasterApp() {
  ...
  async function applyWorkspaceSnapshot(payload: SharedWorkspaceSnapshot) {
    ...
    const restored: AppData = { ...EMPTY_DATA, ...payload.data, historicalMappings: payload.data.historicalMappings || [], rootChanges: payload.data.rootChanges || {}, sourceMeta: payload.data.sourceMeta || {}, validationSettings: { ...EMPTY_DATA.validationSettings, ...(payload.data.validationSettings || {}) } };
    setData(restored);
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="page">
        ...
        {view === "analytics" && <Analytics records={allRecords} ledger={data.ledger} historicalMappings={data.historicalMappings} />}
...
function Analytics({ records, ledger, historicalMappings }: { records: BrandRecord[]; ledger: LedgerEntry[]; historicalMappings: HistoricalMappingEntry[] }) {
  ...
  return <><PageHead eyebrow="MAPPING PERFORMANCE" title="Progress & effort analytics" body="Historical imports and live Brandmaster reviews share one timeline. Current worklist progress remains separate, so old activity never makes today’s queue look complete." actions={historicalMappings.length ? <span className="status ready"><History size={12} />{historicalMappings.length.toLocaleString()} historical actions</span> : undefined} />
    ...
    <section className="mapping-dashboard-grid"><div className="panel mapping-trend-panel"><div className="panel-head"><div><h2>Brand mapping actions over time</h2><p>Historical actions + live reviewed decisions</p></div><div className="analytics-toggle"><button className={granularity === "day" ? "active" : ""} onClick={() => setGranularity("day")}>Daily</button><button className={granularity === "week" ? "active" : ""} onClick={() => setGranularity("week")}>Weekly</button></div></div>{allActivity.length ? <MappingTrendChart entries={allActivity} granularity={granularity} /> : <EmptyState icon={TrendingUp} title="No mapping history yet" body="Add a Historical Mapping Progress CSV in Validation Modules or save decisions in Process & Review." />}</div>
...
function MappingTrendChart({ entries, granularity }: { entries: MappingActivityEntry[]; granularity: MappingGranularity }) {
  const buckets = useMemo(() => cumulativeMappingSeries(buildMappingActivitySeries(entries, granularity, new Date(), granularity === "day" ? 30 : 20)), [entries, granularity]);
  ...
  const labelEvery = granularity === "day" ? 3 : 2;
  ...
  </svg><p className="trend-caption"><CircleHelp size={12} />The chart combines imported historical actions with live reviews. Re-saving a live correction counts as additional effort.</p></div>;
}
...
function SettingsView({ data, ubqSource, onLoadUbq, onClear, onUpdateSettings, onSetReference, onAddDecisions, onAddHistoricalMappings, onBackup, onRestore, createSnapshot, applySnapshot, githubSession, onGitHubSession, githubRemoteUpdate, onGitHubRemoteUpdate, githubTeamSync, onGitHubTeamSync }: SettingsViewProps) {
  ...
  return <><PageHead eyebrow="OFFLINE DATA & VALIDATION" title="Reference tables" body="Load the catalog sources that make matching accurate. Files stay on this Mac and remain available offline." />
    <div className="module-layout"><div className="settings-content"><GitHubWorkspacePanel createSnapshot={createSnapshot} applySnapshot={applySnapshot} session={githubSession} onSession={onGitHubSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onTeamSync={onGitHubTeamSync} />
      ...
      <section><h2>Workspace data</h2><p>{data.batches.length} imports, {data.ledger.length} live reviewed decisions, {data.historicalMappings.length.toLocaleString()} historical mapping actions, and {(data.rootBrands.length + data.acaBrands.length + data.fpaBrands.length).toLocaleString()} reference brands are stored locally and included in workspace sync.</p><WorkspaceBackupPanel onBackup={onBackup} onRestore={onRestore} /><div className="danger-row"><div><b>Clear local workspace</b><p>Remove imports, references, settings, review history, historical mappings, and learned decisions.</p></div>{confirm ? <div className="confirm-actions"><button className="secondary" onClick={() => setConfirm(false)}>Cancel</button><button className="danger" onClick={() => { onClear(); setConfirm(false); }}><Trash2 size={15} />Clear everything</button></div> : <button className="danger-outline" onClick={() => setConfirm(true)}>Clear data</button>}</div></section>
