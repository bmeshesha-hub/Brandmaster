...
function OnlineBrandCleanupV2({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  async function lookupOnline(item: typeof candidates[number]) { setOnlineBusy(item.brand.id); try { const response = await fetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(item.brand.name)}&language=en&format=json&origin=*`); const result = await response.json() as { search?: { label?: string; description?: string; aliases?: { value: string }[] }[] }; const match = result.search?.[0]; if (match) { setOnline((current) => ({ ...current, [item.brand.id]: true })); setEdits((current) => ({ ...current, [item.brand.id]: { name: match.label || item.name, aliases: [...new Set([...(match.aliases || []).map((alias) => alias.value), ...item.aliases.split(",").map((alias) => alias.trim()).filter(Boolean)])].join(", ") } })); } } catch { setOnline((current) => ({ ...current, [item.brand.id]: false })); } finally { setOnlineBusy(null); } }
