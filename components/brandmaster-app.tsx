...
function OnlineBrandCleanupV2({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  const pageSize = 50;
  const candidates = useMemo(() => data.rootBrands.map((brand) => { const edit = edits[brand.id]; const name = edit?.name ?? brand.name.normalize("NFKC").replace(/\s+/g, " ").trim().toLowerCase().replace(/(^|[\s-])\p{L}/gu, (m) => m.toUpperCase()); const aliases = edit?.aliases ?? [...new Set(brand.aliases.map((alias) => alias.normalize("NFKC").replace(/\s+/g, " ").trim()).filter(Boolean))].join(", "); return { brand, name, aliases, changed: name !== brand.name || aliases.split(",").map((a) => a.trim()).join("|") !== brand.aliases.join("|"), confidence: online[brand.id] ? 92 : 78 }; }), [data.rootBrands, edits, online]);
  const filtered = candidates.filter((item) => `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const reviewRows = filtered.filter((item) => selected.includes(item.brand.id) && (filter === "ALL" || filter === "NEEDS_FIX" && item.changed || filter === "PENDING" && !decisions[item.brand.id] || filter === "APPROVED" && decisions[item.brand.id] === "APPROVE" || filter === "REJECTED" && decisions[item.brand.id] === "REJECT")); ; const pages = Math.max(1, Math.ceil(reviewRows.length / pageSize)); const pageRows = reviewRows.slice((page - 1) * pageSize, page * pageSize); const reviewed = Object.keys(decisions).length; const changed = Object.values(snapshots);
  const editFor = (item: typeof candidates[number]) => edits[item.brand.id] || { name: item.name, aliases: item.aliases };
  async function lookupOnline(item: typeof candidates[number]) { setOnlineBusy(item.brand.id); setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Searching Wikidata…" })); try { const response = await fetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(item.brand.name)}&language=en&format=json&origin=*`); const result = await response.json() as { search?: { label?: string; description?: string; aliases?: { value: string }[] }[] }; const match = result.search?.[0]; if (match) { setOnline((current) => ({ ...current, [item.brand.id]: true })); setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Online match found" })); setEdits((current) => ({ ...current, [item.brand.id]: { name: match.label || item.name, aliases: [...new Set([...(match.aliases || []).map((alias) => alias.value), ...item.aliases.split(",").map((alias) => alias.trim()).filter(Boolean)])].join(", ") } })); } else { setOnlineStatus((current) => ({ ...current, [item.brand.id]: "No Wikidata match — edit manually" })); } } catch { setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Online lookup unavailable — edit manually" })); } finally { setOnlineBusy(null); } }
...
function SettingsView({ editingAllowed, data, currentUser, ubqSource, onLoadUbq, onReturnReconciliation, onClear, onUpdateSettings, onSetReference, onAddDecisions, onAddHistoricalMappings, onBackup, onRestore, createSnapshot, applySnapshot, githubSession, onGitHubSession, onGitHubSync, online, serviceSession, onServiceSession, githubRemoteUpdate, onGitHubRemoteUpdate, githubTeamSync, onGitHubTeamSync }: SettingsViewProps) {
  ...
  return <><PageHead eyebrow="DATA SOURCES, VERIFICATION & SYNC" title="Data sources & setup" body="Load fresh UBQ and Root exports, verify what the external Admin tool actually changed, and return unresolved work to team triage." />
    <div className="module-layout"><div className="settings-content">{!LOCAL_MODE && <ApprovedSourcesPanel />}{LOCAL_MODE ? <section className="panel local-connection-panel"><div className="section-title"><div><h2>Local folder connection</h2><p>Select the Root and UBQ files from your Mac. This local workspace does not connect to GitHub or the team workspace.</p></div><span className="offline-chip"><CloudOff size={13} />Working offline</span></div><div className="reference-list"><ReferenceUploader source="ROOT" count={data.rootBrands.length} meta={data.sourceMeta.ROOT} onLoad={(brands, filename) => onSetReference("ROOT", brands, filename)} /><UbqUploader source={ubqSource} meta={data.sourceMeta.UBQ} data={data} onLoad={onLoadUbq} /></div></section> : USE_SYNC_SERVICE ? <ServiceWorkspacePanel createSnapshot={createSnapshot} applySnapshot={applySnapshot} session={serviceSession} onSession={onServiceSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onTeamSync={onGitHubTeamSync} /> : <GitHubWorkspacePanel session={githubSession} onSession={onGitHubSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onSync={onGitHubSync} online={online} />}
      ...
      {!LOCAL_MODE && <ApprovedSourcesPanel />}
