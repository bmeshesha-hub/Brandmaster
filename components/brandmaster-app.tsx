...
function PriorityQueue({ items, currentUser, teamMembers, onChooseTeamMember, syncConnected, onUpdate, onReset, onRemove, onStart, onNavigate }: { items: PriorityQueueItem[]; currentUser: string; teamMembers: string[]; onChooseTeamMember: (member: string) => void; syncConnected: boolean; onUpdate: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onReset: (ids: string[]) => void; onRemove: (ids: string[]) => void; onStart: (ids: string[]) => void; onNavigate: (view: View) => void }) {
  ...
  useEffect(() => { if (currentUser) setAssignmentTarget(currentUser); }, [currentUser]);
  useEffect(() => setRemoveArmed(false), [selected]);
  const progress = items.length ? Math.round(completed / items.length * 100) : 0;
  function claimNext() { if (!currentUser) return; const ids = available.slice(0, 10).map((item) => item.id); onUpdate(ids, "ASSIGNED", currentUser); setSelected(ids); setTab("mine"); }
