...
function OnlineBrandCleanupV2({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  useEffect(() => { if (!selected.length && candidates.length) setSelected(candidates.filter((item) => item.category !== "CLEAN").map((item) => item.brand.id)); }, [candidates, selected.length]);
