...
import { classifyBrand, normalizeBrand, parseCsv, parseDecisionCsv, parseReferenceCsv, SEED_BRANDS, toCsv } from "@/lib/brand-engine";
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="page">
        ...
        {view === "review" && (processing ? <ProcessingView run={processing} /> : <ReviewQueue records={current?.records || []} onUpdate={updateRecord} onSelect={setSelected} query={query} onNavigate={navigate} />)}
...
function ManualGptAssist({ records, onUpdate }: { records: BrandRecord[]; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void }) {
  const [open, setOpen] = useState(false); const [create, setCreate] = useState(""); const [skip, setSkip] = useState(""); const [remove, setRemove] = useState(""); const [merge, setMerge] = useState(""); const [copied, setCopied] = useState(false);
  const uncertain = records.filter((record) => record.status === "needs-review");
  const findRecord = (name: string) => { const key = normalizeBrand(name).toLowerCase(); return uncertain.find((record) => record.normalized.toLowerCase() === key || record.name.trim().toLowerCase() === name.trim().toLowerCase()); };
  const lines = (value: string) => value.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const decisions = new Map<string, { record: BrandRecord; action: Action; targetId?: string; targetName?: string }>();
  ([{ value: create, action: "CREATE" }, { value: skip, action: "SKIP" }, { value: remove, action: "DELETE" }] as { value: string; action: Action }[]).forEach(({ value, action }) => lines(value).forEach((name) => { const record = findRecord(name); if (record) decisions.set(record.id, { record, action, targetName: action === "CREATE" ? record.normalized : undefined }); }));
  lines(merge).forEach((line) => { const [name, targetId, targetName] = line.split("|").map((part) => part.trim()); const record = findRecord(name); if (record && targetId?.startsWith("brand_") && targetName) decisions.set(record.id, { record, action: "MERGE", targetId, targetName }); });
  const preview = [...decisions.values()];
  const prompt = `Validate these automotive fitment brand values. Return decisions in exactly four sections. Use one brand per line. For MERGE use: original brand | brand_xxx target ID | canonical target name. Do not invent target IDs.\n\nCREATE:\n\nSKIP:\n\nDELETE:\n\nMERGE:\n\nBRANDS TO VALIDATE:\n${uncertain.map((record) => record.name).join("\n")}`;
  async function copyPrompt() { await navigator.clipboard.writeText(prompt); setCopied(true); setTimeout(() => setCopied(false), 1800); }
  function apply() { preview.forEach(({ record, action, targetId, targetName }) => onUpdate(record.id, { action, targetId, targetName, confidence: 100, reason: "Decision pasted from external GPT validation", decisionSource: "Pasted GPT decision" }, true)); setCreate(""); setSkip(""); setRemove(""); setMerge(""); setOpen(false); }
  return <section className={`gpt-assist ${open ? "open" : ""}`}><button className="gpt-assist-head" onClick={() => setOpen(!open)}><div className="gpt-icon"><Sparkles size={18} /></div><div><span>OPTIONAL MANUAL AI ASSIST</span><b>Run {uncertain.length} uncertain brand{uncertain.length === 1 ? "" : "s"} through your own validation GPT</b><p>Copy a prepared prompt, then paste its decisions into action buckets. No API key required.</p></div><ChevronDown size={18} /></button>{open && <div className="gpt-assist-body"><div className="gpt-instructions"><div><h3>1. Copy brands to GPT</h3><p>The prompt requests the exact format needed below and warns against invented merge IDs.</p></div><button className="secondary" onClick={copyPrompt}>{copied ? <Check size={15} /> : <BookOpen size={15} />}{copied ? "Copied" : "Copy validation prompt"}</button></div><div className="decision-buckets"><label className="create"><span>CREATE</span><small>One brand per line</small><textarea value={create} onChange={(e) => setCreate(e.target.value)} placeholder={"pegaso\nlechler"} /></label><label className="skip"><span>SKIP</span><small>One brand per line</small><textarea value={skip} onChange={(e) => setSkip(e.target.value)} placeholder={"possible retailer"} /></label><label className="delete"><span>DELETE</span><small>One brand per line</small><textarea value={remove} onChange={(e) => setRemove(e.target.value)} placeholder={"not a brand"} /></label><label className="merge"><span>MERGE</span><small>brand | target ID | target name</small><textarea value={merge} onChange={(e) => setMerge(e.target.value)} placeholder={"volkswagen oe | brand_xxx | Volkswagen"} /></label></div>{preview.length > 0 && <div className="gpt-preview"><div><h3>2. Preview pasted decisions</h3><span>{preview.length} matched</span></div><div className="gpt-preview-table"><div><b>Input brand</b><b>Action</b><b>Output target</b></div>{preview.map(({ record, action, targetId, targetName }) => <div key={record.id}><span>{record.name}</span><ActionPill action={action} /><span>{action === "MERGE" ? `${targetName} · ${targetId}` : targetName || "—"}</span></div>)}</div><button className="primary" onClick={apply}><Check size={15} />Apply {preview.length} decisions to review</button></div>}</div>}</section>;
}

function ReviewQueue({ records, onUpdate, onSelect, query, onNavigate }: { records: BrandRecord[]; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onSelect: (r: BrandRecord) => void; query: string; onNavigate: (view: View) => void }) {
  ...
  return <><WorkflowStepper stage={2} onNavigate={onNavigate} hasImport outputReady={exportReady} /><PageHead eyebrow="STEP 2 OF 3" title="Process and review" body={`${needs} brand${needs === 1 ? "" : "s"} still require a decision. High-confidence rows are already prepared.`} actions={<button className="primary" disabled={!exportReady} title={!exportReady ? "Resolve the remaining checks first" : "Continue to the output file"} onClick={() => onNavigate("output")}>Continue to output →</button>} />
    ...
    {needs > 0 && <ManualGptAssist records={records} onUpdate={onUpdate} />}
