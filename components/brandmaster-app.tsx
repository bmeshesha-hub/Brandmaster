...
function Analytics({ records, ledger, historicalMappings, priorityQueue, currentUser }: { records: BrandRecord[]; ledger: LedgerEntry[]; historicalMappings: HistoricalMappingEntry[]; priorityQueue: PriorityQueueItem[]; currentUser: string }) {
  ...
  const allActivity = useMemo<AnalyticsActivity[]>(() => [
    ...
    ...ledger.map((entry) => ({ ...entry, activitySource: "LIVE" as const })),
  ...
  const summary = useMemo(() => summarizeMappingActivity(filteredActivity, records), [filteredActivity, records]);
  ...
  function downloadExcelReport() {
    download(`brandmaster-analytics-${safeUser}-${new Date().toISOString().slice(0, 10)}.xls`, analyticsExcelXml(filteredActivity, records, priorityQueue), "application/vnd.ms-excel;charset=utf-8");
  }
  ...
  return <><PageHead eyebrow="MAPPING PERFORMANCE" title="Progress & effort analytics" body="Historical imports, live reviews, and confirmed Admin upload results show both team effort and delivered mapping progress." actions={<><button className="secondary" onClick={downloadExcelReport}><ArrowDownToLine size={15} />Download Excel</button><button className="primary" onClick={() => void downloadPdfReport()}><ArrowDownToLine size={15} />Download PDF</button></>} />
    ...
    <section className={`admin-outcome-analytics ${adminResultsRecorded ? "" : "unverified"}`}><div><span><ShieldCheck size={21} /></span><div><small>ADMIN DELIVERY VERIFICATION</small><h2>{adminResultsRecorded ? `${adminSuccessful.toLocaleString()} mappings confirmed by Admin results` : "Admin delivery has not been verified yet"}</h2><p>{adminResultsRecorded ? "Only imported Admin result files and explicit all-success confirmations count here." : `${adminPending.toLocaleString()} reviewed row${adminPending === 1 ? " is" : "s are"} awaiting an Admin result. Historical spreadsheets measure team decisions, but do not prove that Admin applied them.`}</p></div></div><aside><b className="success">{adminSuccessful}<small>confirmed</small></b><b className={adminFailed ? "failed" : ""}>{adminFailed}<small>failed</small></b><b>{adminPending}<small>not verified</small></b></aside></section>
