  ...
  return <div className={`app-shell ebay-theme unified-mode ${cleanTriage ? "clean-workflow" : "advanced-workflow"} view-${view}`}>
    ...
    <main aria-busy={Boolean(navigationTarget || navigationPending)}>
      ...
      <div className="page">
        ...
        {!LOCAL_MODE && view === "brand-cleanup" && <OnlineBrandCleanup data={data} currentUser={currentUser} onSave={saveCatalogBrand} />}
...
function OnlineBrandCleanup({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  const visible = candidates.filter((item) => `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const reviewed = Object.keys(decisions).length; const approved = Object.values(decisions).filter((value) => value === "APPROVE").length; const changedRows = Object.values(approvedSnapshots);
  function decide(item: typeof candidates[number], decision: "APPROVE" | "REJECT") { setDecisions((current) => ({ ...current, [item.brand.id]: decision })); if (decision === "APPROVE" && item.changed) { const after = { ...item.brand, name: item.suggested, aliases: item.aliases }; setApprovedSnapshots((current) => ({ ...current, [item.brand.id]: { before: item.brand, after, confidence: item.confidence } })); onSave(after); } else if (decision === "REJECT") setApprovedSnapshots((current) => { const next = { ...current }; delete next[item.brand.id]; return next; }); }
