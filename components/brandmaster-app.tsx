...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  async function prepareProtectedExport() {
    ...
    if (teamSyncPauseRef.current) { setToast(`Resume Team Sync before downloading. ${teamSyncPauseRef.current.pausedBy} paused the shared workspace.`); return false; }
    setSyncBusy(true);
    try {
  ...
  function updatePriorityItems(ids: string[], status: PriorityQueueStatus, assignee?: string) {
    ...
    if (status === "ASSIGNED") {
      ...
      if (ownedByOthers.length) { setToast(`${ownedByOthers[0].name} is already being worked by ${ownedByOthers[0].assignedTo}. It was not reassigned or overwritten.`); return; }
    }
...
function DataQualityAnalytics({ data, ubqSource, onAddPriority, onNavigate }: { data: AppData; ubqSource: UbqSource | null; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onNavigate: (view: View) => void }) {
  ...
  const filteredIssues = useMemo(() => allIssues.filter((issue) => (source === "ALL" || issue.source === source) && (severity === "ALL" || issue.severity === severity) && (issueType === "ALL" || issue.type === issueType)), [allIssues, source, severity, issueType]);
  ...
  return <>
    ...
    {!data.rootBrands.length && !ubqRows.length ? <div className="panel"><EmptyState icon={Database} title="Load Root and UBQ data to begin" body="Open Data sources & setup and load the latest Root table and full UBQ export. This page analyzes them locally and works offline." action={<button className="primary" onClick={() => onNavigate("settings")}>Open Data sources & setup</button>} /></div> : <>
      ...
      <section className="quality-lifecycle"><div><small>SHARED TRIAGE LIFECYCLE</small><h2>What the team is working on</h2><p>One brand keeps one shared task. Existing, assigned, completed, and verified work is protected from accidental duplicate triage.</p></div><button onClick={() => onNavigate("imports")}><b>{queueCounts.available}</b><small>Available</small></button><button onClick={() => onNavigate("imports")}><b>{queueCounts.assigned}</b><small>Assigned</small></button><button onClick={() => onNavigate("imports")}><b>{queueCounts.review}</b><small>In review</small></button><button onClick={() => onNavigate("output")}><b>{queueCounts.ready}</b><small>Ready</small></button><button onClick={() => onNavigate("settings")}><b>{queueCounts.awaiting}</b><small>Awaiting proof</small></button><button className="verified" onClick={() => onNavigate("imports")}><b>{queueCounts.verified}</b><small>Verified</small></button>{queueCounts.blocked > 0 && <button className="blocked" onClick={() => onNavigate("imports")}><b>{queueCounts.blocked}</b><small>Blocked</small></button>}</section>
      ...
      <section className="quality-top-grid">
        <article className="panel"><div className="panel-head"><div><h2>Top UBQ brands</h2><p>Highest listing volume—research, open the exact Admin queue search, or send to triage</p></div></div><div className="quality-ranking">{topUbq.map((row, index) => <div key={row.id}><em>{index + 1}</em><span><b>{row.name}</b><small>{row.id}</small></span><strong>{(row.listingCount || 0).toLocaleString()}<small>listings</small></strong><div className="quality-row-actions"><ResearchLinks name={row.name} compact /><AdminUnknownBrandLink name={row.name} compact />{taskAction("UBQ", row.id, row.name, () => onAddPriority("UBQ", [{ id: row.id, name: row.name, listingCount: row.listingCount, skuCount: row.skuCount }]))}</div></div>)}</div></article>
        <article className="panel"><div className="panel-head"><div><h2>Root brands with most aliases</h2><p>Research the name or open the exact BrandID in Admin before changing aliases</p></div></div><div className="quality-ranking">{topRoot.map((brand, index) => <div key={brand.id}><em>{index + 1}</em><span><b>{brand.name}</b><small>{brand.id}</small></span><strong>{brand.aliases.length}<small>aliases</small></strong><div className="quality-row-actions"><ResearchLinks name={brand.name} compact /><AdminBrandLink id={brand.id} name={brand.name} compact />{taskAction("ROOT", brand.id, brand.name, () => onAddPriority("ROOT", [{ id: brand.id, name: brand.name }]))}</div></div>)}</div></article>
      </section>
      ...
      <section id="quality-attention" className="panel quality-attention"><div className="panel-head"><div><h2>Brands needing attention</h2><p>Filter findings, research the brand, open its exact Admin destination, or send it to the shared High Priority queue.</p></div><div><select value={source} onChange={(event) => setSource(event.target.value as typeof source)}><option value="ALL">Root + UBQ</option><option value="ROOT">Root only</option><option value="UBQ">UBQ only</option></select><select value={severity} onChange={(event) => setSeverity(event.target.value as typeof severity)}><option value="ALL">All severity</option><option value="HIGH">High risk</option><option value="MEDIUM">Needs review</option><option value="LOW">Cleanup</option></select><select value={issueType} onChange={(event) => setIssueType(event.target.value)}><option value="ALL">All issue types</option>{categoryData.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div></div><div className="quality-findings">{filteredIssues.slice(0, 50).map((issue) => <div key={issue.key}><span className={issue.severity.toLowerCase()}>{issue.severity}</span><div><b>{issue.name}</b><small>{issue.brandId}</small></div><strong>{issue.title}<small>{issue.reason}</small></strong><em>{issue.suggestion || "Manual review"}</em><div className="quality-row-actions"><ResearchLinks name={issue.name} compact />{issue.source === "ROOT" ? <AdminBrandLink id={issue.brandId} name={issue.name} compact /> : <AdminUnknownBrandLink name={issue.name} compact />}{taskAction(issue.source, issue.brandId, issue.name, () => queueIssue(issue))}</div></div>)}</div>{filteredIssues.length > 50 && <p className="quality-more">Showing the first 50 of {filteredIssues.length.toLocaleString()} findings. Use Smart Cleanup for the complete paginated worklist.</p>}</section>
    </>}
  </>;
}
...
function Analytics({ records, ledger, historicalMappings, priorityQueue, currentUser }: { records: BrandRecord[]; ledger: LedgerEntry[]; historicalMappings: HistoricalMappingEntry[]; priorityQueue: PriorityQueueItem[]; currentUser: string }) {
  ...
  const adminConfirmers = [...new Set(records.filter((record) => record.adminUploadStatus === "SUCCESS").map((record) => record.adminUploadedBy).filter((name): name is string => Boolean(name)))];
