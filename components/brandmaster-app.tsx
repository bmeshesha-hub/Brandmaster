...
function UbqUploader({ source, onLoad }: { source: UbqSource | null; onLoad: (filename: string, rows: ParsedRow[]) => void }) {
...
function SettingsView({ data, ubqSource, onLoadUbq, onClear, onUpdateSettings, onSetReference, onAddDecisions }: { data: AppData; ubqSource: UbqSource | null; onLoadUbq: (filename: string, rows: ParsedRow[]) => void; onClear: () => void; onUpdateSettings: (settings: Partial<ValidationSettings>) => void; onSetReference: (source: "ACA" | "FPA" | "ROOT", brands: CatalogBrand[]) => void; onAddDecisions: (decisions: AppData["learned"]) => void }) {
  ...
  return <><PageHead eyebrow="OFFLINE DATA & VALIDATION" title="Reference tables" body="Load the catalog sources that make matching accurate. Files stay on this Mac and remain available offline." />
    <div className="module-layout"><div className="settings-content">
      <section className="reference-section"><div className="section-title"><div><h2>Brand data sources</h2><p>The UBQ export supplies real unmapped IDs. Previous decisions resolve known work first; the existing brand table, ACA, and FPA support validation.</p></div><span className="offline-chip"><CloudOff size={13} />Stored offline</span></div><div className="reference-list"><UbqUploader source={ubqSource} onLoad={onLoadUbq} /><DecisionUploader count={Object.keys(data.learned).length} onLoad={onAddDecisions} /><ReferenceUploader source="ROOT" count={data.rootBrands.length} onLoad={(brands) => onSetReference("ROOT", brands)} /><ReferenceUploader source="ACA" count={data.acaBrands.length} onLoad={(brands) => onSetReference("ACA", brands)} /><ReferenceUploader source="FPA" count={data.fpaBrands.length} onLoad={(brands) => onSetReference("FPA", brands)} /></div>{(ubqSource || data.rootBrands.length > 0) && <div className="tables-ready"><Check size={16} /><div><b>{ubqSource ? "UBQ ID resolution is ready" : "Authoritative existing-brand validation is ready"}</b><p>{ubqSource ? `${ubqSource.count.toLocaleString()} UBQ rows, ` : "No UBQ export, "}{data.rootBrands.length.toLocaleString()} active existing brands, {Object.keys(data.learned).length.toLocaleString()} previous decisions, {data.acaBrands.length.toLocaleString()} ACA brands, and {data.fpaBrands.length.toLocaleString()} FPA brands are available.</p></div></div>}</section>
