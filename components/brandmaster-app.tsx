  ...
  function importRows(filename: string, rows: ReturnType<typeof parseCsv>, priorityItems: PriorityQueueItem[] = [], preflightConfirmed = false, reviewAgainKeys = new Set<string>()) {
    ...
    const advance = (index: number) => {
      ...
      const records = await mapInUiChunks(rows, (row) => {
        ...
        if (!currentUbq) return { ...queuedCorrection, ubqVerified: row.id.startsWith("draft_brand_") };
...
function DataQualityAnalytics({ data, ubqSource, onAddPriority, onNavigate }: { data: AppData; ubqSource: UbqSource | null; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onNavigate: (view: View) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const nextRoot = analyzeRootBrands(data.rootBrands);
      if (cancelled) return;
      setRootIssues(nextRoot);
      window.setTimeout(() => {
        const nextUbq = analyzeUbqBrands(ubqRows, data.rootBrands);
        if (cancelled) return;
        setUbqIssues(nextUbq); setQualityLoading(false);
      }, 0);
    }, 0);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [data.rootBrands, ubqRows]);
