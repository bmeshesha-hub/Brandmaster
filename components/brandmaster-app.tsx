...
function OnlineBrandCleanupV2({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  const prioritized = [...candidates.filter((item) => item.category === "TRIAGE"), ...candidates.filter((item) => item.category === "NORMALIZE"), ...candidates.filter((item) => item.category === "ENHANCE")]; const allMatching = prioritized.filter((item) => `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const filtered = allMatching.filter((item) => item.category !== "CLEAN"); const reviewRows = filtered.filter((item) => selected.includes(item.brand.id) && (filter === "ALL" || filter === "NEEDS_FIX" && item.category !== "CLEAN" || filter === "PENDING" && !decisions[item.brand.id] || filter === "APPROVED" && decisions[item.brand.id] === "APPROVE" || filter === "REJECTED" && decisions[item.brand.id] === "REJECT")); const pages = Math.max(1, Math.ceil(reviewRows.length / pageSize)); const pageRows = reviewRows.slice((page - 1) * pageSize, page * pageSize); const reviewed = Object.keys(decisions).length; const changed = Object.values(snapshots);
