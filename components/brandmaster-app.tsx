...
import {
  ...
  CircleHelp, ClipboardPaste, Cloud, CloudOff, Combine, Database, FileClock, FileUp, Gauge, Github, History, KeyRound, LayoutDashboard, LogOut,
  ...
  Sun, Trash2, TrendingUp, UploadCloud, Users, WandSparkles, X, SearchCheck,
} from "lucide-react";
...
type ProcessingRun = { filename: string; count: number; steps: string[]; current: number; source?: WorkflowSource; processed?: number };
...
async function mapInUiChunks<T, R>(values: T[], mapper: (value: T, index: number) => R, onProgress?: (processed: number) => void, chunkSize = 25) {
  const output: R[] = [];
  for (let start = 0; start < values.length; start += chunkSize) {
    const end = Math.min(values.length, start + chunkSize);
    for (let index = start; index < end; index += 1) output.push(mapper(values[index], index));
    onProgress?.(end);
    if (end < values.length) await new Promise<void>((resolve) => window.setTimeout(resolve, 0));
  }
  return output;
}

function ApprovedSourcesPanel({ sources, onChange }: { sources: { name: string; url: string; enabled: boolean }[]; onChange: (sources: { name: string; url: string; enabled: boolean }[]) => void }) { const save = (next: typeof sources) => onChange(next); return <section className="panel approved-sources-panel"><div className="section-title"><div><h2>Approved research sources</h2><p>Saved in the shared workspace and used for Brand Cleanup research. Google templates are stored for secure API/proxy use.</p></div><span className="offline-chip"><Globe size={13} />Shared settings</span></div><div className="approved-source-list">{sources.map((source, index) => <div className="approved-source-row" key={`${source.url}-${index}`}><input value={source.name} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value } : item))} /><input value={source.url} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item))} /><label><input type="checkbox" checked={source.enabled} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item))} />Enabled</label><button className="danger-outline" onClick={() => save(sources.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={14} /></button></div>)}</div><button className="secondary" onClick={() => save([...sources, { name: "Approved source", url: "https://", enabled: true }])}><Plus size={14} />Add source</button></section>; }

async function enrichUbqFamiliesInUiChunks(records: BrandRecord[], ubqRows: ParsedRow[], data: AppData, chunkSize = 10) {
  const output: BrandRecord[] = [];
  for (let start = 0; start < records.length; start += chunkSize) {
    output.push(...enrichUbqFamilies(records.slice(start, start + chunkSize), ubqRows, data));
    if (start + chunkSize < records.length) await new Promise<void>((resolve) => window.setTimeout(resolve, 0));
  }
  return output;
}
...
function normalizeSharedTaskOwners(data: AppData): AppData {
  ...
  const batches = (data.batches || []).map((batch) => ({ ...batch, records: batch.records.map(normalizeWorkflowRecord) }));
  if (batches.some((batch, index) => batch.records.some((record, recordIndex) => record !== data.batches[index]?.records[recordIndex]))) changed = true;
  const normalizedData = { ...data, batches, exportRuns: data.exportRuns || [], learningOverrides: data.learningOverrides || {}, teamPresence: data.teamPresence || {}, teamActivity: data.teamActivity || [] };
...
export default function BrandmasterApp({ authenticatedIdentity = null, onAuthenticatedSignOut }: { authenticatedIdentity?: AuthenticatedBrandmasterUser | null; onAuthenticatedSignOut?: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const savedData = LOCAL_MODE ? { ...EMPTY_DATA } : normalizeSharedTaskOwners(loadData());
    ...
    setData(savedData);
    const workspaceLoad = LOCAL_MODE ? Promise.resolve(null) : loadWorkspaceData().then((saved) => {
      if (!saved) return;
    ...
    const referenceLoad = LOCAL_MODE ? Promise.resolve(null) : loadReferenceTables().then((tables) => setData((prev) => {
    ...
    const ubqLoad = LOCAL_MODE ? Promise.resolve(null) : loadUbqReference().then((saved) => {
  ...
  const pendingTeamRecords = useMemo(() => data.batches.filter((batch) => !batch.archivedAt).flatMap((batch) => batch.records), [data.batches]);
  ...
  function loadUbqSource(filename: string, rows: ParsedRow[]) {
    ...
    setData((prev) => {
      ...
      const withVerifiedQueue = { ...prev, learned, priorityQueue, adminUpdateRuns };
      const reactivated = reactivateVerifiedQueuedLearning(withVerifiedQueue, `UBQ import · ${filename}`, verifiedAt);
      const verifiedQueueIds = new Set(priorityQueue.filter((item) => item.externalStatus === "VERIFIED").map((item) => item.id));
      const batches = prev.batches.map((batch) => ({ ...batch, records: batch.records.map((record) => {
        ...
        return refreshed.priorityQueueId && verifiedQueueIds.has(refreshed.priorityQueueId) ? { ...refreshed, workflowStage: "SOURCE_VERIFIED" as const } : refreshed;
      }) }));
      const stageById = new Map(batches.flatMap((batch) => batch.records.map((record) => [record.id, workflowStage(record)])));
      const exportRuns = prev.exportRuns.map((run) => run.rowIds.length > 0 && run.rowIds.every((id) => stageById.get(id) === "SOURCE_VERIFIED") ? { ...run, status: "SOURCE_VERIFIED" as const } : run);
      return { ...withVerifiedQueue, learningOverrides: reactivated.overrides, batches, exportRuns, sourceMeta: { ...prev.sourceMeta, UBQ: { filename, updatedAt: verifiedAt, rowCount: rows.length, fingerprint: sourceFingerprint(rows) } } };
    });
  ...
  function importRows(filename: string, rows: ReturnType<typeof parseCsv>, priorityItems: PriorityQueueItem[] = [], preflightConfirmed = false, reviewAgainKeys = new Set<string>()) {
    ...
    const advance = (index: number) => {
      ...
      void (async () => {
      const records = await mapInUiChunks(rows, (row) => {
        ...
        const priorityItem = priorityItems.find((item) => item.brandId === row.id || item.name.toLowerCase() === row.name.toLowerCase());
        ...
        const queuedCorrection = priorityItem?.secondReviewRequired ? {
          ...
          targetId: priorityItem.requestedAction === "MERGE" ? priorityItem.requestedTargetId : undefined,
          targetName: priorityItem.requestedAction === "MERGE" || priorityItem.requestedAction === "CREATE" ? priorityItem.requestedTargetName : undefined,
          ...
          secondReviewReason: priorityItem.reviewRequestNote || "A teammate requested an independent second review.",
          reason: priorityItem.reviewRequestNote || record.reason,
          priorityQueueId,
        } : priorityItem?.learningRuleId && priorityItem.requestedAction ? {
          ...record, action: priorityItem.requestedAction, targetId: priorityItem.requestedAction === "MERGE" ? priorityItem.requestedTargetId : undefined,
          targetName: priorityItem.requestedAction === "MERGE" || priorityItem.requestedAction === "CREATE" ? priorityItem.requestedTargetName : undefined,
          confidence: Math.min(record.confidence, 70), status: "needs-review" as const, decisionSource: "VLR correction request",
          reason: priorityItem.reviewRequestNote || "A questionable VLR rule was sent to the team for correction.",
          evidence: [...(priorityItem.reviewRequestEvidence || []), `VLR rule: ${priorityItem.learningRuleId}`], priorityQueueId,
        ...
        if (row.id.startsWith("draft_brand_")) return { ...queuedCorrection, ubqVerified: true };
        if (!currentUbq) return { ...queuedCorrection, ubqVerified: false };
        if (source) return { ...queuedCorrection, ubqVerified: true };
        return { ...queuedCorrection, ubqVerified: false, status: "needs-review" as const, confidence: Math.min(queuedCorrection.confidence, 40), reason: "This brand was not found in the loaded UBQ export", evidence: ["UBQ lookup failed", ...queuedCorrection.evidence] };
      }, (processed) => setProcessing({ filename, count: rows.length, steps, current: steps.length - 1, source: "IMPORT", processed }));
      const enriched = currentUbq ? await enrichUbqFamiliesInUiChunks(records, [...currentUbq.byId.values()], base) : records;
  ...
  function startSourceWorklist(source: Exclude<WorkflowSource, "IMPORT">, ids: string[], priorityItems: PriorityQueueItem[] = []) {
    ...
    const completedNames = new Set(alreadyCompleted.map((detail) => normalizeBrand(detail.brand).toLowerCase()));
    const actionableRows = rows.filter((row) => !completedNames.has(normalizeBrand(row.name).toLowerCase()));
    if (!actionableRows.length) { showCompletedBrandNotice(alreadyCompleted); return; }
    const filename = `${source === "ROOT" ? "Root table cleanup" : "UBQ worklist"} · ${actionableRows.length} brands`;
    setView("review"); setProcessing({ filename, count: actionableRows.length, steps, current: 0, source });
    const advance = (index: number) => {
      if (index < steps.length) { setProcessing({ filename, count: actionableRows.length, steps, current: index, source }); setTimeout(() => advance(index + 1), 300); return; }
      void (async () => {
      let records: BrandRecord[] = await mapInUiChunks(actionableRows, (row) => {
      ...
      }, (processed) => setProcessing({ filename, count: actionableRows.length, steps, current: steps.length - 1, source, processed }));
      if (source === "ROOT") records = stabilizeRootConsolidations(records, data.rootBrands);
      if (source === "UBQ" && currentUbq) records = await enrichUbqFamiliesInUiChunks(records, [...currentUbq.byId.values()], data);
  ...
  function updateRecord(recordId: string, changes: Partial<BrandRecord>, learn = false) {
    ...
    setData((prev) => {
      ...
      const batches = prev.batches.map((batch) => batch.id !== activeBatchId ? batch : ({ ...batch, records: batch.records.map((record) => {
        ...
        const updated = { ...record, ...changes, decisionSource: changes.decisionSource || "Manual override", status };
        changed = status === "reviewed" ? saveWorkflowReview(updated, currentUser, reviewAt).record : { ...updated, workflowStage: workflowStage(updated) };
        return changed;
      }) }));
  ...
  function approveRecordsAndContinue(recordIds: string[]): ApprovalContinueResult {
    ...
    const approval = saveWorkflowReviews(prepared, selected, currentUser, reviewAt, true);
    if (approval.error) {
  ...
  function saveCatalogBrand(brand: CatalogBrand, origin: "MAPPING" | "BRAND_CLEANUP" = "MAPPING") {
    setData((prev) => {
      ...
      if (brand.source === "Root") {
        ...
        if (changedFields.length) rootChanges[brand.id] = { id: brand.id, type: before ? "UPDATE" : "CREATE", before, after: brand, changedFields, updatedAt: new Date().toISOString(), status: "PENDING", adminStatus: existingChange?.adminStatus || "RECOMMENDED", adminUpdatedAt: new Date().toISOString(), adminUpdatedBy: currentUser, origin };
        else delete rootChanges[brand.id];
        const rootBrands = replace(prev.rootBrands); void saveReferenceTable("ROOT", rootBrands);
        return origin === "BRAND_CLEANUP" ? withTeamActivity({ ...prev, rootBrands, rootChanges, sourceMeta: { ...prev.sourceMeta, ROOT: { filename: prev.sourceMeta.ROOT?.filename || "Root table", updatedAt: new Date().toISOString() } } }, "REVIEWED", `Brand Cleanup approved Root change: ${brand.name}`, 1) : { ...prev, rootBrands, rootChanges, sourceMeta: { ...prev.sourceMeta, ROOT: { filename: prev.sourceMeta.ROOT?.filename || "Root table", updatedAt: new Date().toISOString() } } };
      }
  ...
  function sendLearningRuleToQueue(rule: LearningRule, changes: LearningRuleChanges, note: string) {
    const actor = queueUser || currentUser || "Local reviewer";
    const at = new Date().toISOString();
    const action = changes.action || rule.action;
    const targetId = action === "MERGE" ? changes.targetId ?? rule.targetId : undefined;
    const targetName = action === "MERGE" || action === "CREATE" ? changes.targetName ?? rule.targetName ?? rule.normalizedName : undefined;
    const message = note.trim() || `Review and correct the VLR rule for ${rule.names[0] || rule.normalizedName} in Admin.`;
    setData((prev) => {
      const learningOverrides = {
        ...(prev.learningOverrides || {}),
        [rule.id]: updateLearningOverride(prev.learningOverrides?.[rule.id], rule.id, { ...changes, status: "DISABLED", action, targetId, targetName, note: message }, "SENT_TO_QUEUE", actor, message, at),
      };
      const priorityQueue = enqueueLearningRuleReview(prev.priorityQueue, {
        ruleId: rule.id, brandId: rule.sourceBrandId, name: rule.names[0] || rule.normalizedName, action, targetId, targetName,
        note: message, evidence: rule.evidence.map((item) => item.value), createdBy: actor, at,
      });
      return withTeamActivity({ ...prev, learningOverrides, priorityQueue }, "QUEUE_ADDED", `${actor} sent ${rule.names[0] || rule.normalizedName} from VLR to the team queue`, 1);
    });
    markPriorityPending();
  ...
  function addPriorityRows(source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>, forceReopen = false) {
    ...
    const completedKeys = new Set(completedRows.map((detail) => `${normalizeBrand(detail.brand).toLowerCase()}`));
    const queueRows = forceReopen ? rows : rows.filter((row) => !completedKeys.has(normalizeBrand(row.name).toLowerCase()));
    if (!queueRows.length) { showCompletedBrandNotice(completedRows); return; }
    const now = new Date().toISOString();
  ...
  async function startPriorityWorklist(ids: string[]) {
    ...
    const selfReview = latest.priorityQueue.find((item) => ids.includes(item.id) && item.secondReviewRequired && item.firstReviewedBy?.toLowerCase() === queueUser.toLowerCase());
    if (selfReview) { setToast(`${queueUser} completed the first review of ${selfReview.name}. A different teammate must claim this second review.`); return; }
  ...
  function applyAdminUploadResults(batchId: string | undefined, attemptedIds: string[], rows: AdminUploadResultRow[], exportFilename: string, resultFilename: string, moveFailuresToReview: boolean, markNotFoundDone = false, exportRunId?: string) {
    if (!batchId) return;
    const now = new Date().toISOString();
    ...
    setData((prev) => {
      ...
      const exportRuns = prev.exportRuns.map((exportRun) => exportRun.id === exportRunId ? confirmExportRun(exportRun, successfulIds, failedIds, resultFilename, owner, now) : exportRun);
      const updated = { ...prev, batches, priorityQueue, adminUpdateRuns, exportRuns, userWorkspaces };
  ...
  function recordBulkExport(run: ExportRun) {
    setData((prev) => ({ ...prev, exportRuns: [run, ...prev.exportRuns.filter((item) => item.id !== run.id)].slice(0, 500), batches: prev.batches.map((batch) => batch.id === run.batchId ? { ...batch, records: markRecordsDownloaded(batch.records, run.rowIds) } : batch) }));
    markPriorityPending();
  }
  ...
  function setRecordsExportExcluded(recordIds: string[], excluded: boolean) {
    ...
    setData((prev) => {
      ...
      const batches = prev.batches.map((item) => item.id !== current.id ? item : ({ ...item, records: updateExportInclusion(item.records, ids, excluded) }));
      const priorityQueue = excluded ? prev.priorityQueue : prev.priorityQueue.map((item) => queueIds.has(item.id) ? {
        ...item,
        ...
        triageResolutionNote: undefined,
        updatedAt: now,
        activity: [queueActivity("REOPENED", "Restored to the Step 3 Admin download", now), ...(item.activity || [])].slice(0, 30),
      } : item);
      return { ...prev, batches, priorityQueue };
    });
    markPriorityPending();
    setToast(excluded ? `${ids.size} brand${ids.size === 1 ? "" : "s"} excluded from this bulk download` : `${ids.size} brand${ids.size === 1 ? "" : "s"} restored to the bulk download`);
  }
  ...
  function reopenRecordsForReview(recordIds: string[]) {
    ...
    setData((prev) => {
      ...
      const sourceRecords = prev.batches.find((batch) => batch.id === current?.id)?.records.filter((record) => selectedIds.has(record.id)) || [];
      const transferred = new Map(sourceRecords.map((record) => {
        const review = requestSecondReview(record, requestedBy, `Returned from Step 3 for an independent second review: ${record.reason}`, now);
        const taskKey = priorityTaskKey(record.workflowSource === "ROOT" ? "ROOT" : record.id.startsWith("draft_brand_") ? "UBQ" : "CSV", record.id, record.name);
        return [record.priorityQueueId || `priority:${encodeURIComponent(taskKey)}`, { review, taskKey }];
      }));
      ...
      const batches = prev.batches.map((batch) => batch.id === current?.id ? ({ ...batch, adminCompletedAt: undefined, records: batch.records.filter((record) => !selectedIds.has(record.id)) }) : batch);
      ...
      const priorityQueue = prev.priorityQueue.map((item) => {
        ...
        return { ...item, status: "UNASSIGNED" as const, assignedTo: undefined, assignedAt: undefined, completedAt: undefined, exportedAt: undefined, exportedBy: undefined, exportFilename: undefined, externalStatus: "NOT_STARTED" as const, secondReviewRequired: true, firstReviewedBy: review.firstReviewedBy, firstReviewedAt: review.firstReviewedAt, secondReviewRequestedBy: requestedBy, secondReviewRequestedAt: now, requestedAction: review.action, requestedTargetId: review.targetId, requestedTargetName: review.targetName, reviewRequestNote: review.secondReviewReason, updatedAt: now, activity: [queueActivity("REOPENED", `Independent second review requested by ${requestedBy}`, now, requestedBy), ...(item.activity || [])].slice(0, 30) };
      });
      transferred.forEach(({ review, taskKey }, id) => {
        if (existingIds.has(id)) return;
        priorityQueue.push({ id, taskKey, brandId: review.id, name: review.name, source: review.workflowSource === "ROOT" ? "ROOT" : review.id.startsWith("draft_brand_") ? "UBQ" : "CSV", listingCount: review.listingCount, skuCount: review.skuCount, status: "UNASSIGNED", externalStatus: "NOT_STARTED", createdAt: now, createdBy: requestedBy, updatedAt: now, secondReviewRequired: true, firstReviewedBy: review.firstReviewedBy, firstReviewedAt: review.firstReviewedAt, secondReviewRequestedBy: requestedBy, secondReviewRequestedAt: now, requestedAction: review.action, requestedTargetId: review.targetId, requestedTargetName: review.targetName, reviewRequestNote: review.secondReviewReason, activity: [queueActivity("REOPENED", `Independent second review requested by ${requestedBy}`, now, requestedBy)] });
      });
      return { ...prev, batches, priorityQueue };
    });
    setReviewFocusIds([]);
    ...
    setSelected(null);
    sessionStorage.setItem("brandmaster.queue.filter", "UNASSIGNED");
    setToast(`${uniqueIds.length} brand${uniqueIds.length === 1 ? "" : "s"} sent to the team queue for an independent second review`);
  }
  ...
  return <div className={`app-shell ebay-theme unified-mode ${cleanTriage ? "clean-workflow" : "advanced-workflow"} view-${view}`}>
    ...
    <main aria-busy={Boolean(navigationTarget || navigationPending)}>
      ...
      {LOCAL_MODE && <div className="offline-workspace-banner local-mode-banner" role="status"><CloudOff size={19} /><span><b>Working offline</b><small>Local mode is active. Files are read from this Mac and saved only in this browser.</small></span><button onClick={() => navigate("settings")}>Open local data sources</button></div>}
      <div className="page">
        ...
        {!LOCAL_MODE && view === "brand-cleanup" && <OnlineBrandCleanupV2 data={data} currentUser={currentUser} initialSelectedIds={brandCleanupFocusIds} onSave={(brand) => saveCatalogBrand(brand, "BRAND_CLEANUP")} onSaveEnrichmentResource={saveEnrichmentResource} />}
        ...
        {view === "cleanup" && <SmartCleanup data={data} ubqSource={currentUbqSource} onSaveRoot={saveCatalogBrand} onValidate={(source, ids) => source === "ROOT" && !LOCAL_MODE ? (setBrandCleanupFocusIds(ids), navigate("brand-cleanup")) : startSourceWorklist(source, ids)} onAddPriority={addPriorityRows} onSetConfirmation={updateCleanupConfirmations} onNavigate={navigate} />}
        ...
        {view === "ledger" && <Ledger entries={data.ledger} records={allRecords} pendingRecords={pendingTeamRecords} priorityQueue={data.priorityQueue} onRebuild={rebuildHistoryUpload} />}
        ...
        {view === "analytics" && <Analytics records={allRecords} ledger={data.ledger} historicalMappings={data.historicalMappings} priorityQueue={data.priorityQueue} completionActivity={teamWeeklyCompletionActivity} rootChanges={data.rootChanges} teamActivity={data.teamActivity} currentUser={queueUser || "team"} />}
...
function DataPager({ page, pageSize, total, onPage, onPageSize, label = "rows", sizes = [10, 25, 50, 100] }: { page: number; pageSize: number; total: number; onPage: (page: number) => void; onPageSize?: (size: number) => void; label?: string; sizes?: number[] }) {
  const pages = Math.max(1, Math.ceil(total / pageSize));
  const safePage = Math.min(page, pages);
  const first = total ? (safePage - 1) * pageSize + 1 : 0;
  const last = Math.min(safePage * pageSize, total);
  return <div className="data-pager"><span>{first.toLocaleString()}–{last.toLocaleString()} of {total.toLocaleString()} {label}</span>{onPageSize && <label>Show<select value={pageSize} onChange={(event) => onPageSize(Number(event.target.value))}>{sizes.map((size) => <option key={size} value={size}>{size}</option>)}</select></label>}<div><button className="icon-button" disabled={safePage === 1} onClick={() => onPage(1)} title="First page"><ChevronLeft size={14} /><ChevronLeft size={14} /></button><button className="secondary" disabled={safePage === 1} onClick={() => onPage(safePage - 1)}>Previous</button><b>Page {safePage} of {pages}</b><button className="secondary" disabled={safePage === pages} onClick={() => onPage(safePage + 1)}>Next</button><button className="icon-button" disabled={safePage === pages} onClick={() => onPage(pages)} title="Last page"><ChevronRight size={14} /><ChevronRight size={14} /></button></div></div>;
}
...
function ImportPreflightDialog({ decisions: allDecisions, onCancel, onConfirm, onReviewAgain }: { decisions: ImportIntakeDecision[]; onCancel: () => void; onConfirm: () => void; onReviewAgain: (keys: string[]) => void }) {
  ...
  const imported = allDecisions.filter((item) => item.outcome === "IMPORTED").length;
  ...
  return <div className="modal-backdrop import-preflight-backdrop" role="presentation"><section className="import-preflight-dialog" role="dialog" aria-modal="true" aria-labelledby="import-preflight-title">
    <div className="import-preflight-head"><span><ShieldCheck size={25} /></span><div><small>STEP 1 CHECK · CONFIRM BEFORE PROCESSING</small><h2 id="import-preflight-title">You submitted {allDecisions.length} brands</h2><p>Brandmaster found previous or active work. Nothing has been imported yet. Review every outcome below and decide whether to continue.</p></div></div>
    <div className="import-preflight-counts"><span className="will-import"><b>{imported + selectedCount}</b><small>Will import</small></span><span className="will-not-import"><b>{notImported - selectedCount}</b><small>Will not import</small></span><span><b>{allDecisions.length}</b><small>Total submitted</small></span></div>
    ...
    <DataPager page={page} pageSize={pageSize} total={allDecisions.length} onPage={setPage} label="submitted brands" />
...
function CompletedBrandDialog({ details, onConfirm }: { details: CompletedBrandDetail[]; onConfirm: () => void }) {
  ...
  return <><div className="modal-backdrop completed-brand-backdrop" role="presentation"><section className="completed-brand-dialog" role="dialog" aria-modal="true" aria-labelledby="completed-brand-title">
    ...
    <div className="completed-brand-table">
      ...
      {visibleDetails.map((detail) => <div key={`${detail.brand}:${detail.action}:${detail.date}`}><strong>{detail.brand}</strong><span className={`completed-action ${detail.action.toLowerCase()}`}>{detail.action}</span><time>{fmtDate(detail.date)}<small>{fmtTime(detail.date)}</small></time></div>)}
    </div>
    <DataPager page={page} pageSize={pageSize} total={details.length} onPage={setPage} label="completed brands" />
...
function ProcessingView({ run }: { run: ProcessingRun }) {
  const progress = run.processed === undefined ? Math.round(((run.current + 1) / run.steps.length) * 100) : Math.round(run.processed / Math.max(1, run.count) * 100);
  const mode = run.source === "ROOT" ? "root" : "ubq";
  return <div className={`processing-page ${mode}`}><span className="processing-mode">{run.source === "ROOT" ? "ROOT TABLE CLEANUP" : "UBQ BRAND CLEANUP"}</span><div className="processing-orbit"><span className="orbit-ring ring-one" /><span className="orbit-ring ring-two" /><div><WandSparkles size={30} /><b>{progress}%</b></div></div><span className="processing-eyebrow">VALIDATION ENGINE RUNNING</span><h1>Checking {run.count.toLocaleString()} brand{run.count === 1 ? "" : "s"}</h1><p>{run.processed === undefined ? run.filename : `${run.processed.toLocaleString()} of ${run.count.toLocaleString()} processed in small batches`}</p><div className="process-progress"><i style={{ width: `${progress}%` }} /></div><div className="process-modules">{run.steps.map((step, index) => <div className={index < run.current ? "done" : index === run.current ? "active" : ""} key={step}><span>{index < run.current ? <Check size={14} /> : index === run.current ? <Activity size={14} /> : index + 1}</span><div><b>{step}</b><small>{index < run.current ? "Checked" : index === run.current ? run.processed === undefined ? "Searching now…" : `Processing ${run.processed.toLocaleString()} of ${run.count.toLocaleString()}` : "Waiting"}</small></div>{index === run.current && <em><i /><i /><i /></em>}</div>)}</div><small className="processing-note"><ShieldCheck size={13} />Work is processed locally in small batches so the page stays responsive. Your data is not uploaded.</small></div>;
}
...
function PriorityQueue({ items, currentUser, pinnedQueueIds, teamMembers, maxSelection, onChooseTeamMember, onTogglePin, syncConnected, savePending, saveBusy, saveCountdown, lastSavedAt, onSave, onUpdate, onReset, onRemove, onAdminDone, onStart, onNavigate }: { items: PriorityQueueItem[]; currentUser: string; pinnedQueueIds: string[]; teamMembers: string[]; maxSelection?: number; onChooseTeamMember: (member: string) => void; onTogglePin: (id: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; lastSavedAt?: string; onSave: () => void; onUpdate: (ids: string[], status: PriorityQueueStatus, assignee?: string, force?: boolean) => void; onReset: (ids: string[]) => void; onRemove: (ids: string[]) => void; onAdminDone: (ids: string[]) => void; onStart: (ids: string[]) => void; onNavigate: (view: View) => void }) {
  ...
  const visible = activeItems.filter((item) => {
    const text = `${item.name} ${item.brandId} ${item.assignedTo || ""} ${item.finalTargetName || ""} ${item.finalTargetId || ""} ${item.requestedTargetName || ""} ${item.requestedTargetId || ""} ${item.reviewRequestNote || ""}`.toLowerCase();
  ...
  const startableSelected = selectedItems.filter((item) => item.status !== "COMPLETED").map((item) => item.id);
  ...
  useEffect(() => setPage(1), [queueQuery, queueSource, queueStatus, queueOwner, pageSize]);
  useEffect(() => setPage((current) => Math.min(current, Math.max(1, Math.ceil(visible.length / pageSize)))), [visible.length, pageSize]);
...
function Imports({ cleanMode, batches, activeBatchId, priorityQueue, currentUser, pinnedQueueIds, teamMembers, onChooseTeamMember, onTogglePin, syncConnected, savePending, saveBusy, saveCountdown, lastSavedAt, onSave, onImport, onAddPriority, onUpdatePriority, onResetPriority, onRemovePriority, onAdminDone, onStartPriority, onNavigate, onRestart, ubqSource }: { cleanMode?: boolean; batches: ImportBatch[]; activeBatchId?: string; priorityQueue: PriorityQueueItem[]; currentUser: string; pinnedQueueIds: string[]; teamMembers: string[]; onChooseTeamMember: (member: string) => void; onTogglePin: (id: string) => void; syncConnected: boolean; savePending: boolean; saveBusy: boolean; saveCountdown: number; lastSavedAt?: string; onSave: () => void; onImport: (name: string, rows: ReturnType<typeof parseCsv>) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onUpdatePriority: (ids: string[], status: PriorityQueueStatus, assignee?: string) => void; onResetPriority: (ids: string[]) => void; onRemovePriority: (ids: string[]) => void; onAdminDone: (ids: string[]) => void; onStartPriority: (ids: string[]) => void; onNavigate: (v: View) => void; onRestart: () => void; ubqSource: UbqSource | null }) {
  ...
  const [intakeProgress, setIntakeProgress] = useState<{ title: string; detail: string; progress: number; count: number } | null>(null);
  ...
  function startValidation(filename: string, rows: ReturnType<typeof parseCsv>, initialProgress = 18) {
    if (!rows.length) return;
    setError("");
    setIntakeProgress({ title: "Preparing validation", detail: `Securing ${rows.length.toLocaleString()} brand${rows.length === 1 ? "" : "s"} for the validation engine`, progress: initialProgress, count: rows.length });
    window.setTimeout(() => {
      setIntakeProgress({ title: "Running validation checks", detail: "Comparing names, prior decisions, aliases, Root, ACA, and FPA sources", progress: 62, count: rows.length });
      window.setTimeout(() => {
        onImport(filename, rows);
        finishIntake("Validation ready", "Opening the review worklist", rows.length);
      }, 120);
    }, 120);
  }
  function startQueueImport(source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>, after?: () => void) {
    if (!rows.length) return;
    setError("");
    setIntakeProgress({ title: "Updating the team queue", detail: `Preparing ${rows.length.toLocaleString()} shared brand${rows.length === 1 ? "" : "s"}`, progress: 34, count: rows.length });
    window.setTimeout(() => {
      onAddPriority(source, rows);
      after?.();
      finishIntake("Team queue updated", "The shared work is ready to claim", rows.length);
    }, 140);
  }
  function accept(file?: File, target = destination.current) {
    ...
    setError("");
    setIntakeProgress({ title: "Reading CSV", detail: file.name, progress: 8, count: 0 });
    const reader = new FileReader();
    reader.onprogress = (event) => {
      if (!event.lengthComputable) return;
      setIntakeProgress({ title: "Reading CSV", detail: file.name, progress: Math.max(8, Math.min(38, Math.round(event.loaded / event.total * 38))), count: 0 });
    };
    reader.onload = () => {
      setIntakeProgress({ title: "Parsing brand rows", detail: "Checking the file structure and required fields", progress: 44, count: 0 });
      window.setTimeout(() => {
        const rows = parseCsv(String(reader.result));
        if (!rows.length) { setIntakeProgress(null); setError("No brand rows found. Include UnmappedBrandID and UnmappedBrandName columns."); }
        else if (overLimit(rows.length)) { setIntakeProgress(null); setError(`Add no more than ${MAX_WORKLIST_SIZE} brands in Clean View, or switch to Advanced View.`); }
        else if (target === "queue") { setIntakeProgress(null); startQueueImport("CSV", rows); }
        else { setIntakeProgress(null); window.setTimeout(() => startValidation(file.name, rows, 52), 0); }
      }, 80);
    };
    reader.onerror = () => { setIntakeProgress(null); setError("This CSV could not be read."); };
  ...
  function validatePasted() { if (!intakeBusy) startValidation("pasted-brand-list.csv", pastedRows); }
  ...
  return <>{!cleanMode && <WorkflowStepper stage={1} onNavigate={onNavigate} onRestart={onRestart} hasImport={Boolean(currentBatch)} outputReady={currentOutputReady} owner={currentBatch?.owner || currentUser || "Shared team"} counts={currentCounts} basketRecords={currentBatch?.records || []} />}
    ...
    {intakeProgress && <section className="intake-progress" role="status" aria-live="polite" aria-label={`${intakeProgress.title}: ${intakeProgress.progress}% complete`}>
      <div className="intake-progress-orbit"><span /><span /><WandSparkles size={24} /></div>
      <div className="intake-progress-copy"><small>BRANDMASTER IS WORKING</small><b>{intakeProgress.title}</b><p>{intakeProgress.detail}</p><div className="intake-progress-track"><i style={{ width: `${intakeProgress.progress}%` }} /></div></div>
      <strong>{intakeProgress.progress}%<small>{intakeProgress.count ? `${intakeProgress.count.toLocaleString()} brands` : "Please wait"}</small></strong>
    </section>}
    {cleanMode && <section className="step1-choice-grid" aria-label="Choose how to start Step 1">
      <section className="step1-choice-card add step1-add-tile">
        ...
        <div className="step1-add-tile-actions">{currentBatch ? <button className="primary" onClick={onRestart}><RotateCcw size={15} />Start over to add brands</button> : <><button className="secondary" disabled={intakeBusy} onClick={() => { setCleanChoice("add"); destination.current = "validate"; input.current?.click(); }}><FileUp size={15} />Add CSV</button><button className="primary" disabled={intakeBusy || !currentUser || !pastedNames.length || pastedNames.length > MAX_WORKLIST_SIZE} onClick={validatePasted}>{intakeBusy ? <RefreshCw className="spinning" size={15} /> : <WandSparkles size={15} />}{intakeBusy ? "Validating…" : <>Add &amp; review{pastedNames.length ? ` ${pastedNames.length}` : ""}</>}</button></>}</div>
      </section>
    ...
    <section className={`compact-import advanced-worklist-legacy ${inputMode === "priority" ? "priority-intake-open" : ""} ${cleanMode && cleanChoice === "add" ? "clean-add-open" : ""} ${cleanMode && cleanChoice !== "add" ? "clean-choice-hidden" : ""}`}>{cleanMode && <div className="step1-expanded-head"><span><FileUp size={20} /></span><div><small>OPTION 1 · ADD BRANDS</small><b>Add 1–{MAX_WORKLIST_SIZE} brands</b><p>Paste names directly or add a CSV file.</p></div><button className="secondary" onClick={() => setCleanChoice(null)}><X size={15} />Close</button></div>}
      {cleanMode ? <div className="clean-add-composer">
        ...
        <div className="clean-add-actions">
          ...
          <button className="primary clean-add-review" disabled={intakeBusy || !currentUser || !pastedNames.length || pastedNames.length > MAX_WORKLIST_SIZE} onClick={validatePasted}>{intakeBusy ? <RefreshCw className="spinning" size={16} /> : <WandSparkles size={16} />}{intakeBusy ? "Validating…" : <>Add &amp; review{pastedNames.length ? ` ${pastedNames.length}` : ""}</>}</button>
        </div>
      </div> : <>
...
function AiReviewAssist({ records, knownBrandIds, onUpdate, initiallyOpen = false, selectionMode = false, onClose }: { records: BrandRecord[]; knownBrandIds: Set<string>; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; initiallyOpen?: boolean; selectionMode?: boolean; onClose?: () => void }) {
  const [open, setOpen] = useState(initiallyOpen); const [copied, setCopied] = useState(false); const [correctionCopied, setCorrectionCopied] = useState(false); const [response, setResponse] = useState(""); const [result, setResult] = useState<ReturnType<typeof parseAiReviewJson> | null>(null); const [strictness, setStrictness] = useState<AiReviewStrictness>("STANDARD"); const [customInstructions, setCustomInstructions] = useState(""); const jsonInput = useRef<HTMLInputElement>(null);
  const reviewableRecords = records;
  const unresolvedIdCount = records.filter((record) => record.workflowSource !== "ROOT" && !record.id.startsWith("draft_brand_")).length;
  const requestId = useMemo(() => aiReviewRequestId(reviewableRecords), [reviewableRecords]);
  const prompt = useMemo(() => buildAiReviewPrompt(reviewableRecords, { strictness, customInstructions }), [reviewableRecords, strictness, customInstructions]);
  ...
  return <section className={`ai-review ${open ? "open" : ""}`}>
    ...
    {open && <div className="ai-review-body">
      {unresolvedIdCount > 0 && <div className="ai-review-id-warning"><CircleHelp size={16} /><span><b>{unresolvedIdCount} row{unresolvedIdCount === 1 ? " has" : "s have"} a temporary ID</b><small>AI Review will research these brands now. Resolve each real UBQ ID before continuing to Step 3 or exporting.</small></span></div>}
      <div className="ai-review-step"><div className="step-number">1</div><div className="ai-review-content"><div className="ai-review-title"><div><h3>Generate the validator prompt</h3><p>Request <code>{requestId}</code> replaces earlier chat batches and allowlists only these {reviewableRecords.length} brands.</p></div><div><button className="secondary" onClick={() => download("brandmaster-ai-review-prompt.txt", prompt)}><ArrowDownToLine size={14} />Download</button><button className="primary" onClick={copyPrompt}>{copied ? <Check size={14} /> : <BookOpen size={14} />}{copied ? "Copied" : "Copy prompt"}</button></div></div><div className="ai-prompt-options"><label>Review mode<select value={strictness} onChange={(e) => setStrictness(e.target.value as AiReviewStrictness)}><option value="BASIC">Basic — include small and marketplace brands</option><option value="STANDARD">Standard — balanced evidence</option><option value="STRICT">Strict — direct evidence</option><option value="SUPER_STRICT">Super strict — strongest proof only</option></select></label><label>Custom instructions<textarea value={customInstructions} onChange={(e) => setCustomInstructions(e.target.value)} placeholder="Optional: add your own review requirements…" /></label></div><textarea className="prompt-preview" value={prompt} readOnly /></div></div>
...
function InlineReviewEditor({ record, brands, rootMode = false, onCancel, onFullReview, onSave }: { record: BrandRecord; brands: CatalogBrand[]; rootMode?: boolean; onCancel: () => void; onFullReview: () => void; onSave: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void }) {
  ...
  return <div className="inline-review-editor" onClick={(event) => event.stopPropagation()}><div className="inline-editor-head"><div><Pencil size={15} /><span><b>Edit {record.name}</b><small>Fast manual review · the full side window is still available</small></span></div><button className="text-button" onClick={onFullReview}>Open full review →</button></div><div className="inline-editor-fields">
    ...
    <label><span>{rootMode ? "Root recommendation" : "Action"}</span><select value={action} onChange={(event) => changeAction(event.target.value as Action)}>{(["MERGE", "CREATE", "SKIP", "DELETE"] as Action[]).map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
...
function ReviewQueue({ cleanMode, records, batch, brands, ubqRows, knownBrandIds, focusIds, onClearFocus, onUpdate, onApproveAndContinue, onResolveUbqId, onResolveWithoutMapping, onSelect, query, onNavigate, onRestart }: { cleanMode?: boolean; records: BrandRecord[]; batch?: ImportBatch; brands: CatalogBrand[]; ubqRows: ParsedRow[]; knownBrandIds: Set<string>; focusIds: string[]; onClearFocus: () => void; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onApproveAndContinue: (ids: string[]) => ApprovalContinueResult; onResolveUbqId: (id: string, row: ParsedRow) => void; onResolveWithoutMapping: (ids: string[], resolution: NonNullable<BrandRecord["triageResolution"]>, note?: string) => void; onSelect: (r: BrandRecord) => void; query: string; onNavigate: (view: View) => void; onRestart: () => void }) {
  ...
  const isMissingId = (record: BrandRecord) => record.workflowSource !== "ROOT" && (!record.ubqVerified || !record.id.startsWith("draft_brand_"));
  const visible = focusedRecords.filter((record) => {
    ...
    const idVisible = idFilter === "ALL" || (idFilter === "MISSING" ? isMissingId(record) : !isMissingId(record));
    const activeQuery = `${query} ${reviewQuery}`.trim().toLowerCase();
    return statusVisible && idVisible && (actionFilter === "ALL" || record.action === actionFilter) && `${record.id} ${record.name} ${record.normalized} ${record.action} ${record.targetName || ""}`.toLowerCase().includes(activeQuery);
  });
  ...
  const allFilteredSelected = selectionScope.length > 0 && selectionScope.every((record) => checked.includes(record.id));
  useEffect(() => setReviewPage(1), [filter, actionFilter, idFilter, reviewQuery, query]);
  ...
  const missingIdCount = rootMode ? 0 : focusedRecords.filter(isMissingId).length;
  ...
  return <>{!cleanMode && <WorkflowStepper stage={2} onNavigate={onNavigate} onRestart={onRestart} hasImport outputReady={exportReady} rootMode={rootMode} counts={triageCounts} />}<PageHead eyebrow={rootMode ? "ROOT CLEANUP · SECOND STEP" : cleanMode ? undefined : "STEP 2 OF 3"} title={rootMode ? "Review Root cleanup decisions" : cleanMode ? `Review ${focusedRecords.length} brand${focusedRecords.length === 1 ? "" : "s"}` : "Process and review"} body={cleanMode ? `${focusedNeeds} need${focusedNeeds === 1 ? "s" : ""} a decision · ${focusedRecords.length - focusedNeeds} ready. Review the table, then continue directly below it.` : focusedReview ? "Only the rows returned from Step 3 are shown. The rest of the batch remains ready and unchanged." : "Confirm recommendations before generating a file for the real bulk-upload tool."} actions={<><button className="secondary" onClick={() => onNavigate("imports")}><ChevronLeft size={15} />Back</button>{focusedReview && <button className="secondary" onClick={onClearFocus}>Show all Step 2 brands</button>}{!rootMode && missingIdCount > 0 && <button className={idFilter === "MISSING" ? "primary" : "secondary"} onClick={() => { setIdFilter("MISSING"); setFilter("all"); setActionFilter("ALL"); setReviewQuery(""); window.setTimeout(() => document.getElementById("review-worklist-filters")?.scrollIntoView({ behavior: "smooth", block: "start" }), 50); }}><Database size={15} />{idFilter === "MISSING" ? `Showing ${missingIdCount} missing IDs` : `Show ${missingIdCount} missing IDs`}</button>}</>} />
    ...
    {cleanMode ? <div className="review-toolbar" id="review-worklist-filters"><div className="tabs"><button className={filter === "all" && idFilter === "ALL" ? "active" : ""} onClick={() => { setFilter("all"); setIdFilter("ALL"); }}>All brands <span>{focusedRecords.length}</span></button><button className={filter === "needs-review" && idFilter === "ALL" ? "active" : ""} onClick={() => { setFilter("needs-review"); setIdFilter("ALL"); }}>Needs your decision <span>{focusedNeeds}</span></button><button className={filter === "ready" && idFilter === "ALL" ? "active" : ""} onClick={() => { setFilter("ready"); setIdFilter("ALL"); }}>Already ready <span>{focusedRecords.length - focusedNeeds}</span></button>{missingIdCount > 0 && <button className={idFilter === "MISSING" ? "active missing-id-tab" : "missing-id-tab"} onClick={() => { setFilter("all"); setIdFilter(idFilter === "MISSING" ? "ALL" : "MISSING"); }}><Database size={13} />Missing IDs <span>{missingIdCount}</span></button>}</div><span className="review-visible-count">Showing <b>{visible.length}</b> of <b>{focusedRecords.length}</b></span></div>
    : <div className="review-toolbar simplified" id="review-worklist-filters"><div className="review-toolbar-title"><b>Brands for Review</b><label><Search size={14} /><input value={reviewQuery} onChange={(event) => setReviewQuery(event.target.value)} placeholder="Search brands…" /></label></div><div className="tabs status-tabs"><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "needs-review" ? "active" : ""} onClick={() => setFilter("needs-review")}>To Review</button><button className={filter === "ready" ? "active" : ""} onClick={() => setFilter("ready")}>Approved</button><button className={filter === "disapproved" ? "active" : ""} onClick={() => setFilter("disapproved")}>Disapproved</button></div><div className="review-secondary-filters"><label className="action-filter">Decision<select value={actionFilter} onChange={(event) => setActionFilter(event.target.value as "ALL" | Action)}><option value="ALL">All</option>{(["MERGE", "CREATE", "SKIP", "DELETE"] as Action[]).map((action) => <option key={action}>{action}</option>)}</select><ChevronDown size={14} /></label>{!rootMode && <label className={`action-filter id-filter ${idFilter === "MISSING" ? "missing-active" : ""}`}>Brand ID<select value={idFilter} onChange={(event) => setIdFilter(event.target.value as "ALL" | "MISSING" | "VERIFIED")}><option value="ALL">All IDs</option><option value="MISSING">Missing IDs ({missingIdCount})</option><option value="VERIFIED">Verified IDs ({Math.max(0, focusedRecords.length - missingIdCount)})</option></select><ChevronDown size={14} /></label>}</div><span className="review-visible-count">{visible.length} items</span></div>}
    {checked.length > 0 && <div className="bulk-bar"><b>{checked.length} selected{!cleanMode && visible.length > reviewPageSize ? ` across ${reviewPageCount} pages` : ""}</b><button onClick={() => bulk()}><Check size={13} />Approve &amp; continue</button><button onClick={() => bulk("MERGE")}>Merge</button><button onClick={() => bulk("SKIP")}>Skip</button><button onClick={() => bulk("DELETE")}>Delete</button><button className="bulk-ai-review" onClick={() => setAiReviewIds([...checked])}><Sparkles size={14} />AI Review</button>{!rootMode && <button className="resolve-without-mapping" onClick={() => setResolutionDialog([...checked])}>Resolve without mapping</button>}<button className="icon-button" onClick={() => { setChecked([]); setAiReviewIds([]); }}><X size={16} /></button></div>}
    {bulkNotice && <section className={`bulk-approval-notice ${bulkNotice.kind}`}><span>{bulkNotice.kind === "missing" ? <Database size={18} /> : <ShieldCheck size={18} />}</span><div><b>{bulkNotice.kind === "missing" ? "Approval saved—Brand IDs still need attention" : "Approval saved—more checks remain"}</b><p>{bulkNotice.message}</p></div><button className="icon-button" onClick={() => setBulkNotice(null)} title="Dismiss"><X size={15} /></button></section>}
    {aiReviewIds.length > 0 && <AiReviewAssist records={activeRecords.filter((record) => aiReviewIds.includes(record.id))} knownBrandIds={knownBrandIds} onUpdate={onUpdate} initiallyOpen selectionMode onClose={() => setAiReviewIds([])} />}
    <div className="table-panel"><div className="data-table review-table research-enabled"><div className="table-row table-head-row"><div><input type="checkbox" aria-label={`Select all ${selectionScope.length} filtered brands across every page`} checked={allFilteredSelected} onChange={(e) => setChecked(e.target.checked ? selectionScope.map((r) => r.id) : [])} /></div>{!cleanMode && <div>Brand ID</div>}<div>{cleanMode ? rootMode ? "Root brand" : "Unmapped brand" : "Brand Name"}</div><div>{cleanMode ? "Normalized" : "Match Type"}</div><div>{cleanMode ? "Action" : "Decision Recommendation"}</div><div>Source</div><div>Confidence</div><div className="review-status-heading">{cleanMode ? "Status" : "Decision Status"}</div><div className="review-description-heading">{cleanMode ? "Manual research" : "Description"}</div><div className="review-action-heading">{cleanMode ? "Edit" : "Review"}</div></div>
      {(cleanMode ? visible : pagedVisible).map((r) => <Fragment key={r.id}>
        <div className={`table-row ${inlineEditId === r.id ? "editing" : ""}`} onClick={() => onSelect(r)}>
          ...
          {!cleanMode && <div className={`advanced-brand-id ${isMissingId(r) ? "missing" : ""}`}><code>{r.id}</code>{isMissingId(r) && <span className="missing-id-controls"><small>Missing ID</small><button className="fix-missing-id" onClick={(event) => { event.stopPropagation(); setIdFinderRecord(r); }}>Fix ID</button></span>}</div>}
          ...
          <div className="review-status-cell">{r.status === "needs-review" ? <span className="status review">Needs review</span> : r.status === "reviewed" ? <span className="status done"><Check size={12} />Saved task</span> : <span className="status ready"><Sparkles size={12} />Auto-ready</span>}</div>
          <div className="row-research-actions review-description-cell" onClick={(event) => event.stopPropagation()}>{cleanMode ? <><InternalBrandSearch name={r.name} rootBrands={brands} ubqRows={ubqRows} excludeId={r.id} onMerge={(brand) => onUpdate(r.id, { action: "MERGE", targetId: brand.id, targetName: brand.name, confidence: 100, status: "reviewed", reason: `Reviewer selected ${brand.name} from internal Root search`, decisionSource: "Internal Root + UBQ search", blockedByTargetCreation: false, mergeOverride: true }, true)} /><ResearchLinks name={r.name} />{rootMode ? <AdminBrandLink id={r.id} name={r.name} /> : <AdminUnknownBrandLink name={r.name} />}</> : <span className="advanced-review-note">{r.targetName || r.reason}</span>}</div>
          <div className="review-action-cell" onClick={(event) => event.stopPropagation()}><button className={`icon-button row-edit ${cleanMode && r.id === firstNeedsReviewId ? "recommended-action" : ""}`} onClick={() => setInlineEditId(inlineEditId === r.id ? null : r.id)} title={`Edit ${r.name} in this table`}><Pencil size={14} /></button></div>
        </div>
      ...
      {cleanMode ? <section className={`review-table-next ${exportReady ? "ready" : ""}`} aria-label="Continue to Step 3"><span className="review-table-next-icon">{exportReady ? <Check size={21} /> : <ShieldCheck size={21} />}</span><div><small>{rootMode ? "STEP 2 COMPLETE" : "STEP 2 COMPLETE"}</small><b>{exportReady ? (rootMode ? "All Root cleanup decisions are ready for Step 3" : `All ${activeRecords.length} brands are ready for Step 3`) : `${blockingChecks} check${blockingChecks === 1 ? "" : "s"} must be resolved first`}</b><p>{exportReady ? "Continue now to review and download the final file." : "Complete the highlighted decisions in this table. This button will turn on automatically."}</p></div><button className="primary" disabled={!exportReady} title={!exportReady ? "Resolve the remaining checks first" : "Continue to the output file"} onClick={() => onNavigate("output")}>Continue to Step 3 <ChevronRight size={16} /></button></section>
      : <><div className="advanced-review-pagination"><span>{visible.length ? `${(activeReviewPage - 1) * reviewPageSize + 1}–${Math.min(activeReviewPage * reviewPageSize, visible.length)}` : "0"} of {visible.length} items</span><div><button disabled={activeReviewPage === 1} onClick={() => setReviewPage(1)}>‹‹</button><button disabled={activeReviewPage === 1} onClick={() => setReviewPage((page) => Math.max(1, page - 1))}>‹</button><b>{activeReviewPage}</b><button disabled={activeReviewPage === reviewPageCount} onClick={() => setReviewPage((page) => Math.min(reviewPageCount, page + 1))}>›</button><button disabled={activeReviewPage === reviewPageCount} onClick={() => setReviewPage(reviewPageCount)}>››</button></div></div><section className={`review-table-next simplified ${exportReady ? "ready" : ""}`} aria-label={rootMode ? "Finish Root cleanup" : "Continue to Step 3"}><span>{visible.length ? `${(activeReviewPage - 1) * reviewPageSize + 1}–${Math.min(activeReviewPage * reviewPageSize, visible.length)}` : "0"} of {visible.length} items</span><small>{exportReady ? "All decisions are ready" : `${blockingChecks} check${blockingChecks === 1 ? "" : "s"} remaining`}</small><div><button className="secondary" disabled={!checked.length} onClick={() => bulk()}><Check size={15} />Approve selected &amp; continue</button>{rootMode ? <button className="primary" disabled={!exportReady} onClick={() => onNavigate("output")}>Save and Finish</button> : <button className="primary" disabled={!exportReady} title={!exportReady ? "Resolve the remaining checks first" : "Continue to the output file"} onClick={() => onNavigate("output")}>Save and Continue to Step 3 <ChevronRight size={15} /></button>}</div></section></>}
    </div>
...
function BulkOutput({ cleanMode, records: allRecords, batch, data, currentUser, onUpdate, onSetExcluded, onReopen, onApplyAdminUploadResults, onRecordBulkExport, onRecordRootExport, onConfirmRootTriage, onBeforeExport, onNavigate, onRestart }: { cleanMode?: boolean; records: BrandRecord[]; batch?: ImportBatch; data: AppData; currentUser: string; onUpdate: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onSetExcluded: (ids: string[], excluded: boolean) => void; onReopen: (ids: string[]) => void; onApplyAdminUploadResults: (batchId: string | undefined, attemptedIds: string[], rows: AdminUploadResultRow[], exportFilename: string, resultFilename: string, moveFailuresToReview: boolean, markNotFoundDone?: boolean, exportRunId?: string) => void; onRecordBulkExport: (run: ExportRun) => void; onRecordRootExport: (changes: AppData["rootChanges"][string][], filename: string) => void; onConfirmRootTriage: (batchId: string | undefined, recordIds: string[], filename: string) => void; onBeforeExport: (onProgress?: (step: "local" | "team") => void) => Promise<boolean>; onNavigate: (view: View) => void; onRestart: () => void }) {
  ...
  const visibleOutputRecords = records.slice((outputPage - 1) * outputPageSize, outputPage * outputPageSize);
  useEffect(() => setOutputPage(1), [batch?.id, outputPageSize]);
  useEffect(() => setOutputPage((current) => Math.min(current, Math.max(1, Math.ceil(records.length / outputPageSize)))), [records.length, outputPageSize]);
  ...
  async function finishAdminResult(rows: AdminUploadResultRow[], resultFilename: string, moveFailures: boolean, markNotFoundDone = false) {
    ...
    await new Promise((resolve) => setTimeout(resolve, 220));
      onApplyAdminUploadResults(batch?.id, attemptedIds, rows, exportContext.filename, resultFilename, moveFailures, markNotFoundDone, currentExportRun?.id);
  ...
  return <><WorkflowStepper stage={3} onNavigate={onNavigate} onRestart={onRestart} hasImport={records.length > 0} outputReady={ready} rootMode={rootMode} counts={getTriageCounts(records, rootMode)} basketRecords={records} />
    ...
    {!records.length ? <div className="panel"><EmptyState icon={FileUp} title="No brands have reached Step 3" body="Start at Step 1 by adding brands, then confirm every required decision in Step 2." action={<button className="primary" onClick={() => onNavigate("imports")}>Go to Step 1</button>} /></div> : !ready ? <div className="output-blocked"><div className="output-status-icon"><FileClock size={24} /></div><h2>Your file needs attention</h2><p>Return to Step 2 and resolve every check before downloading the final file.</p><div className="output-checks">{!rootMode && <span className={invalidIds ? "bad" : "good"}>{invalidIds ? <X size={14} /> : <Check size={14} />}Valid unmapped IDs <b>{invalidIds ? `${invalidIds} missing` : "Complete"}</b></span>}<span className={needs ? "bad" : "good"}>{needs ? <X size={14} /> : <Check size={14} />}Review decisions <b>{needs ? `${needs} remaining` : "Complete"}</b></span><span className={(rootMode ? rootIncomplete : invalidMerges) ? "bad" : "good"}>{(rootMode ? rootIncomplete : invalidMerges) ? <X size={14} /> : <Check size={14} />}MERGE targets <b>{(rootMode ? rootIncomplete : invalidMerges) ? `${rootMode ? rootIncomplete : invalidMerges} incomplete` : "Complete"}</b></span>{!rootMode && <span className={invalidCreates ? "bad" : "good"}>{invalidCreates ? <X size={14} /> : <Check size={14} />}CREATE target names <b>{invalidCreates ? `${invalidCreates} incomplete` : "Complete"}</b></span>}</div><button className="primary" onClick={() => onNavigate("review")}>Return to Step 2 review</button></div> : rootMode ? <>
      ...
      <section className="panel output-preview output-worklist">
        <div className="panel-head"><div><h2>Manage the final worklist</h2><p>Use Include all to restore every row to this download. To request another review, select rows and send them to second review.</p></div><div className="output-worklist-head-actions">{excludedRecords.length > 0 && <button className="primary" onClick={() => onSetExcluded(records.map((record) => record.id), false)}><Check size={14} />Include all {records.length.toLocaleString()} rows in download</button>}<span className="status done"><Check size={12} />5 export columns unchanged</span></div></div>
        {selectedRows.length > 0 && <div className="output-bulk-manager"><span><b>{selectedRows.length} selected</b><small>Choose one action for these rows</small></span><button className="primary" onClick={() => onReopen(selectedRows)}><ChevronLeft size={14} />Send selected to second review</button><button className="secondary" onClick={() => applySelectedExclusion(true)}>Exclude selected</button><button className="secondary" onClick={() => applySelectedExclusion(false)}>Include selected</button><button className="secondary" onClick={setSelectedToSkip}>Set selected to SKIP</button><button className="icon-button" onClick={() => setSelectedRows([])} title="Clear selection"><X size={16} /></button></div>}
        <div className="output-table">
          <div><span className="output-select"><input type="checkbox" aria-label="Select this page" checked={visibleOutputRecords.length > 0 && visibleOutputRecords.every((record) => selectedRowSet.has(record.id))} onChange={(event) => { const pageIds = visibleOutputRecords.map((record) => record.id); setSelectedRows(event.target.checked ? [...new Set([...selectedRows, ...pageIds])] : selectedRows.filter((id) => !pageIds.includes(id))); }} /></span><b>Include</b><b>UnmappedBrandID</b><b>UnmappedBrandName</b><b>Action</b><b>TargetBrandID</b><b>TargetBrandName</b><b>Row controls</b></div>
          {visibleOutputRecords.map((record) => { const included = !record.excludedFromExport && !record.triageResolution; return <div className={`${!included ? "excluded " : ""}${selectedRowSet.has(record.id) ? "selected" : ""}`} key={record.id}><span className="output-select"><input type="checkbox" aria-label={`Select ${record.name}`} checked={selectedRowSet.has(record.id)} onChange={(event) => setSelectedRows(event.target.checked ? [...selectedRows, record.id] : selectedRows.filter((id) => id !== record.id))} /></span><label className="output-include"><input type="checkbox" checked={included} onChange={(event) => onSetExcluded([record.id], !event.target.checked)} /><span>{included ? "Include" : "Excluded"}</span></label><code>{record.id}</code><span>{record.name}</span><ActionPill action={record.action} /><code>{record.action === "MERGE" ? record.targetId : ""}</code><span>{record.action === "CREATE" || record.action === "MERGE" ? record.targetName : ""}</span><div className="output-row-controls"><button className="secondary" disabled={record.action === "SKIP"} onClick={() => onUpdate(record.id, { action: "SKIP", targetId: undefined, targetName: undefined, reason: "Set to SKIP during Step 3 export review", excludedFromExport: false }, true)}>Set SKIP</button><button className="secondary" onClick={() => onReopen([record.id])}><ChevronLeft size={13} />Send to second review</button></div></div>; })}
        </div>
        <DataPager page={outputPage} pageSize={outputPageSize} total={records.length} onPage={setOutputPage} onPageSize={(size) => { setOutputPageSize(size); setOutputPage(1); }} label="rows" />
      </section>
    </>}
...
function DecisionDrawer({ record, records, brands, ubqRows, onClose, onSave, onApplyRelated }: { record: BrandRecord; records: BrandRecord[]; brands: CatalogBrand[]; ubqRows: { id: string; name: string }[]; onClose: () => void; onSave: (id: string, changes: Partial<BrandRecord>, learn?: boolean) => void; onApplyRelated: (ids: string[], targetId: string, targetName: string) => void }) {
  ...
  return <><div className="drawer-scrim" onClick={onClose} /><aside className="drawer"><div className="drawer-head"><div><span>BRAND DECISION</span><h2>{record.name}</h2></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div><div className="drawer-body">
    ...
    <section><h3>{rootMode ? "Root recommendation" : "Your decision"}</h3><div className="action-picker">{(["MERGE", "CREATE", "SKIP", "DELETE"] as Action[]).map((a) => <button key={a} className={`${a.toLowerCase()} ${action === a ? "active" : ""}`} onClick={() => changeAction(a)}><span>{a === "MERGE" ? "↗" : a === "CREATE" ? "+" : a === "SKIP" ? "–" : "×"}</span>{a}<Check size={14} /></button>)}</div>
...
function SmartCleanup({ data, ubqSource, onSaveRoot, onValidate, onAddPriority, onSetConfirmation, onNavigate }: { data: AppData; ubqSource: UbqSource | null; onSaveRoot: (brand: CatalogBrand) => void; onValidate: (source: "ROOT" | "UBQ", ids: string[]) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>, forceReopen?: boolean) => void; onSetConfirmation: (source: CleanupSource, rows: { brandId: string; name: string; fingerprint: string }[], status: "CONFIRMED" | "REOPENED") => void; onNavigate: (view: View) => void }) {
  ...
  const bulkSelectedIssues = selected.length === page.length && page.length > 0 ? page : selectedIssues;
  const actionableIssues = (rows: CleanupIssue[]) => rows.filter((issue) => {
    const record = sourceRecords.get(issue.brandId);
    return !record || !findCompletedBrandDetails(data, [{ id: issue.brandId, name: issue.name }]).length;
  });
    ...
    setTimeout(() => {
      const found = source === "ROOT" ? analyzeRootBrands(data.rootBrands) : analyzeUbqBrands(ubqSource ? [...ubqSource.byId.values()] : [], data.rootBrands)
        .filter((issue) => {
          ...
          return !row || !findCompletedBrandDetails(data, [row]).some((detail) => normalizeBrand(detail.brand).toLowerCase() === normalizeBrand(issue.name).toLowerCase());
        });
...
function BrandDatabase({ data, ubqSource, query, onSave, onUndoRootChange, onUpdateRootTask, onValidate, onAddPriority }: { data: AppData; ubqSource: UbqSource | null; query: string; onSave: (brand: CatalogBrand) => void; onUndoRootChange: (id: string) => void; onUpdateRootTask: (id: string, status: NonNullable<AppData["rootChanges"][string]["adminStatus"]>) => void; onValidate: (source: "ROOT" | "UBQ", ids: string[]) => void; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void }) {
  ...
  const ubqRows = useMemo(() => {
    ...
    const completedNames = new Set(findCompletedBrandDetails(data, [...ubqSource.byId.values()]).map((detail) => normalizeBrand(detail.brand).toLowerCase()));
    return [...ubqSource.byId.values()]
      .filter((row) => !completedNames.has(normalizeBrand(row.name).toLowerCase()))
      .filter((row) => `${row.name} ${row.id}`.toLowerCase().includes(ubqQuery.trim().toLowerCase()))
      .slice(0, 250);
...
function PendingWork({ data, onNavigate }: { data: AppData; onNavigate: (view: View) => void }) {
  ...
  const downloadSelected = () => {
    ...
    const adminReady = records.length > 0 && records.every((record) => ["READY_TO_UPLOAD", "DOWNLOADED"].includes(workflowStage(record)));
    download(`brandmaster-${adminReady ? "admin-ready" : "workflow-handoff"}-${new Date().toISOString().slice(0, 10)}.csv`, adminReady ? toCsv(records) : workflowHandoffCsv(records));
  };
...
function Ledger({ entries, records, pendingRecords, priorityQueue, onRebuild }: { entries: LedgerEntry[]; records: BrandRecord[]; pendingRecords: BrandRecord[]; priorityQueue: PriorityQueueItem[]; onRebuild: (entries: LedgerEntry[]) => void }) {
  const [query, setQuery] = useState("");
  ...
  const pendingRows = useMemo(() => pendingBrandRows(pendingRecords, priorityQueue), [pendingRecords, priorityQueue]);
  const waitingForUpload = pendingRows.filter((row) => row.status === "WAITING_FOR_UPLOAD");
  const waitingForSecondReview = pendingRows.filter((row) => row.status === "WAITING_FOR_SECOND_REVIEW");
  const visibleEntries = filtered.slice((page - 1) * pageSize, page * pageSize);
  useEffect(() => setPage(1), [query, action, source, reviewer, confidence, range, reviewDate, order, pageSize]);
  useEffect(() => setPage((current) => Math.min(current, Math.max(1, Math.ceil(filtered.length / pageSize)))), [filtered.length, pageSize]);
  function clearFilters() { setQuery(""); setAction("ALL"); setSource("ALL"); setReviewer("ALL"); setConfidence("ALL"); setRange("ALL"); setReviewDate(""); setOrder("NEWEST"); setPage(1); }
  return <><PageHead eyebrow="DECISION HISTORY" title="Review history" body="Choose a reviewed day to recover its Admin CSV directly, or search prior decisions and rebuild them in Step 3." actions={<><button className="secondary" disabled={!filtered.length} title="External report columns: Brand, DATE, ACTION" onClick={() => download("brandmaster-external-progress-report.csv", reviewHistoryProgressCsv(filtered))}><ArrowDownToLine size={16} />External progress report</button><button className="secondary" disabled={!exportRecords.length} onClick={() => download("brandmaster-review-history.json", JSON.stringify(exportRecords, null, 2), "application/json")}><ArrowDownToLine size={16} />Export shown details</button><button className="secondary" disabled={!rebuildable.length} onClick={() => onRebuild(rebuildable)}><RotateCcw size={16} />Rebuild in Step 3 · {rebuildable.length}</button><button className="primary" disabled={!reviewDate || !rebuildable.length} title={reviewDate ? "Download the uploadable decisions matching the current filters" : "Choose a reviewed day first"} onClick={() => download(`brandmaster-admin-review-history-${reviewDate}.csv`, reviewHistoryAdminCsv(filtered))}><ArrowDownToLine size={16} />{reviewDate ? `Admin CSV · ${rebuildable.length}` : "Choose day for Admin CSV"}</button></>} />
    <section className="pending-export-panel"><div><FileClock size={22} /><span><small>PENDING BRAND EXPORT</small><h2>Current handoff queue</h2><p>Track reviewed brands that still need Admin upload and reopened brands that need a second review.</p></span></div><div className="pending-export-counts"><span className="ready"><b>{waitingForUpload.length.toLocaleString()}</b><small>Waiting for upload</small></span><span className="attention"><b>{waitingForSecondReview.length.toLocaleString()}</b><small>Waiting for second review</small></span></div><div className="pending-export-actions"><button className="secondary" disabled={!pendingRows.length} title="Operational report with both pending states" onClick={() => download(`brandmaster-pending-brands-${new Date().toISOString().slice(0, 10)}.csv`, pendingBrandWorkflowCsv(pendingRows))}><ArrowDownToLine size={16} />Pending workflow CSV · {pendingRows.length}</button><button className="primary" disabled={!waitingForUpload.length} title="Exact five-column file; second-review rows are excluded" onClick={() => download(`brandmaster-pending-admin-upload-${new Date().toISOString().slice(0, 10)}.csv`, pendingBrandAdminCsv(pendingRows))}><ArrowDownToLine size={16} />Admin-ready CSV · {waitingForUpload.length}</button></div></section>
...
function LearningCenter({ data, currentUser, onUpdate, onBulk, onRebuild, onSendToQueue, onBulkSendToQueue }: { data: AppData; currentUser: string; onUpdate: (ruleId: string, changes: LearningRuleChanges, type: LearningModerationEventType, note: string) => void; onBulk: (ruleIds: string[], status: LearningOverrideStatus) => void; onRebuild: () => void; onSendToQueue: (rule: LearningRule, changes: LearningRuleChanges, note: string) => void; onBulkSendToQueue: (rules: LearningRule[]) => void }) {
  ...
  const trustCards: { key: LearningTrust; label: string; value: number; body: string }[] = [
    ...
    { key: "CONTRADICTED", label: "Conflict — review required", value: registry.stats.contradicted, body: "Blocked until a reviewer resolves the evidence conflict" },
  ];
  ...
  useEffect(() => setPage((current) => Math.min(current, Math.max(1, Math.ceil(rules.length / pageSize)))), [rules.length, pageSize]);
  const selectedSet = new Set(selected);
  const selectedRules = registry.rules.filter((rule) => selectedSet.has(rule.id));
  const selectedHasConflicts = selectedRules.some((rule) => rule.trust === "CONTRADICTED");
  ...
  return <>
    ...
    <section className="learning-trust-grid">
      {trustCards.map((card) => <button key={card.key} className={`${card.key.toLowerCase()} ${trust === card.key ? "active" : ""}`} onClick={() => { const next = trust === card.key ? "ALL" : card.key; setTrust(next); if (card.key === "CONTRADICTED") { setStatus(next === "ALL" ? "ACTIVE" : "ALL"); setCleanup(next === "ALL" ? "ALL" : "CONTRADICTED"); } }}><span>{card.label}</span><b>{card.value.toLocaleString()}</b><small>{card.body}</small></button>)}
    </section>
    ...
    <section className="learning-cleanup panel"><div><AlertTriangle size={19} /><span><b>VLR cleanup queue</b><small>{cleanupCount.toLocaleString()} rules need attention across old proposals, evidence conflicts, stale targets, and duplicate identities.</small></span></div>{(["ALL", "STALE", "PROPOSED", "CONTRADICTED", "DUPLICATES"] as const).map((item) => <button key={item} className={cleanup === item ? "active" : ""} onClick={() => { setCleanup(item); setStatus(item === "ALL" ? "ACTIVE" : "ALL"); setSelected([]); }}>{item === "ALL" ? "Active registry" : item === "CONTRADICTED" ? "CONFLICTS TO RESOLVE" : item.replaceAll("_", " ")}</button>)}</section>
    <div className="learning-layout">
      <section className="panel learning-rules">
        ...
        <div className="learning-filters"><label><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find brand, ID, or target…" /></label><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)}><option value="ACTIVE">Active only</option><option value="ALL">Every status</option><option value="DISABLED">Disabled</option><option value="ARCHIVED">Archived</option><option value="STALE">Stale</option></select><select value={trust} onChange={(event) => { const next = event.target.value as "ALL" | LearningTrust; setTrust(next); if (next === "CONTRADICTED") { setStatus("ALL"); setCleanup("CONTRADICTED"); } }}><option value="ALL">All trust levels</option>{trustCards.map((card) => <option key={card.key} value={card.key}>{card.label}</option>)}</select>{(query || trust !== "ALL" || status !== "ACTIVE" || cleanup !== "ALL") && <button className="text-button" onClick={() => { setQuery(""); setTrust("ALL"); setStatus("ACTIVE"); setCleanup("ALL"); }}>Clear</button>}</div>
        {selected.length > 0 && <div className={`learning-bulk ${selectedHasConflicts ? "conflict" : ""}`}><span><b>{selected.length} {selectedHasConflicts ? `conflict${selected.length === 1 ? "" : "s"}` : "selected"}</b><small>{selectedHasConflicts ? "Unresolved conflicts cannot be activated. Send them for research or disable them pending review." : "Moderation is synced and retained in each rule timeline."}</small></span><button className="primary" onClick={() => { onBulkSendToQueue(selectedRules); setSelected([]); }}><Users size={14} />{selectedHasConflicts ? "Send conflicts to Team Queue" : "Send to Team Queue"}</button>{!selectedHasConflicts && <button className="secondary" onClick={() => { onBulk(selected, "ACTIVE"); setSelected([]); }}>Activate</button>}<button className="secondary" onClick={() => { onBulk(selected, "DISABLED"); setSelected([]); }}>{selectedHasConflicts ? "Disable pending review" : "Disable"}</button><button className="secondary" onClick={() => { onBulk(selected, "ARCHIVED"); setSelected([]); }}>Archive</button><button className="icon-button" onClick={() => setSelected([])}><X size={14} /></button></div>}
        {rules.length ? <><div className="learning-rule-list"><div className="learning-rule-head"><input type="checkbox" checked={visibleRules.length > 0 && visibleRules.every((rule) => selectedSet.has(rule.id))} onChange={(event) => { const pageIds = visibleRules.map((rule) => rule.id); setSelected(event.target.checked ? [...new Set([...selected, ...pageIds])] : selected.filter((id) => !pageIds.includes(id))); }} /><span>Select this page</span></div>{visibleRules.map((rule) => <article key={rule.id} className={`${rule.isActive ? "" : "inactive"} ${rule.trust === "CONTRADICTED" ? "conflict" : ""} ${selectedSet.has(rule.id) ? "selected" : ""}`}><input type="checkbox" checked={selectedSet.has(rule.id)} onChange={(event) => setSelected(event.target.checked ? [...new Set([...selected, rule.id])] : selected.filter((id) => id !== rule.id))} /><span className={`learning-trust ${rule.trust.toLowerCase()}`}>{rule.trust === "CONTRADICTED" ? "CONFLICT · REVIEW" : rule.trust.replaceAll("_", " ")}</span><div><b>{rule.names[0] || rule.normalizedName}</b><small>{rule.sourceBrandId || `Normalized identity · ${rule.normalizedName}`}</small><p>{rule.action}{rule.targetName ? ` → ${rule.targetName}` : ""}{rule.targetId ? ` · ${rule.targetId}` : ""}</p>{(rule.trust === "CONTRADICTED" ? rule.contradictionReason || "New Admin or source evidence conflicts with this saved rule." : rule.staleReasons[0]) && <em>{rule.trust === "CONTRADICTED" ? rule.contradictionReason || "New Admin or source evidence conflicts with this saved rule." : rule.staleReasons[0]}</em>}</div><div className="learning-rule-meta"><b>{rule.confidence}%</b><small>{rule.trust === "CONTRADICTED" ? "BLOCKED PENDING RESOLUTION" : rule.moderationStatus}{rule.mergedIntoRuleId ? " · MERGED" : ""}</small>{rule.autoApplyEligible ? <em><Check size={11} />Exact-ID automation</em> : rule.correctionCount ? <em className="warning"><RotateCcw size={11} />{rule.correctionCount} correction{rule.correctionCount === 1 ? "" : "s"}</em> : null}<button className={rule.trust === "CONTRADICTED" ? "primary" : "secondary"} onClick={() => setEditing(rule)}>{rule.trust === "CONTRADICTED" ? "Resolve conflict" : "Inspect"}</button></div></article>)}</div><DataPager page={page} pageSize={pageSize} total={rules.length} onPage={setPage} onPageSize={(size) => { setPageSize(size); setPage(1); }} label="rules" /></> : <EmptyState icon={BrainCircuit} title="No learning rules match" body="Clear the filters, or review and verify more brand decisions to grow the registry." />}
      </section>
    ...
    {editing && <LearningRuleInspector key={`${editing.id}:${editing.lastUpdatedAt}`} rule={registry.rules.find((rule) => rule.id === editing.id) || editing} allRules={registry.rules} currentUser={currentUser} onClose={() => setEditing(null)} onUpdate={(changes, type, note) => { onUpdate(editing.id, changes, type, note); setEditing(null); }} onSendToQueue={(changes, note) => { onSendToQueue(editing, changes, note); setEditing(null); }} />}
  </>;
}

function LearningRuleInspector({ rule, allRules, currentUser, onClose, onUpdate, onSendToQueue }: { rule: LearningRule; allRules: LearningRule[]; currentUser: string; onClose: () => void; onUpdate: (changes: LearningRuleChanges, type: LearningModerationEventType, note: string) => void; onSendToQueue: (changes: LearningRuleChanges, note: string) => void }) {
  type ConflictResolution = "UNRESOLVED" | "USE_CORRECTED" | "KEEP_EXISTING" | "MERGE_DUPLICATE" | "NEEDS_RESEARCH";
  ...
  const [resolution, setResolution] = useState<ConflictResolution>(isConflict ? "UNRESOLVED" : "USE_CORRECTED");
  ...
  const conflictEvent = rule.provenance.find((event) => event.type === "CONTRADICTED");
  const fieldsValid = (action !== "MERGE" || (targetId.trim().startsWith("brand_") && targetName.trim())) && confidence >= 0 && confidence <= 100;
  const decisionChanged = action !== rule.action || targetId !== (rule.targetId || "") || targetName !== (rule.targetName || "") || confidence !== rule.confidence;
  const resolutionValid = !isConflict || (resolution !== "UNRESOLVED" && (resolution !== "USE_CORRECTED" || decisionChanged) && (resolution !== "KEEP_EXISTING" || Boolean(note.trim())) && (resolution !== "MERGE_DUPLICATE" || Boolean(mergedIntoRuleId)));
  const valid = fieldsValid && resolutionValid;
  function chooseResolution(next: ConflictResolution) {
    setResolution(next);
    if (next === "KEEP_EXISTING" || next === "USE_CORRECTED") setStatus("ACTIVE");
    if (next === "KEEP_EXISTING") {
      setAction(rule.action); setTargetId(rule.targetId || ""); setTargetName(rule.targetName || ""); setConfidence(rule.confidence); setMergedIntoRuleId("");
    }
    if (next === "MERGE_DUPLICATE") setStatus("ARCHIVED");
    if (next === "NEEDS_RESEARCH") { setStatus("DISABLED"); setMergedIntoRuleId(""); }
  }
  function save() {
    if (!valid) return;
    if (isConflict && resolution === "NEEDS_RESEARCH") { sendToQueue(); return; }
    const evidenceChanged = excludedEvidence.length !== rule.excludedEvidenceCount;
    const corrected = decisionChanged;
    const nextStatus: LearningOverrideStatus = mergedIntoRuleId || resolution === "MERGE_DUPLICATE" ? "ARCHIVED" : isConflict ? "ACTIVE" : status;
    const type: LearningModerationEventType = mergedIntoRuleId ? "IDENTITY_MERGED" : isConflict ? "CORRECTED" : corrected ? "CORRECTED" : evidenceChanged ? (excludedEvidence.length > rule.excludedEvidenceCount ? "EVIDENCE_EXCLUDED" : "EVIDENCE_RESTORED") : nextStatus === "ARCHIVED" ? "ARCHIVED" : nextStatus === "DISABLED" ? "DISABLED" : "ACTIVATED";
    const message = note.trim() || (mergedIntoRuleId ? `Resolved the conflict by merging this duplicate identity into ${allRules.find((candidate) => candidate.id === mergedIntoRuleId)?.names[0] || mergedIntoRuleId}.` : isConflict ? `Resolved the conflict using the corrected decision: ${action}${targetName ? ` → ${targetName}` : ""}.` : corrected ? `Corrected VLR rule to ${action}${targetName ? ` → ${targetName}` : ""}.` : evidenceChanged ? "Updated the rule's active evidence set." : `${nextStatus === "ACTIVE" ? "Activated" : nextStatus === "DISABLED" ? "Disabled" : "Archived"} this VLR rule.`);
  ...
  function sendToQueue() {
    ...
    const message = note.trim() || (isConflict ? `${rule.contradictionReason || "Conflicting Admin or source evidence"}. Research which decision is correct before this VLR rule is reactivated.` : `Review the proposed ${action}${targetName ? ` → ${targetName}` : ""} correction in Admin before this VLR rule is reactivated.`);
    onSendToQueue({ status: "DISABLED", action, targetId: action === "MERGE" ? targetId.trim() : undefined, targetName: action === "MERGE" || action === "CREATE" ? (targetName.trim() || rule.normalizedName) : undefined, confidence, mergedIntoRuleId: undefined, excludedEvidence, note: message }, message);
  }
  return <><div className="fresh-dialog-scrim" onClick={onClose} /><section className="learning-inspector" role="dialog" aria-modal="true" aria-labelledby="learning-inspector-title">
    ...
    <div className="learning-inspector-summary"><span className={`learning-trust ${rule.trust.toLowerCase()}`}>{isConflict ? "CONFLICT · REVIEW REQUIRED" : rule.trust.replaceAll("_", " ")}</span><span className={`learning-moderation ${rule.isActive ? "active" : "inactive"}`}>{rule.isActive ? "ACTIVE IN TRIAGE" : "BLOCKED FROM TRIAGE"}</span><small>Editing as {currentUser}</small></div>
    <div className="learning-admin-actions"><span><b>Admin handoff</b><small>Inspect the source queue record or the current canonical target before saving a correction.</small></span><AdminUnknownBrandLink name={rule.names[0] || rule.normalizedName} compact />{rule.targetId?.startsWith("brand_") && <AdminBrandLink id={rule.targetId} name={rule.targetName || rule.normalizedName} compact />}</div>
    {isConflict && <section className="learning-conflict-panel">
      <div className="learning-conflict-head"><AlertTriangle size={19} /><span><b>Conflict — review required</b><p>This rule is blocked. Compare the saved rule with the new signal, then choose one clear outcome.</p></span></div>
      <div className="learning-conflict-compare"><article><small>SAVED VLR RULE</small><b>{rule.action}{rule.targetName ? ` → ${rule.targetName}` : ""}</b><p>{rule.confidence}% confidence · {rule.reviewer || "Unknown reviewer"}</p><time>{fmtDate(rule.lastUpdatedAt)}</time></article><article><small>NEW CONFLICTING SIGNAL</small><b>{rule.contradictionReason || "Admin or source evidence disagrees with this saved rule."}</b><p>{conflictEvent?.by || "Admin/source reconciliation"}</p><time>{fmtDate(conflictEvent?.at || rule.lastUpdatedAt)}</time></article></div>
      <div className="learning-resolution-options" role="radiogroup" aria-label="Conflict resolution"><button className={resolution === "USE_CORRECTED" ? "active" : ""} onClick={() => chooseResolution("USE_CORRECTED")}><Check size={15} /><span><b>Use corrected decision</b><small>Edit the fields below and replace the saved rule.</small></span></button><button className={resolution === "KEEP_EXISTING" ? "active" : ""} onClick={() => chooseResolution("KEEP_EXISTING")}><ShieldCheck size={15} /><span><b>Keep saved rule</b><small>Confirm the original rule and explain why.</small></span></button><button className={resolution === "MERGE_DUPLICATE" ? "active" : ""} onClick={() => chooseResolution("MERGE_DUPLICATE")}><Combine size={15} /><span><b>Merge duplicate brands</b><small>Archive this identity under another active rule.</small></span></button><button className={resolution === "NEEDS_RESEARCH" ? "active" : ""} onClick={() => chooseResolution("NEEDS_RESEARCH")}><Users size={15} /><span><b>Not enough evidence</b><small>Disable it and send it to the Team Queue.</small></span></button></div>
    </section>}
    ...
    <div className="learning-inspector-grid"><label><span>Moderation status</span>{isConflict ? <div className="learning-locked-status"><ShieldCheck size={14} />{resolution === "UNRESOLVED" ? "Blocked pending resolution" : resolution === "NEEDS_RESEARCH" ? "Disabled · Team Queue" : resolution === "MERGE_DUPLICATE" ? "Archived after merge" : "Active after resolution"}</div> : <select value={status} onChange={(event) => { setStatus(event.target.value as LearningOverrideStatus); if (event.target.value !== "ARCHIVED") setMergedIntoRuleId(""); }}><option value="ACTIVE">Active</option><option value="DISABLED">Disabled</option><option value="ARCHIVED">Archived</option></select>}</label><label><span>Action</span><select value={action} onChange={(event) => setAction(event.target.value as Action)}>{(["MERGE", "CREATE", "SKIP", "DELETE"] as Action[]).map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Confidence</span><input type="number" min={0} max={100} value={confidence} onChange={(event) => setConfidence(Number(event.target.value))} /></label>{action === "MERGE" && <label><span>Target BrandID</span><input value={targetId} onChange={(event) => setTargetId(event.target.value)} placeholder="brand_…" /></label>}{(action === "MERGE" || action === "CREATE") && <label><span>Target brand name</span><input value={targetName} onChange={(event) => setTargetName(event.target.value)} /></label>}<label className="learning-merge-field"><span>{resolution === "MERGE_DUPLICATE" ? "Merge duplicate identity into (required)" : "Merge duplicate identity into"}</span><select value={mergedIntoRuleId} onChange={(event) => { setMergedIntoRuleId(event.target.value); if (event.target.value) setStatus("ARCHIVED"); }}><option value="">Keep as its own identity</option>{mergeCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.names[0] || candidate.normalizedName} · {candidate.action}{candidate.sourceBrandId ? ` · ${candidate.sourceBrandId}` : ""}</option>)}</select>{resolution === "MERGE_DUPLICATE" && !mergedIntoRuleId && <small className="learning-field-error">Choose the active rule that should own this duplicate.</small>}</label><label className="learning-note-field"><span>Moderation note{resolution === "KEEP_EXISTING" ? " (required)" : ""}</span><textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} placeholder={resolution === "KEEP_EXISTING" ? "Explain why the saved rule is correct despite the conflicting signal…" : "Why is this correction or cleanup appropriate?"} />{resolution === "USE_CORRECTED" && !decisionChanged && <small className="learning-field-error">Change the action, target, or confidence to use a corrected decision.</small>}{resolution === "KEEP_EXISTING" && !note.trim() && <small className="learning-field-error">A reason is required to keep a contradicted rule.</small>}</label></div>
    ...
    <footer><button className="secondary" onClick={onClose}>Cancel</button><button className={isConflict ? "primary learning-send-queue" : "secondary learning-send-queue"} disabled={!fieldsValid} onClick={sendToQueue}><Users size={14} />{isConflict ? "Send for team research" : "Disable & send to Team Queue"}</button><button className="primary" disabled={!valid} onClick={save}><Check size={14} />{isConflict ? resolution === "NEEDS_RESEARCH" ? "Send to Team Queue" : "Resolve conflict" : "Save moderated rule"}</button></footer>
  </section></>;
}

type EnrichmentJob = { id: string; status: string; processed: number; total?: number; filename?: string; error?: string };
type EnrichmentCandidate = { id: string; brandId?: string; currentName: string; suggestedName: string; currentAliases: string[]; suggestedAliases: string[]; removedAliases: string[]; reasons: string[]; confidence: number; decision?: "PENDING" | "APPROVE" | "REJECT"; evidence?: { url: string; title: string }[]; onlineStatus?: string };
function EnrichmentJourney({ job, candidates }: { job: EnrichmentJob; candidates: EnrichmentCandidate[] }) {
  const approved = candidates.filter((item) => item.decision === "APPROVE").length;
  const review = candidates.filter((item) => item.decision === "PENDING").length;
  const progress = job.status === "COMPLETED" ? 100 : 0;
  const steps = [
    [FileClock, "Raw Root table", `${Number(job.processed || 0).toLocaleString()} rows read`],
    [WandSparkles, "Clean & organize", "Normalize names and aliases"],
    [Globe, "Check online", `${candidates.filter((item) => item.onlineStatus).length} checked`],
...
function BrandEnrichment({ onPromote, teamConnected, onOpenSettings }: { onPromote: (items: EnrichmentCandidate[]) => Promise<void>; teamConnected: boolean; onOpenSettings: () => void }) {
  const [files, setFiles] = useState<string[]>([]); const [filename, setFilename] = useState(""); const [jobs, setJobs] = useState<EnrichmentJob[]>([]); const [job, setJob] = useState<EnrichmentJob | null>(null); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [candidates, setCandidates] = useState<EnrichmentCandidate[]>([]); const [exporting, setExporting] = useState(false); const [publishing, setPublishing] = useState(false); const [publishedCount, setPublishedCount] = useState<number | null>(null);
  const refresh = async () => { try { const result = await brandEnrichmentApi.files(); setFiles(result.files || []); const jobsResult = await brandEnrichmentApi.jobs(); setJobs(jobsResult.jobs || []); if (job) setJob((await brandEnrichmentApi.job(job.id)).job); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "The local enrichment service is not running."); } };
  const loadJob = async (id: string) => { const result = await brandEnrichmentApi.job(id); setJob(result.job); setCandidates(result.candidates || []); };
  useEffect(() => { void refresh(); const timer = window.setInterval(() => { void refresh(); if (job) void loadJob(job.id); }, 2500); return () => window.clearInterval(timer); }, [job?.id]);
  async function start() { setBusy(true); try { const result = await brandEnrichmentApi.start(filename); setJob(result.job); setFilename(""); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Could not start enrichment."); } finally { setBusy(false); } }
  async function update(id: string, changes: Record<string, unknown>) { try { await brandEnrichmentApi.updateCandidate(id, changes); if (job) await loadJob(job.id); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Could not save this decision."); } }
  async function checkOnline(id: string) { await brandEnrichmentApi.checkOnline(id); if (job) await loadJob(job.id); }
  async function exportCsv() { if (!job) return; setExporting(true); try { const result = await brandEnrichmentApi.exportJob(job.id); download(result.filename, result.csv); } finally { setExporting(false); } }
  const reviewed = candidates.filter((c) => c.decision !== "PENDING").length; const approved = candidates.filter((c) => c.decision === "APPROVE").length; const high = candidates.filter((c) => c.confidence >= .8).length;
  async function publish() { if (!teamConnected) { onOpenSettings(); return; } setPublishing(true); setError(""); try { await onPromote(candidates); setPublishedCount(candidates.length); } catch (e) { setError(e instanceof Error ? e.message : "Could not publish enrichment recommendations."); } finally { setPublishing(false); } }
  return <><PageHead eyebrow="BRAND TOOLS · LOCAL WORKER" title="Brand enrichment" body="Analyze locally, review in batches, then publish recommendations directly to the shared Brandmaster workspace." actions={<span className="status ready"><CloudOff size={12} />Local processing</span>} /><section className="enrichment-hero"><div><span className="enrichment-kicker">ONE-TIME CLEANUP WORKSPACE</span><h2>Make the Root brand list trustworthy</h2><p>Local processing protects page memory. Publishing sends only the recommendation resource to Team Sync; it does not overwrite Root or approve changes.</p></div><div className="enrichment-steps"><span><b>1</b>Analyze</span><span><b>2</b>Review</span><span><b>3</b>Publish</span></div></section><section className="panel enrichment-source"><div className="panel-head"><div><h2>1. Choose a local table</h2><p>Files are read from your local folder. The shared workspace is updated only when you publish completed recommendations.</p></div><button className="secondary" onClick={() => void refresh()}><RefreshCw size={15} />Refresh files</button></div><div className="enrichment-source-row"><select value={filename} onChange={(e) => setFilename(e.target.value)}><option value="">Choose Root or UBQ CSV…</option>{files.map((file) => <option key={file} value={file}>{file}</option>)}</select><button className="primary" disabled={busy || !filename} onClick={() => void start()}><SearchCheck size={15} />{busy ? "Starting…" : "Analyze table"}</button><span className="enrichment-file-note">{files.length ? `${files.length} local file${files.length === 1 ? "" : "s"} found` : "Put root.csv and ubq.csv in Brandmaster-data/local"}</span></div>{error && <div className="reference-error"><CircleHelp size={14} />{error}</div>}</section><section className="panel"><div className="panel-head"><div><h2>Runs</h2><p>Select a completed or running analysis to review its recommendations.</p></div></div><div className="enrichment-runs">{jobs.length ? jobs.map((item) => <button className={`enrichment-run ${job?.id === item.id ? "active" : ""}`} key={item.id} onClick={() => void loadJob(item.id)}><span className={`enrichment-run-dot ${item.status.toLowerCase()}`} /><span><b>{item.filename || "Local table"}</b><small>{item.status} · {Number(item.processed || 0).toLocaleString()} rows processed</small></span><ChevronRight size={16} /></button>) : <p className="enrichment-empty">No runs yet. Choose a local table above.</p>}</div></section>{job && <EnrichmentJourney job={job} candidates={candidates} />}{job && <section className="panel enrichment-review"><div className="panel-head"><div><span className="enrichment-kicker">2. HUMAN REVIEW</span><h2>Recommendations</h2><p>{job.status} · {candidates.length.toLocaleString()} recommendations · {reviewed.toLocaleString()} reviewed</p></div><button className="primary" disabled={!candidates.length || publishing} onClick={() => void publish()}><Database size={15} />{publishing ? "Publishing…" : teamConnected ? "Publish to shared workspace" : "Connect Team Sync to publish"}</button><button className="secondary" disabled={!candidates.length || exporting} onClick={() => void exportCsv()}><ArrowDownToLine size={15} />{exporting ? "Preparing…" : `Export backup`}</button></div>{publishedCount !== null && <div className="tables-ready"><Check size={16} /><div><b>{publishedCount.toLocaleString()} recommendations published</b><p>Brand Cleanup will use them after the shared workspace refreshes.</p></div></div>}<div className="enrichment-metrics"><div><b>{candidates.length.toLocaleString()}</b><span>Needs review</span></div><div><b>{high.toLocaleString()}</b><span>High confidence</span></div><div><b>{approved.toLocaleString()}</b><span>Approved</span></div><div><b>{(candidates.length - reviewed).toLocaleString()}</b><span>Remaining</span></div></div><EnrichmentCandidates items={candidates} onUpdate={update} onCheckOnline={checkOnline} /></section>}</>;
}

function EnrichmentCandidates({ items, onUpdate, onCheckOnline }: { items: EnrichmentCandidate[]; onUpdate: (id: string, changes: Record<string, unknown>) => Promise<void>; onCheckOnline: (id: string) => Promise<void> }) { const [query, setQuery] = useState(""); const [filter, setFilter] = useState("PENDING"); const [selected, setSelected] = useState<string[]>([]); const [page, setPage] = useState(1); const pageSize = 250; const visible = items.filter((item) => (!query || `${item.currentName} ${item.suggestedName} ${item.currentAliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())) && (filter === "ALL" || item.decision === filter || (filter === "HIGH" && item.confidence >= .8))); const pages = Math.max(1, Math.ceil(visible.length / pageSize)); const pageItems = visible.slice((page - 1) * pageSize, page * pageSize); const allPageSelected = pageItems.length > 0 && pageItems.every((item) => selected.includes(item.id)); const selectNext = () => { const next = visible.filter((item) => item.decision === "PENDING").slice(0, pageSize); setSelected((current) => [...new Set([...current, ...next.map((item) => item.id)])]); if (next.length) setPage(Math.floor(visible.indexOf(next[0]) / pageSize) + 1); }; const bulk = async (decision: "APPROVE" | "REJECT") => { await Promise.all(selected.map((id) => onUpdate(id, { decision }))); setSelected([]); }; useEffect(() => { if (page > pages) setPage(pages); }, [page, pages]); return <div className="enrichment-table-wrap"><div className="enrichment-toolbar"><label className="enrichment-search"><Search size={15} /><input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search brand or alias" /></label><select value={filter} onChange={(e) => { setFilter(e.target.value); setPage(1); }}><option value="PENDING">Needs review</option><option value="ALL">All recommendations</option><option value="HIGH">High confidence</option><option value="APPROVE">Approved</option><option value="REJECT">Rejected</option></select><span className="enrichment-selected">{selected.length} selected · {visible.length.toLocaleString()} shown</span><button className="secondary compact" disabled={!visible.some((item) => item.decision === "PENDING")} onClick={selectNext}>Review next 250 pending</button><button className="secondary" disabled={!selected.length} onClick={() => void bulk("REJECT")}>Reject selected</button><button className="primary" disabled={!selected.length} onClick={() => void bulk("APPROVE")}>Approve selected</button></div><div className="enrichment-table"><div className="enrichment-table-head"><label><input type="checkbox" checked={allPageSelected} onChange={() => setSelected(allPageSelected ? selected.filter((id) => !pageItems.some((item) => item.id === id)) : [...new Set([...selected, ...pageItems.map((item) => item.id)])])} /></label><span>Current brand / aliases</span><span>Proposed canonical form</span><span>Evidence & confidence</span><span>Human decision</span></div>{pageItems.map((item) => <article className="enrichment-row" key={item.id}><label><input type="checkbox" checked={selected.includes(item.id)} onChange={() => setSelected(selected.includes(item.id) ? selected.filter((id) => id !== item.id) : [...selected, item.id])} /></label><div><small className="enrichment-field-label">BEFORE</small><b>{item.currentName || "Unnamed brand"}</b><small>{item.currentAliases.length ? item.currentAliases.join(" · ") : "No aliases"}</small></div><div><small className="enrichment-field-label">PROPOSED</small><strong>{item.suggestedName}</strong><small>{item.suggestedAliases.length ? item.suggestedAliases.join(" · ") : "No proposed aliases"}</small></div><div><span className={`confidence-pill ${item.confidence >= .8 ? "high" : item.confidence >= .6 ? "medium" : "low"}`}>{Math.round(item.confidence * 100)}% confidence</span><small>{item.onlineStatus === "FOUND" ? "Online evidence found" : item.onlineStatus === "NO_MATCH" ? "No online match" : item.reasons[0] || "No issue detected"}</small>{item.evidence?.slice(0, 2).map((e) => <a href={e.url} target="_blank" rel="noreferrer" key={e.url}><Globe size={11} />{e.title || e.url}</a>)}</div><div className="enrichment-row-actions"><span className="decision-badge">{item.decision === "APPROVE" ? "SAVED · APPROVED" : item.decision === "REJECT" ? "SAVED · REJECTED" : "PENDING REVIEW"}</span><button type="button" className="secondary compact" onClick={() => void onCheckOnline(item.id)}><Globe size={13} />Check online</button><button type="button" className="secondary compact" onClick={() => void onUpdate(item.id, { decision: "REJECT" })}>Reject</button><button type="button" className="primary compact" onClick={() => void onUpdate(item.id, { decision: "APPROVE" })}>Approve</button></div></article>)}</div>{visible.length > 0 && <div className="brand-cleanup-pagination"><button className="secondary compact" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>Previous</button><span>Page {page} of {pages} · {pageItems.length} rows</span><button className="secondary compact" disabled={page >= pages} onClick={() => setPage((current) => current + 1)}>Next</button></div>}{!visible.length && <p className="enrichment-empty">No recommendations match this filter.</p>}</div>; }

function cleanupNameKey(value: string) { return value.normalize("NFKD").toLowerCase().replace(/[^a-z0-9]/g, ""); }
function cleanupSimilarity(left: string, right: string) { if (left === right) return 1; if (!left || !right || Math.abs(left.length - right.length) > 2) return 0; const previous = Array.from({ length: right.length + 1 }, (_, index) => index); for (let row = 1; row <= left.length; row += 1) { const current = [row]; for (let column = 1; column <= right.length; column += 1) current[column] = Math.min(current[column - 1] + 1, previous[column] + 1, previous[column - 1] + (left[row - 1] === right[column - 1] ? 0 : 1)); for (let column = 0; column <= right.length; column += 1) previous[column] = current[column]; } return 1 - previous[right.length] / Math.max(left.length, right.length); }
function EnrichmentResourceUploader({ count, onLoad }: { count: number; onLoad: (rows: AppData["enrichmentResources"]) => void }) { const input = useRef<HTMLInputElement>(null); const [message, setMessage] = useState(""); function accept(file?: File) { if (!file) return; const reader = new FileReader(); reader.onload = () => { const rows = parseCsv(String(reader.result)) as unknown as Array<Record<string, string>>; const resources = rows.map((row: Record<string, string>) => ({ brandId: row.brandid || row.id || "", rootName: row.rootname || row.currentname || row.name || "", rootAliases: String(row.rootaliases || row.currentaliases || "").split("|").map((x) => x.trim()).filter(Boolean), proposedName: row.proposedname || row.suggestedname || "", proposedAliases: String(row.proposedaliases || row.suggestedaliases || "").split("|").map((x) => x.trim()).filter(Boolean), confidence: Math.max(0, Math.min(1, Number(row.confidence || 0) / 100)), recommendation: row.recommendation === "NO_CHANGE" ? "NO_CHANGE" as const : "REVIEW" as const, acaMatch: row.aca_match_id ? { id: row.aca_match_id, name: row.aca_match_name || "", aliases: [], confidence: 0.92 } : undefined, evidence: String(row.evidence || "").split("|").filter(Boolean).map((detail) => ({ source: detail.startsWith("ACA") ? "ACA" : "Brand Enrichment", detail })), generatedAt: row.generatedat || new Date().toISOString() })).filter((row) => row.brandId && row.proposedName); onLoad(resources); setMessage(`${resources.length.toLocaleString()} recommendations loaded`); }; reader.readAsText(file); } return <div className="tables-ready"><Sparkles size={16} /><div><b>Brand Enrichment resource</b><p>{count ? `${count.toLocaleString()} enrichment recommendations are available as additional evidence.` : "Import the CSV exported by the offline Brand Enrichment worker. It recommends; it never approves."}{message ? ` ${message}.` : ""}</p></div><input ref={input} type="file" accept=".csv,text/csv" hidden onChange={(e) => { accept(e.target.files?.[0]); e.currentTarget.value = ""; }} /><button className="secondary compact" onClick={() => input.current?.click()}>{count ? "Replace resource" : "Add resource CSV"}</button></div>; }
function OnlineBrandCleanupV2({ data, currentUser, initialSelectedIds = [], onSave, onSaveEnrichmentResource }: { data: AppData; currentUser: string; initialSelectedIds?: string[]; onSave: (brand: CatalogBrand) => void; onSaveEnrichmentResource: (rows: AppData["enrichmentResources"]) => void }) {
  const [useAca, setUseAca] = useState(true); const [useEnrichment, setUseEnrichment] = useState(true); const [step, setStep] = useState<1 | 2 | 3>(1); const [query, setQuery] = useState(""); const [selected, setSelected] = useState<string[]>([]); const [page, setPage] = useState(1); const [filter, setFilter] = useState("ALL"); const [queueFilter, setQueueFilter] = useState("NEEDS_ATTENTION"); const [decisions, setDecisions] = useState<Record<string, "APPROVE" | "REJECT">>({}); const [online, setOnline] = useState<Record<string, boolean>>({}); const [onlineWebsite, setOnlineWebsite] = useState<Record<string, string>>({}); const [onlineStatus, setOnlineStatus] = useState<Record<string, string>>({}); const [edits, setEdits] = useState<Record<string, { name: string; aliases: string }>>({}); const [snapshots, setSnapshots] = useState<Record<string, { before: CatalogBrand; after: CatalogBrand; confidence: number }>>({}); const [onlineBusy, setOnlineBusy] = useState<string | null>(null); const [pasteList, setPasteList] = useState(""); const [pasteMessage, setPasteMessage] = useState(""); const [pasteMatchIds, setPasteMatchIds] = useState<string[]>([]); const [downloaded, setDownloaded] = useState(false); const [confirmed, setConfirmed] = useState(false);
  useEffect(() => { if (initialSelectedIds.length) { setSelected(initialSelectedIds); setStep(2); } }, [initialSelectedIds]);
  useEffect(() => { if (step === 2 && !selected.length) setStep(1); if (step === 3 && (!selected.length || Object.keys(decisions).length < selected.length)) setStep(2); }, [step, selected.length, decisions]);
  useEffect(() => { if (!LOCAL_MODE || data.enrichmentResources.length) return; let active = true; void brandEnrichmentApi.jobs().then(async (result) => { const latest = (result.jobs || []).find((job: EnrichmentJob) => job.status === "COMPLETED"); if (!latest) return; const detail = await brandEnrichmentApi.job(latest.id); if (!active) return; const resources = (detail.candidates || []).map((candidate: EnrichmentCandidate) => ({ brandId: candidate.brandId || candidate.id, rootName: candidate.currentName, rootAliases: candidate.currentAliases, proposedName: candidate.suggestedName, proposedAliases: candidate.suggestedAliases, confidence: candidate.confidence, recommendation: candidate.suggestedName === candidate.currentName && candidate.suggestedAliases.join("|") === candidate.currentAliases.join("|") ? "NO_CHANGE" as const : "REVIEW" as const, evidence: (candidate.evidence || []).map((e) => ({ source: e.title || "Brand Enrichment", url: e.url })), generatedAt: latest.completedAt || new Date().toISOString() })).filter((resource: AppData["enrichmentResources"][number]) => resource.brandId && resource.proposedName); if (resources.length) onSaveEnrichmentResource(resources); }).catch(() => undefined); return () => { active = false; }; }, [data.enrichmentResources.length, onSaveEnrichmentResource]);
  const pageSize = 20;
  const acaResourceStatus = data.acaBrands.length ? `Ready · ${data.acaBrands.length.toLocaleString()} rows` : "Unavailable · load ACA table";
  const enrichmentResourceStatus = data.enrichmentResources.length ? `Ready · ${data.enrichmentResources.length.toLocaleString()} recommendations` : "Unavailable · publish enrichment first";
  const duplicateIds = useMemo(() => { const buckets = new Map<string, { id: string; key: string }[]>(); data.rootBrands.forEach((brand) => { const key = cleanupNameKey(brand.name); const bucket = `${key.slice(0, 8)}:${key.length}`; buckets.set(bucket, [...(buckets.get(bucket) || []), { id: brand.id, key }]); }); const duplicates = new Set<string>(); buckets.forEach((items) => { if (items.length < 2) return; items.forEach((item, index) => { if (items.some((other, otherIndex) => otherIndex !== index && cleanupSimilarity(item.key, other.key) >= 0.98)) duplicates.add(item.id); }); }); return duplicates; }, [data.rootBrands]);
  const enrichmentById = useMemo(() => new Map(data.enrichmentResources.map((item) => [item.brandId, item])), [data.enrichmentResources]); const acaByKey = useMemo(() => { const map = new Map(); data.acaBrands.forEach((item) => { map.set(item.id, item); map.set(item.name.trim().toLowerCase(), item); }); return map; }, [data.acaBrands]);
  const candidates = useMemo(() => { const counts = new Map<string, number>(); data.rootBrands.forEach((brand) => { const key = brand.name.normalize("NFKC").replace(/[^\p{L}\p{N}\s]/gu, " ").replace(/\s+/g, " ").trim().toLowerCase(); counts.set(key, (counts.get(key) || 0) + 1); }); return data.rootBrands.map((brand) => { const resource = useEnrichment ? enrichmentById.get(brand.id) : undefined; const acaMatch = useAca ? (acaByKey.get(brand.id) || acaByKey.get(brand.name.trim().toLowerCase())) : undefined; const edit = edits[brand.id]; const cleanName = brand.name.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/\s+/g, " ").trim(); const name = edit?.name ?? resource?.proposedName ?? acaMatch?.name ?? cleanName.toLowerCase().replace(/(^|[\s-])\p{L}/gu, (m) => m.toUpperCase()); const aliases = edit?.aliases ?? [...new Set(brand.aliases.map((alias) => alias.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/\s+/g, " ").trim()).filter(Boolean))].join(", "); const aliasValues = aliases.split(",").map((a) => a.trim()).filter(Boolean); const reasons = [...(cleanName !== brand.name ? ["Name formatting needs normalization"] : []), ...(brand.aliases.some((alias) => alias !== alias.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/\s+/g, " ").trim()) ? ["Alias formatting needs normalization"] : []), ...(aliasValues.some((alias) => alias.length > 60 || /https?:|www\.|@/.test(alias) ? true : false) ? ["Alias contains suspicious text"] : []), ...(!brand.aliases.length ? ["No aliases available"] : []), ...((counts.get(cleanName.replace(/[^\p{L}\p{N}\s]/gu, " ").replace(/\s+/g, " ").trim().toLowerCase()) || 0) > 1 ? ["Possible duplicate canonical brand"] : []), ...(duplicateIds.has(brand.id) ? ["Possible 98%+ duplicate or near-duplicate"] : [])]; const changed = name !== brand.name || aliasValues.join("|") !== brand.aliases.join("|"); const category = reasons.some((reason) => reason.includes("suspicious") || reason.includes("duplicate")) ? "TRIAGE" : reasons.some((reason) => reason.includes("No aliases")) ? "ENHANCE" : changed ? "NORMALIZE" : "CLEAN"; const confidence = online[brand.id] ? 92 : category === "CLEAN" ? 100 : category === "TRIAGE" ? 55 : category === "ENHANCE" ? 68 : 78; return { brand, name, aliases, changed, reasons, category, confidence }; }); }, [data.rootBrands, data.acaBrands, data.enrichmentResources, enrichmentById, acaByKey, useAca, useEnrichment, edits, online, duplicateIds]);
  const prioritized = [...candidates.filter((item) => item.category === "TRIAGE"), ...candidates.filter((item) => item.category === "NORMALIZE"), ...candidates.filter((item) => item.category === "ENHANCE")]; const allMatching = (queueFilter === "ALL" ? candidates : prioritized).filter((item) => (!pasteMatchIds.length || pasteMatchIds.includes(item.brand.id)) && `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const filtered = allMatching.filter((item) => queueFilter === "ALL" || item.category === queueFilter || (queueFilter === "NEEDS_ATTENTION" && item.category !== "CLEAN")); const reviewRows = filtered.filter((item) => selected.includes(item.brand.id) && (filter === "ALL" || filter === "NEEDS_FIX" && item.category !== "CLEAN" || filter === "PENDING" && !decisions[item.brand.id] || filter === "APPROVED" && decisions[item.brand.id] === "APPROVE" || filter === "REJECTED" && decisions[item.brand.id] === "REJECT")); const pages = Math.max(1, Math.ceil(reviewRows.length / pageSize)); const pageRows = reviewRows.slice((page - 1) * pageSize, page * pageSize); const reviewed = Object.keys(decisions).length; const changed = Object.values(snapshots);
  const editFor = (item: typeof candidates[number]) => edits[item.brand.id] || { name: item.name, aliases: item.aliases };
  function applyPastedList() { const terms = pasteList.split(/[\n,\t]+/).map((value) => value.trim().toLowerCase()).filter(Boolean); const matches = candidates.filter((item) => terms.some((term) => term === item.brand.id.toLowerCase() || term === item.brand.name.trim().toLowerCase() || item.brand.aliases.some((alias) => term === alias.trim().toLowerCase()))); const matchedIds = [...new Set(matches.map((item) => item.brand.id))]; setPasteMatchIds(matchedIds); setQueueFilter("ALL"); setQuery(""); setPage(1); setSelected(matchedIds); setPasteMessage(`${matchedIds.length} found in Root · ${Math.max(0, terms.length - matchedIds.length)} not found`); }
  useEffect(() => { if (!selected.length && candidates.length) setSelected(prioritized.slice(0, pageSize).map((item) => item.brand.id)); }, [candidates, selected.length]);
  /* eslint-disable prefer-const */
  async function lookupOnline(item: typeof candidates[number]) { setOnlineBusy(item.brand.id); setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Checking approved sources…" })); try { const configured = data.validationSettings.approvedResearchSources || []; const enabled = configured.filter((source) => source.enabled); const has = (term: string) => enabled.length === 0 || enabled.some((source) => source.url.toLowerCase().includes(term)); let match: { label?: string; aliases?: { value: string }[] } | undefined; let evidence: string[] = []; let foundWebsite = ""; if (has("wikidata")) { const response = await fetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(item.name)}&language=en&format=json&origin=*`); const result = await response.json() as { search?: { label?: string; aliases?: { value: string }[] }[] }; match = result.search?.[0]; if (match) evidence.push("Wikidata"); } if (!match && has("wikipedia")) { const response = await fetch(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(item.name)}&limit=5&namespace=0&format=json&origin=*`); const result = await response.json() as [string, string[], string[], string[]]; if (result[1]?.[0]) { match = { label: result[1][0], aliases: [] }; evidence.push("Wikipedia"); } } if (!match) { const base = item.name.toLowerCase().replace(/\b(equipment|parts|company|corporation|industries|automotive|racing equipment)\b/g, " ").trim(); const slugs = [item.name, base, base.replace(/\s+/g, "")].map((value) => value.normalize("NFKD").replace(/[^a-z0-9]/g, "")).filter((value) => value.length >= 4); const domains = [...new Set(slugs.flatMap((slug) => [`${slug}.com`, `${slug}.net`, `${slug}.org`, `${slug}.io`, `${slug}.co`]))]; for (const domain of domains) { try { const response = await fetch(`https://dns.google/resolve?name=${encodeURIComponent(domain)}&type=A`); const result = await response.json() as { Answer?: unknown[] }; if (result.Answer?.length) { match = { label: item.name, aliases: [] }; foundWebsite = `https://${domain}`; evidence.push(`Domain resolves: ${domain}`); break; } } catch {} } } if (match) { setOnline((current) => ({ ...current, [item.brand.id]: true })); if (foundWebsite) setOnlineWebsite((current) => ({ ...current, [item.brand.id]: foundWebsite })); setOnlineStatus((current) => ({ ...current, [item.brand.id]: `Match found in ${evidence.join(" + ")} · review evidence` })); setEdits((current) => ({ ...current, [item.brand.id]: { name: match?.label || item.name, aliases: [...new Set([...(match?.aliases || []).map((alias) => alias.value), ...item.aliases.split(",").map((alias) => alias.trim()).filter(Boolean)])].join(", ") } })); } else setOnlineStatus((current) => ({ ...current, [item.brand.id]: "No approved-source match · human review required" })); } catch { setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Approved-source lookup unavailable · edit manually" })); } finally { setOnlineBusy(null); } }
  function decide(item: typeof candidates[number], decision: "APPROVE" | "REJECT") { const edit = editFor(item); setDecisions((current) => ({ ...current, [item.brand.id]: decision })); if (decision === "APPROVE" && item.changed) { const after = { ...item.brand, name: edit.name.trim(), aliases: edit.aliases.split(",").map((a) => a.trim()).filter(Boolean), ...(onlineWebsite[item.brand.id] ? { website: onlineWebsite[item.brand.id], brandEvidence: [{ source: "Domain resolution", url: onlineWebsite[item.brand.id], checkedAt: new Date().toISOString(), confidence: item.confidence }] } : {}) }; setSnapshots((current) => ({ ...current, [item.brand.id]: { before: item.brand, after, confidence: item.confidence } })); } else if (decision === "REJECT") setSnapshots((current) => { const next = { ...current }; delete next[item.brand.id]; return next; }); }
  function exportCsv() { const rows = changed.map((item) => [item.before.id, item.before.name, item.after.name, item.before.aliases.join(" | "), item.after.aliases.join(" | "), "APPROVE", item.confidence, currentUser, new Date().toISOString()]); const csv = [["BrandID", "BeforeBrand", "AfterBrand", "BeforeAliases", "AfterAliases", "Decision", "Confidence", "ReviewedBy", "ReviewedAt"], ...rows].map((row) => row.map((value) => `"${String(value ?? "").replaceAll('"', '""')}"`).join(",")).join("\n"); download(`brand-cleanup-changes-${new Date().toISOString().slice(0, 10)}.csv`, csv); setDownloaded(true); window.setTimeout(() => { if (window.confirm("The cleanup CSV was downloaded. Confirm that the Admin upload is complete to save these approved changes to the shared Root workspace.")) confirmUpload(); }, 0); }
  function confirmUpload() { changed.forEach((item) => onSave(item.after)); setConfirmed(true); }
  return <><PageHead eyebrow="DAILY WORK · ONLINE TEAM WORKFLOW" title="Brand Cleanup" body="Clean the shared Root table with pagination, filters, manual edits, online research, and reviewed exports." actions={<span className="status ready"><Cloud size={12} />Team shared</span>} /><section className="brand-cleanup-funnel"><div className="brand-cleanup-funnel-head"><div><span>BRAND CLEANUP JOURNEY</span><h2>Root table → reviewed master changes</h2><p>Only approved changes are exported. Unchanged reviewed rows are counted but omitted from the changes CSV.</p></div><strong>STEP {step} OF 3</strong></div><div className="brand-cleanup-steps">{[[1,"Add brands","Select Root rows"],[2,"Review decisions","Edit and verify"],[3,"Download file","Export changes only"]].map(([n,label,detail]) => <button key={n} className={step === Number(n) ? "active" : step > Number(n) ? "done" : ""} onClick={() => setStep(Number(n) as 1 | 2 | 3)}><b>{step > Number(n) ? <Check size={15} /> : n}</b><span><strong>{label}</strong><small>{detail}</small></span></button>)}</div></section>{step === 1 && <section className="panel brand-cleanup-panel"><div className="panel-head"><div><h2>1. Add brands from Root table</h2><p>{data.rootBrands.length.toLocaleString()} shared Root rows available.</p></div><button className="primary" disabled={!selected.length} onClick={() => setStep(2)}>Review {selected.length} brands</button></div><div className="tables-ready"><ShieldCheck size={16} /><div><b>Validation resources</b><p>Choose which existing workspace resources assist this cleanup run. They provide recommendations only; approval remains human-controlled.</p><label><input type="checkbox" checked={useAca} onChange={(e) => setUseAca(e.target.checked)} /> Use ACA table ({acaResourceStatus})</label><label><input type="checkbox" checked={useEnrichment} onChange={(e) => setUseEnrichment(e.target.checked)} /> Use enriched master recommendations ({enrichmentResourceStatus})</label></div></div><EnrichmentResourceUploader count={data.enrichmentResources.length} onLoad={onSaveEnrichmentResource} /><details className="brand-cleanup-paste"><summary>Paste your own brand list</summary><p>Paste one BrandID, brand name, or alias per line. Only matches found in Root will be selected.</p><textarea rows={4} value={pasteList} onChange={(e) => setPasteList(e.target.value)} placeholder={"brand_123…\nToyota\nAcme Parts"} /><div><button className="secondary compact" disabled={!pasteList.trim()} onClick={applyPastedList}>Find in Root and select</button>{pasteMessage && <span>{pasteMessage}</span>}</div></details><div className="brand-cleanup-review-tools"><select value={queueFilter} onChange={(e) => { setQueueFilter(e.target.value); setPage(1); setSelected([]); }}><option value="NEEDS_ATTENTION">Needs attention (recommended)</option><option value="TRIAGE">Triage only</option><option value="NORMALIZE">Normalize only</option><option value="ENHANCE">Enhance only</option><option value="ALL">All Root rows</option></select><button className="secondary compact" onClick={() => setSelected(filtered.slice((page - 1) * pageSize, page * pageSize).map((item) => item.brand.id))}>Select visible 20</button><button className="secondary compact" onClick={() => setSelected([])}>Clear selection</button><span>{filtered.length.toLocaleString()} in this queue · select what you want to review</span></div><label className="enrichment-search"><Search size={15} /><input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search BrandID, brand, or alias" /></label><div className="brand-cleanup-list">{filtered.slice((page - 1) * pageSize, page * pageSize).map((item) => <label key={item.brand.id}><input type="checkbox" checked={selected.includes(item.brand.id)} onChange={() => setSelected((current) => current.includes(item.brand.id) ? current.filter((id) => id !== item.brand.id) : [...current, item.brand.id])} /><span><b>{item.brand.name}</b><small>{item.brand.id} · {item.brand.aliases.length} aliases</small></span></label>)}</div></section>}{step === 2 && <section className="panel brand-cleanup-panel"><div className="panel-head"><div><h2>2. Review decisions</h2><p>Use the editor to correct the proposed brand and aliases before approval.</p></div><div className="brand-cleanup-actions"><button className="secondary" onClick={() => setStep(1)}>Back</button><button className="primary" disabled={reviewed < selected.length} onClick={() => setStep(3)}>Finish review</button></div></div><div className="brand-cleanup-review-tools"><label className="enrichment-search"><Search size={15} /><input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search selected brands" /></label><select value={filter} onChange={(e) => { setFilter(e.target.value); setPage(1); }}><option value="ALL">All selected</option><option value="NEEDS_FIX">Needs fixing</option><option value="PENDING">Pending review</option><option value="APPROVED">Approved</option><option value="REJECTED">Rejected</option></select><span>{reviewRows.length} rows · page {page} of {pages}</span></div><div className="brand-cleanup-review-list">{pageRows.map((item) => { const edit = editFor(item); return <article key={item.brand.id}><div><small>BEFORE · {item.brand.id}</small><b>{item.brand.name}</b><p>{item.brand.aliases.join(" · ") || "No aliases"}</p></div><ChevronRight size={18} /><div><small>PROPOSED · EDITABLE</small><input value={edit.name} onChange={(e) => setEdits((current) => ({ ...current, [item.brand.id]: { ...editFor(item), name: e.target.value } }))} /><textarea rows={2} value={edit.aliases} onChange={(e) => setEdits((current) => ({ ...current, [item.brand.id]: { ...editFor(item), aliases: e.target.value } }))} /><small>Separate aliases with commas</small></div><div><span className={`confidence-pill ${item.confidence >= 90 ? "high" : "medium"}`}>{item.confidence}% confidence</span><small>{onlineStatus[item.brand.id] || (item.changed ? "Needs fixing" : "No change detected")}</small></div><div className="brand-cleanup-row-actions"><button className="secondary compact" onClick={() => { void lookupOnline(item); }}><Globe size={13} />Search online</button><button className="secondary compact" onClick={() => decide(item, "REJECT")}>Keep original</button><button className="primary compact" onClick={() => decide(item, "APPROVE")}>Approve</button><span className="decision-badge">{decisions[item.brand.id] || "PENDING REVIEW"}</span></div></article>; })}</div><div className="brand-cleanup-pagination"><button className="secondary compact" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>Previous</button><span>Page {page} of {pages}</span><button className="secondary compact" disabled={page >= pages} onClick={() => setPage((p) => p + 1)}>Next</button></div></section>}{step === 3 && <section className="panel brand-cleanup-panel"><div className="brand-cleanup-complete"><Check size={28} /><div><h2>3. Download cleanup changes</h2><p>{changed.length} approved changes will be exported. Reviewed rows with no change are not included.</p></div><button className="primary" disabled={!reviewed} onClick={exportCsv}><ArrowDownToLine size={16} />Download changes CSV</button></div></section>}</>;
}

function OnlineBrandCleanup({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  const [step, setStep] = useState<1 | 2 | 3>(1); const [query, setQuery] = useState(""); const [selected, setSelected] = useState<string[]>([]); const [decisions, setDecisions] = useState<Record<string, "APPROVE" | "REJECT">>({}); const [online, setOnline] = useState<Record<string, boolean>>({}); const [approvedSnapshots, setApprovedSnapshots] = useState<Record<string, { before: CatalogBrand; after: CatalogBrand; confidence: number }>>({}); const [reviewPage, setReviewPage] = useState(1); const [reviewFilter, setReviewFilter] = useState("ALL"); const [edits, setEdits] = useState<Record<string, { name: string; aliases: string }>>({}); const pageSize = 25;
  const candidates = useMemo(() => data.rootBrands.map((brand) => { const suggested = brand.name.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/\s+/g, " ").trim().toLowerCase().replace(/(^|[\s-])\p{L}/gu, (m) => m.toUpperCase()); const aliases = [...new Set(brand.aliases.map((alias) => alias.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF]/g, "").replace(/\s+/g, " ").trim()).filter(Boolean))]; const changed = suggested !== brand.name || aliases.join("|") !== brand.aliases.join("|"); return { brand, suggested, aliases, changed, confidence: changed ? (online[brand.id] ? 92 : 78) : 100 }; }), [data.rootBrands, online]);
  const visible = candidates.filter((item) => `${item.brand.id} ${item.brand.name} ${item.brand.aliases.join(" ")}`.toLowerCase().includes(query.toLowerCase())); const reviewed = Object.keys(decisions).length; const approved = Object.values(decisions).filter((value) => value === "APPROVE").length; const changedRows = Object.values(approvedSnapshots); const reviewItems = candidates.filter((item) => selected.includes(item.brand.id) && (reviewFilter === "ALL" || reviewFilter === "NEEDS_FIX" && item.changed || reviewFilter === "PENDING" && !decisions[item.brand.id] || reviewFilter === "APPROVED" && decisions[item.brand.id] === "APPROVE" || reviewFilter === "REJECTED" && decisions[item.brand.id] === "REJECT")); ; const reviewPages = Math.max(1, Math.ceil(reviewItems.length / pageSize)); const pageItems = reviewItems.slice((reviewPage - 1) * pageSize, reviewPage * pageSize);
  function draft(item: typeof candidates[number]) { const edit = edits[item.brand.id]; return { name: edit?.name ?? item.suggested, aliases: edit?.aliases ?? item.aliases.join(", ") }; }
  function decide(item: typeof candidates[number], decision: "APPROVE" | "REJECT") { const proposed = draft(item); setDecisions((current) => ({ ...current, [item.brand.id]: decision })); if (decision === "APPROVE" && item.changed) { const after = { ...item.brand, name: proposed.name.trim(), aliases: proposed.aliases.split(",").map((alias) => alias.trim()).filter(Boolean) }; setApprovedSnapshots((current) => ({ ...current, [item.brand.id]: { before: item.brand, after, confidence: item.confidence } })); onSave(after); } else if (decision === "REJECT") setApprovedSnapshots((current) => { const next = { ...current }; delete next[item.brand.id]; return next; }); }
  function exportChanges() { const headers = ["BrandID", "BeforeBrand", "AfterBrand", "BeforeAliases", "AfterAliases", "Decision", "Confidence", "ReviewedBy", "ReviewedAt"]; const rows = changedRows.map((item) => [item.before.id, item.before.name, item.after.name, item.before.aliases.join(" | "), item.after.aliases.join(" | "), "APPROVE", item.confidence, currentUser, new Date().toISOString()]); const csv = [headers, ...rows].map((row) => row.map((value) => `"${String(value ?? "").replaceAll('"', '""')}"`).join(",")).join("\n"); download(`brand-cleanup-changes-${new Date().toISOString().slice(0, 10)}.csv`, csv); }
  return <><PageHead eyebrow="DAILY WORK · ONLINE TEAM WORKFLOW" title="Brand Cleanup" body="Normalize Root brand names and aliases, verify questionable brands online, review the recommendation, and download only the changes." actions={<span className="status ready"><Cloud size={12} />Team shared</span>} /><section className="brand-cleanup-funnel"><div className="brand-cleanup-funnel-head"><div><span>BRAND CLEANUP JOURNEY</span><h2>From Root table to trusted brand master</h2><p>Every row is reviewed. Changed rows are saved to the shared Root cleanup workspace; unchanged rows are recorded as reviewed and approved.</p></div><strong>{step === 3 ? "READY TO DOWNLOAD" : `STEP ${step} OF 3`}</strong></div><div className="brand-cleanup-steps">{[[1,"Add brands","Select Root rows"],[2,"Review decisions","Normalize and verify"],[3,"Download file","Export changes only"]].map(([number,label,detail]) => <button key={number} className={step === Number(number) ? "active" : step > Number(number) ? "done" : ""} onClick={() => setStep(Number(number) as 1 | 2 | 3)}><b>{step > Number(number) ? <Check size={15} /> : number}</b><span><strong>{label}</strong><small>{detail}</small></span></button>)}</div></section>{step === 1 && <section className="panel brand-cleanup-panel"><div className="panel-head"><div><h2>1. Add brands from Root table</h2><p>{data.rootBrands.length.toLocaleString()} Root brands available. Select the rows you want this cleanup run to review.</p></div><button className="primary" disabled={!selected.length} onClick={() => setStep(2)}>Review {selected.length || "selected"} brands <ChevronRight size={15} /></button></div><label className="enrichment-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search BrandID, brand, or alias" /></label><div className="brand-cleanup-list">{visible.slice(0, 250).map((item) => <label key={item.brand.id}><input type="checkbox" checked={selected.includes(item.brand.id)} onChange={() => setSelected((current) => current.includes(item.brand.id) ? current.filter((id) => id !== item.brand.id) : [...current, item.brand.id])} /><span><b>{item.brand.name}</b><small>{item.brand.id} · {item.brand.aliases.length} aliases</small></span></label>)}</div></section>}{step === 2 && <section className="panel brand-cleanup-panel"><div className="panel-head"><div><h2>2. Review decisions</h2><p>Before and after are shown together. Use one-click online search before approving uncertain rows.</p></div><div className="brand-cleanup-actions"><button className="secondary" onClick={() => setStep(1)}>Back</button><button className="primary" onClick={() => setStep(3)}>Continue to download <ChevronRight size={15} /></button></div></div><div className="brand-cleanup-review-list">{candidates.filter((item) => selected.includes(item.brand.id)).map((item) => <article key={item.brand.id}><div><small>BEFORE · {item.brand.id}</small><b>{item.brand.name}</b><p>{item.brand.aliases.join(" · ") || "No aliases"}</p></div><ChevronRight size={18} /><div><small>PROPOSED</small><b>{item.suggested}</b><p>{item.aliases.join(" · ") || "No aliases"}</p></div><div><span className={`confidence-pill ${item.confidence >= 90 ? "high" : "medium"}`}>{item.confidence}% confidence</span><small>{item.changed ? "Normalization recommendation" : "No change detected · auto-approved when reviewed"}</small></div><div className="brand-cleanup-row-actions"><button className="secondary compact" onClick={() => { setOnline((current) => ({ ...current, [item.brand.id]: true })); window.open(`https://www.google.com/search?q=${encodeURIComponent(`${item.suggested} brand`)}`, "_blank", "noopener,noreferrer"); }}><Globe size={13} />Search online</button><button className="secondary compact" onClick={() => decide(item, "REJECT")}>Reject</button><button className="primary compact" onClick={() => decide(item, "APPROVE")}>Approve</button><span className="decision-badge">{decisions[item.brand.id] || "PENDING REVIEW"}</span></div></article>)}</div><div className="brand-cleanup-review-footer"><span>{reviewed} reviewed · {approved} approved · {selected.length - reviewed} remaining</span><button className="primary" disabled={reviewed < selected.length} onClick={() => setStep(3)}>Finish review <ChevronRight size={15} /></button></div></section>}{step === 3 && <section className="panel brand-cleanup-panel"><div className="brand-cleanup-complete"><Check size={28} /><div><h2>3. Download the cleanup changes</h2><p>{changedRows.length} changed rows will be exported. Unchanged rows are considered reviewed and approved but are not included in the changes CSV.</p></div><button className="primary" disabled={!reviewed || !selected.length} onClick={exportChanges}><ArrowDownToLine size={16} />Download changes CSV</button></div><div className="brand-cleanup-summary"><span><b>{selected.length}</b>Reviewed</span><span><b>{approved}</b>Approved</span><span><b>{changedRows.length}</b>Changes saved</span><span><b>{selected.length - changedRows.length}</b>No change</span></div></section>}</>;
}
...
function DataQualityAnalytics({ data, ubqSource, onAddPriority, onNavigate }: { data: AppData; ubqSource: UbqSource | null; onAddPriority: (source: PriorityQueueSource, rows: ReturnType<typeof parseCsv>) => void; onNavigate: (view: View) => void }) {
  ...
  const qualityRootRows = useMemo(() => data.rootBrands.slice(0, QUALITY_PREVIEW_ROWS), [data.rootBrands]);
  const qualityUbqRows = useMemo(() => ubqRows.slice(0, QUALITY_PREVIEW_ROWS), [ubqRows]);
  ...
  useEffect(() => {
    let cancelled = false;
    setQualityLoading(true); setRootIssues([]); setUbqIssues([]); setIssuePage(1);
    const timer = window.setTimeout(() => {
      ...
      window.setTimeout(() => {
        ...
        if (cancelled) return;
        setUbqIssues(nextUbq); setQualityLoading(false);
      }, 0);
    }, 0);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [qualityRootRows, qualityUbqRows]);
  const allIssues = useMemo(() => [...rootIssues, ...ubqIssues], [rootIssues, ubqIssues]);
  const allFilteredIssues = useMemo(() => allIssues.filter((issue) => (source === "ALL" || issue.source === source) && (severity === "ALL" || issue.severity === severity) && (issueType === "ALL" || issue.type === issueType)).sort((left, right) => qualityScore(right) - qualityScore(left)), [allIssues, source, severity, issueType]);
  const filteredIssues = allFilteredIssues.slice((issuePage - 1) * issuePageSize, issuePage * issuePageSize);
  useEffect(() => setIssuePage(1), [source, severity, issueType, issuePageSize]);
  useEffect(() => setIssuePage((current) => Math.min(current, Math.max(1, Math.ceil(allFilteredIssues.length / issuePageSize)))), [allFilteredIssues.length, issuePageSize]);
  ...
  const topUbq = useMemo(() => [...qualityUbqRows].sort((left, right) => (right.listingCount || 0) - (left.listingCount || 0) || (right.skuCount || 0) - (left.skuCount || 0)).slice(0, 8), [qualityUbqRows]);
  const topRoot = useMemo(() => qualityRootRows.filter((brand) => (brand.rootStatus || "ACTIVE") === "ACTIVE").sort((left, right) => right.aliases.length - left.aliases.length || left.name.localeCompare(right.name)).slice(0, 8), [qualityRootRows]);
  const aliasRisks = useMemo(() => qualityRootRows.flatMap((brand) => brand.aliases.map((alias) => ({ brand, alias, compatibility: assessMergeCompatibility(alias, brand.name) }))).filter(({ alias, brand, compatibility }) => alias.trim().length >= 4 && normalizeBrand(alias).toLowerCase() !== normalizeBrand(brand.name).toLowerCase() && !compatibility.safe).sort((left, right) => left.compatibility.score - right.compatibility.score).slice(0, 12), [qualityRootRows]);
  const categoryData = [
  ...
  return <>
    ...
    {(data.rootBrands.length > QUALITY_PREVIEW_ROWS || ubqRows.length > QUALITY_PREVIEW_ROWS) && <div className="incremental-loading"><Database size={17} /><span><b>Browser preview limited to {QUALITY_PREVIEW_ROWS.toLocaleString()} rows per source</b><small>Use the offline Root cleanup command for complete analysis without freezing this page.</small></span></div>}
    {qualityLoading && (data.rootBrands.length > 0 || ubqRows.length > 0) && <div className="incremental-loading"><RefreshCw className="spinning" size={17} /><span><b>Analyzing data in stages</b><small>The page is ready while Root and UBQ checks finish separately.</small></span></div>}
    {!data.rootBrands.length && !ubqRows.length ? <div className="panel"><EmptyState icon={Database} title={LOCAL_MODE ? "Connect your local Root and UBQ files" : "Load Root and UBQ data to begin"} body={LOCAL_MODE ? "Open the local folder connection and choose the Root table and full UBQ export from your Mac." : "Open Data sources & setup and load the latest Root table and full UBQ export. This page analyzes them locally and works offline."} action={<button className="primary" onClick={() => onNavigate("settings")}>{LOCAL_MODE ? "Open local folder connection" : "Open Data sources & setup"}</button>} /></div> : <>
      ...
      <section id="quality-attention" className="panel quality-attention"><div className="panel-head"><div><h2>Brands needing attention</h2><p>Ranked by risk, impact, and source. Select several findings to create one protected team worklist.</p></div><div><select value={source} onChange={(event) => setSource(event.target.value as typeof source)}><option value="ALL">Root + UBQ</option><option value="ROOT">Root only</option><option value="UBQ">UBQ only</option></select><select value={severity} onChange={(event) => setSeverity(event.target.value as typeof severity)}><option value="ALL">All severity</option><option value="HIGH">High risk</option><option value="MEDIUM">Needs review</option><option value="LOW">Cleanup</option></select><select value={issueType} onChange={(event) => setIssueType(event.target.value)}><option value="ALL">All issue types</option>{categoryData.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div></div>
        ...
        <DataPager page={issuePage} pageSize={issuePageSize} total={allFilteredIssues.length} onPage={setIssuePage} onPageSize={(size) => { setIssuePageSize(size); setIssuePage(1); }} label="findings" sizes={[10, 25, 50]} />
...
function Analytics({ records, ledger, historicalMappings, priorityQueue, completionActivity, rootChanges, teamActivity, currentUser }: { records: BrandRecord[]; ledger: LedgerEntry[]; historicalMappings: HistoricalMappingEntry[]; priorityQueue: PriorityQueueItem[]; completionActivity: MappingActivityEntry[]; rootChanges: AppData["rootChanges"]; teamActivity: AppData["teamActivity"]; currentUser: string }) {
  ...
  const cleanupActivity = useMemo(() => teamActivity.filter((entry) => entry.type === "REVIEWED" && entry.message.startsWith("Brand Cleanup")), [teamActivity]);
  const cleanupThisWeek = cleanupActivity.filter((entry) => Date.now() - new Date(entry.at).getTime() < 7 * 86400000).reduce((sum, entry) => sum + (entry.count || 1), 0);
  const cleanupToday = cleanupActivity.filter((entry) => new Date(entry.at).toDateString() === new Date().toDateString()).reduce((sum, entry) => sum + (entry.count || 1), 0);
  const combinedThisWeek = completionSummary.thisWeek + cleanupThisWeek;
  const cleanupPending = Object.values(rootChanges).filter((change) => change.origin === "BRAND_CLEANUP" && change.status !== "APPLIED" && change.adminStatus !== "REJECTED").length;
...
<section className="panel combined-progress-panel"><div className="panel-head"><div><span className="enrichment-kicker">COMBINED TEAM SCORE</span><h2>{combinedThisWeek.toLocaleString()} of 700 completed this week</h2><p>Brand Mapping and Brand Cleanup contribute to the same team target.</p></div><strong>{Math.min(100, Math.round(combinedThisWeek / 700 * 100))}%</strong></div><div className="progress-track"><i style={{ width: `${Math.min(100, combinedThisWeek / 700 * 100)}%` }} /></div><div className="combined-progress-breakdown"><span><b>{completionSummary.thisWeek.toLocaleString()}</b><small>Brand Mapping</small></span><span><b>{cleanupThisWeek.toLocaleString()}</b><small>Brand Cleanup</small></span><span><b>{cleanupToday.toLocaleString()}</b><small>Cleanup today</small></span><span><b>{cleanupPending.toLocaleString()}</b><small>Cleanup pending</small></span></div></section>
...
function SettingsView({ editingAllowed, data, currentUser, ubqSource, onLoadUbq, onReturnReconciliation, onClear, onUpdateSettings, onSetReference, onAddDecisions, onAddHistoricalMappings, onBackup, onRestore, createSnapshot, applySnapshot, githubSession, onGitHubSession, onGitHubSync, online, serviceSession, onServiceSession, githubRemoteUpdate, onGitHubRemoteUpdate, githubTeamSync, onGitHubTeamSync }: SettingsViewProps) {
  ...
  return <><PageHead eyebrow="DATA SOURCES, VERIFICATION & SYNC" title="Data sources & setup" body="Load fresh UBQ and Root exports, verify what the external Admin tool actually changed, and return unresolved work to team triage." />
    <div className="module-layout"><div className="settings-content">{!LOCAL_MODE && <ApprovedSourcesPanel sources={data.validationSettings.approvedResearchSources} onChange={(approvedResearchSources) => onUpdateSettings({ approvedResearchSources })} />}{LOCAL_MODE ? <><section className="panel local-connection-panel"><div className="section-title"><div><h2>Local folder connection</h2><p>Select the Root and UBQ files from your Mac. This local workspace does not connect to GitHub or the team workspace unless you explicitly publish enrichment results.</p></div><span className="offline-chip"><CloudOff size={13} />Working offline</span></div><div className="reference-list"><ReferenceUploader source="ROOT" count={data.rootBrands.length} meta={data.sourceMeta.ROOT} onLoad={(brands, filename) => onSetReference("ROOT", brands, filename)} /><UbqUploader source={ubqSource} meta={data.sourceMeta.UBQ} data={data} onLoad={onLoadUbq} /></div></section><section className="panel local-connection-panel"><div className="section-title"><div><h2>Publish enrichment to the shared workspace</h2><p>Connect Team Sync only when you are ready to publish the completed enrichment resource. Root and UBQ files remain local; only recommendations are shared.</p></div><span className="offline-chip"><Github size={13} />Explicit publish</span></div>{USE_SYNC_SERVICE ? <ServiceWorkspacePanel createSnapshot={createSnapshot} applySnapshot={applySnapshot} session={serviceSession} onSession={onServiceSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onTeamSync={onGitHubTeamSync} /> : <GitHubWorkspacePanel session={githubSession} onSession={onGitHubSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onSync={onGitHubSync} online={online} />}</section></> : USE_SYNC_SERVICE ? <ServiceWorkspacePanel createSnapshot={createSnapshot} applySnapshot={applySnapshot} session={serviceSession} onSession={onServiceSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onTeamSync={onGitHubTeamSync} /> : <GitHubWorkspacePanel session={githubSession} onSession={onGitHubSession} remoteUpdate={githubRemoteUpdate} onRemoteUpdate={onGitHubRemoteUpdate} teamSync={githubTeamSync} onSync={onGitHubSync} online={online} />}
      {LOCAL_MODE && <section className="panel local-connection-panel"><div className="section-title"><div><h2>Local connection</h2><p>You are connected to this Mac only. Choose the complete Root and UBQ CSV files below; they stay local and are not sent to GitHub.</p></div><span className="offline-chip"><CloudOff size={13} />Local only</span></div><div className="tables-ready"><Check size={16} /><div><b>Ready for local files</b><p>Use “Add table” for Root and “Choose full UBQ export” for UBQ. The selected files are loaded into this offline browser workspace.</p></div></div></section>}
