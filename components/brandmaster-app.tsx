  ...
  return <div className={`app-shell ebay-theme unified-mode ${cleanTriage ? "clean-workflow" : "advanced-workflow"} view-${view}`}>
    ...
    <main aria-busy={Boolean(navigationTarget || navigationPending)}>
      ...
      <div className="page">
        ...
        {LOCAL_MODE && view === "enrichment" && <BrandEnrichment onPromote={promoteEnrichmentCandidates} />}
...
function BrandEnrichment({ onPromote }: { onPromote: (items: EnrichmentCandidate[]) => void }) {
  ...
  return <><PageHead eyebrow="BRAND TOOLS · LOCAL WORKER" title="Brand enrichment" body="Review canonical brand names and aliases in a bounded local workspace. Nothing is sent to Team Sync or GitHub unless you export and choose to share it." actions={<span className="status ready"><CloudOff size={12} />Local only</span>} /><section className="enrichment-hero"><div><span className="enrichment-kicker">ONE-TIME CLEANUP WORKSPACE</span><h2>Make the Root brand list trustworthy</h2><p>Analyze a local CSV, verify uncertain aliases online, then approve only the changes that pass human review.</p></div><div className="enrichment-steps"><span><b>1</b>Analyze</span><span><b>2</b>Review</span><span><b>3</b>Export</span></div></section><section className="panel enrichment-source"><div className="panel-head"><div><h2>1. Choose a local table</h2><p>Files are read from your local folder. The GitHub/team workspace is not used.</p></div><button className="secondary" onClick={() => void refresh()}><RefreshCw size={15} />Refresh files</button></div><div className="enrichment-source-row"><select value={filename} onChange={(e) => setFilename(e.target.value)}><option value="">Choose Root or UBQ CSV…</option>{files.map((file) => <option key={file} value={file}>{file}</option>)}</select><button className="primary" disabled={busy || !filename} onClick={() => void start()}><SearchCheck size={15} />{busy ? "Starting…" : "Analyze table"}</button><span className="enrichment-file-note">{files.length ? `${files.length} local file${files.length === 1 ? "" : "s"} found` : "Put root.csv and ubq.csv in Brandmaster-data/local"}</span></div>{error && <div className="reference-error"><CircleHelp size={14} />{error} Start the combined local app with <code>npm run dev:local</code>.</div>}</section><section className="panel"><div className="panel-head"><div><h2>Runs</h2><p>Select a completed or running analysis to review its recommendations.</p></div></div><div className="enrichment-runs">{jobs.length ? jobs.map((item) => <button className={`enrichment-run ${job?.id === item.id ? "active" : ""}`} key={item.id} onClick={() => void loadJob(item.id)}><span className={`enrichment-run-dot ${item.status.toLowerCase()}`} /><span><b>{item.filename || "Local table"}</b><small>{item.status} · {Number(item.processed || 0).toLocaleString()} rows processed</small></span><ChevronRight size={16} /></button>) : <p className="enrichment-empty">No runs yet. Choose a local table above.</p>}</div></section>{job && <EnrichmentJourney job={job} candidates={candidates} />}{job && <section className="panel enrichment-review"><div className="panel-head"><div><span className="enrichment-kicker">2. HUMAN REVIEW</span><h2>Recommendations</h2><p>{job.status} · {candidates.length.toLocaleString()} recommendations · {reviewed.toLocaleString()} reviewed</p></div><button className="secondary" disabled={!candidates.length} onClick={() => onPromote(candidates)}><Database size={15} />Use as enriched master resource</button><button className="primary" disabled={!candidates.length || exporting} onClick={() => void exportCsv()}><ArrowDownToLine size={15} />{exporting ? "Preparing…" : `Export full enrichment resource`}</button></div><div className="enrichment-metrics"><div><b>{candidates.length.toLocaleString()}</b><span>Needs review</span></div><div><b>{high.toLocaleString()}</b><span>High confidence</span></div><div><b>{approved.toLocaleString()}</b><span>Approved</span></div><div><b>{(candidates.length - reviewed).toLocaleString()}</b><span>Remaining</span></div></div><EnrichmentCandidates items={candidates} onUpdate={update} onCheckOnline={checkOnline} /></section>}</>;
}
