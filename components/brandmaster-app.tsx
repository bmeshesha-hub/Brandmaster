...
function ApprovedSourcesPanel() { const [sources, setSources] = useState<{ name: string; url: string; enabled: boolean }[]>(() => { try { return JSON.parse(localStorage.getItem("brandmaster-approved-research-sources") || "null") || [{ name: "Wikidata", url: "https://www.wikidata.org/", enabled: true }, { name: "Wikipedia", url: "https://en.wikipedia.org/", enabled: true }]; } catch { return [{ name: "Wikidata", url: "https://www.wikidata.org/", enabled: true }, { name: "Wikipedia", url: "https://en.wikipedia.org/", enabled: true }]; } }); const save = (next: typeof sources) => { setSources(next); localStorage.setItem("brandmaster-approved-research-sources", JSON.stringify(next)); }; return <section className="panel approved-sources-panel"><div className="section-title"><div><h2>Approved research sources</h2><p>Enabled sources are used for Brand Cleanup research. Other websites are retained for manual evidence.</p></div><span className="offline-chip"><Globe size={13} />Browser-safe sources</span></div><div className="approved-source-list">{sources.map((source, index) => <div className="approved-source-row" key={`${source.url}-${index}`}><input value={source.name} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value } : item))} /><input value={source.url} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, url: event.target.value } : item))} /><label><input type="checkbox" checked={source.enabled} onChange={(event) => save(sources.map((item, itemIndex) => itemIndex === index ? { ...item, enabled: event.target.checked } : item))} />Enabled</label><button className="danger-outline" onClick={() => save(sources.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={14} /></button></div>)}</div><button className="secondary" onClick={() => save([...sources, { name: "Approved source", url: "https://", enabled: true }])}><Plus size={14} />Add source</button></section>; }
...
function OnlineBrandCleanupV2({ data, currentUser, onSave }: { data: AppData; currentUser: string; onSave: (brand: CatalogBrand) => void }) {
  ...
  async function lookupOnline(item: typeof candidates[number]) { setOnlineBusy(item.brand.id); setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Checking approved sources…" })); try { let configured: { name: string; url: string; enabled: boolean }[] = []; try { configured = JSON.parse(localStorage.getItem("brandmaster-approved-research-sources") || "[]"); } catch {} const enabled = configured.filter((source) => source.enabled); const has = (term: string) => enabled.length === 0 || enabled.some((source) => source.url.toLowerCase().includes(term)); let match: { label?: string; aliases?: { value: string }[] } | undefined; let evidence: string[] = []; if (has("wikidata")) { const response = await fetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(item.name)}&language=en&format=json&origin=*`); const result = await response.json() as { search?: { label?: string; aliases?: { value: string }[] }[] }; match = result.search?.[0]; if (match) evidence.push("Wikidata"); } if (!match && has("wikipedia")) { const response = await fetch(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(item.name)}&limit=5&namespace=0&format=json&origin=*`); const result = await response.json() as [string, string[], string[], string[]]; if (result[1]?.[0]) { match = { label: result[1][0], aliases: [] }; evidence.push("Wikipedia"); } } if (match) { setOnline((current) => ({ ...current, [item.brand.id]: true })); setOnlineStatus((current) => ({ ...current, [item.brand.id]: `Match found in ${evidence.join(" + ")} · review evidence` })); setEdits((current) => ({ ...current, [item.brand.id]: { name: match?.label || item.name, aliases: [...new Set([...(match?.aliases || []).map((alias) => alias.value), ...item.aliases.split(",").map((alias) => alias.trim()).filter(Boolean)])].join(", ") } })); } else setOnlineStatus((current) => ({ ...current, [item.brand.id]: "No approved-source match · human review required" })); } catch { setOnlineStatus((current) => ({ ...current, [item.brand.id]: "Approved-source lookup unavailable · edit manually" })); } finally { setOnlineBusy(null); } }
