  ...
  return <>
    ...
    {!data.rootBrands.length && !ubqRows.length ? <div className="panel"><EmptyState icon={Database} title="Load Root and UBQ data to begin" body="Open Data sources & setup and load the latest Root table and full UBQ export. This page analyzes them locally and works offline." action={<button className="primary" onClick={() => onNavigate("settings")}>Open Data sources & setup</button>} /></div> : <>
      ...
      <section className="quality-top-grid">
        <article className="panel"><div className="panel-head"><div><h2>Top UBQ brands</h2><p>Highest listing volume—clean these first for the greatest impact</p></div></div><div className="quality-ranking">{topUbq.map((row, index) => <div key={row.id}><em>{index + 1}</em><span><b>{row.name}</b><small>{row.id}</small></span><strong>{(row.listingCount || 0).toLocaleString()}<small>listings</small></strong><button onClick={() => onAddPriority("UBQ", [{ id: row.id, name: row.name, listingCount: row.listingCount, skuCount: row.skuCount }])}>Triage</button></div>)}</div></article>
        <article className="panel"><div className="panel-head"><div><h2>Root brands with most aliases</h2><p>Large alias families deserve stronger conflict review</p></div></div><div className="quality-ranking">{topRoot.map((brand, index) => <div key={brand.id}><em>{index + 1}</em><span><b>{brand.name}</b><small>{brand.id}</small></span><strong>{brand.aliases.length}<small>aliases</small></strong><button onClick={() => onAddPriority("ROOT", [{ id: brand.id, name: brand.name }])}>Review</button></div>)}</div></article>
      </section>
      {aliasRisks.length > 0 && <section className="panel quality-alias-review"><div className="panel-head"><div><h2>Aliases that need a human check</h2><p>These aliases have weak name identity with their canonical brand. They may be legitimate acquired brands, so Brandmaster never removes them automatically.</p></div><button className="secondary" onClick={() => onNavigate("aliases")}>Open alias management</button></div><div>{aliasRisks.slice(0, 8).map(({ brand, alias, compatibility }) => <span key={`${brand.id}:${alias}`}><b>{alias}</b><em>→</em><strong>{brand.name}</strong><small>{compatibility.reason}</small><button onClick={() => onAddPriority("ROOT", [{ id: brand.id, name: brand.name }])}>Review</button></span>)}</div></section>}
      <section id="quality-attention" className="panel quality-attention"><div className="panel-head"><div><h2>Brands needing attention</h2><p>Filter the findings, then send individual records into the shared High Priority queue.</p></div><div><select value={source} onChange={(event) => setSource(event.target.value as typeof source)}><option value="ALL">Root + UBQ</option><option value="ROOT">Root only</option><option value="UBQ">UBQ only</option></select><select value={severity} onChange={(event) => setSeverity(event.target.value as typeof severity)}><option value="ALL">All severity</option><option value="HIGH">High risk</option><option value="MEDIUM">Needs review</option><option value="LOW">Cleanup</option></select><select value={issueType} onChange={(event) => setIssueType(event.target.value)}><option value="ALL">All issue types</option>{categoryData.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></div></div><div className="quality-findings">{filteredIssues.slice(0, 50).map((issue) => <div key={issue.key}><span className={issue.severity.toLowerCase()}>{issue.severity}</span><div><b>{issue.name}</b><small>{issue.brandId}</small></div><strong>{issue.title}<small>{issue.reason}</small></strong><em>{issue.suggestion || "Manual review"}</em><button onClick={() => queueIssue(issue)}>Add to priority</button></div>)}</div>{filteredIssues.length > 50 && <p className="quality-more">Showing the first 50 of {filteredIssues.length.toLocaleString()} findings. Use Smart Cleanup for the complete paginated worklist.</p>}</section>
    </>}
  </>;
}
