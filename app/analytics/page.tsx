  ...
  return <main className="public-analytics-page">
    ...
    <div className="public-analytics-content">
      ...
      {loading && !snapshot ? <div className="public-analytics-state"><RefreshCw className="spinning" /><h2>Loading published analytics…</h2></div> : error ? <div className="public-analytics-state error"><BarChart3 /><h2>{error}</h2><button onClick={() => void load()}>Try again</button></div> : snapshot && <>
        <section className="public-kpis"><article><span><BarChart3 /></span><small>TOTAL RECORDED</small><b>{number(snapshot.totals.decisions)}</b><p>mapping decisions</p></article><article><span><Activity /></span><small>TEAM COMPLETED THIS WEEK</small><b>{number(snapshot.totals.thisWeek)}</b><p>{number(snapshot.totals.today)} completed today · same weekly-target source</p></article><article><span><CheckCircle2 /></span><small>CONFIRMED DELIVERY</small><b>{number(snapshot.delivery.confirmed)}</b><p>{number(snapshot.delivery.awaiting)} awaiting confirmation</p></article><article><span><Users /></span><small>QUEUE PROGRESS</small><b>{number(snapshot.queue.exported)}</b><p>{number(snapshot.queue.available + snapshot.queue.assigned + snapshot.queue.inReview + snapshot.queue.blocked + snapshot.queue.ready)} active</p></article></section>
