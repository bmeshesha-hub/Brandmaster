...
export type ChunkManifest = {
  ...
  core: string;
  arrays: Record<"ledger" | "customBrands" | "acaBrands" | "fpaBrands" | "rootBrands", string[]> & { historicalMappings?: string[]; manualFpaIds?: string[]; priorityQueue?: string[]; cleanupConfirmations?: string[]; adminUpdateRuns?: string[]; exportRuns?: string[]; teamActivity?: string[]; teamProgressSnapshots?: string[]; enrichmentResources?: string[] };
...
export function serializeWorkspaceFiles(workspace: SharedWorkspaceSnapshot) {
  ...
  const arrays: ChunkManifest["arrays"] = {
    ...
    teamProgressSnapshots: writeChunks(files, "progress-daily", workspace.data.teamProgressSnapshots || []),
...
export async function hydrateWorkspaceManifest(manifest: ChunkManifest, load: (path: string) => Promise<string>): Promise<SharedWorkspaceSnapshot> {
  ...
  const core = JSON.parse(await limitedLoad(manifest.core)) as Omit<AppData, "batches" | "ledger" | "historicalMappings" | "manualFpaIds" | "priorityQueue" | "cleanupConfirmations" | "adminUpdateRuns" | "exportRuns" | "teamActivity" | "teamProgressSnapshots" | "enrichmentResources" | "learned" | "learningOverrides" | "customBrands" | "acaBrands" | "fpaBrands" | "rootBrands" | "rootChanges" | "userWorkspaces" | "teamPresence">;
  const [ledger, historicalMappings, manualFpaIds, priorityQueue, cleanupConfirmations, adminUpdateRuns, exportRuns, teamActivity, teamProgressSnapshots, enrichmentResources, customBrands, acaBrands, fpaBrands, rootBrands, learnedEntries, learningOverrideEntries, rootChangeEntries, userWorkspaceEntries, teamPresenceEntries] = await Promise.all([
    read<AppData["ledger"][number]>(manifest.arrays.ledger), read<AppData["historicalMappings"][number]>(manifest.arrays.historicalMappings || []), read<AppData["manualFpaIds"][number]>(manifest.arrays.manualFpaIds || []), read<AppData["priorityQueue"][number]>(manifest.arrays.priorityQueue || []), read<AppData["cleanupConfirmations"][number]>(manifest.arrays.cleanupConfirmations || []), read<AppData["adminUpdateRuns"][number]>(manifest.arrays.adminUpdateRuns || []), read<AppData["exportRuns"][number]>(manifest.arrays.exportRuns || []), read<AppData["teamActivity"][number]>(manifest.arrays.teamActivity || []), read<AppData["teamProgressSnapshots"][number]>(manifest.arrays.teamProgressSnapshots || []), read<AppData["enrichmentResources"][number]>(manifest.arrays.enrichmentResources || []), read<AppData["customBrands"][number]>(manifest.arrays.customBrands), read<AppData["acaBrands"][number]>(manifest.arrays.acaBrands),
  ...
  const workspace: SharedWorkspaceSnapshot = { schemaVersion: "brandmaster.workspace.v1", exportedAt: manifest.exportedAt, data: { ...core, batches, ledger, historicalMappings, manualFpaIds, priorityQueue, cleanupConfirmations, adminUpdateRuns, exportRuns, teamActivity, teamProgressSnapshots, enrichmentResources, learned: Object.fromEntries(learnedEntries), learningOverrides: Object.fromEntries(learningOverrideEntries), customBrands, acaBrands, fpaBrands, rootBrands, rootChanges: Object.fromEntries(rootChangeEntries), userWorkspaces: Object.fromEntries(userWorkspaceEntries), teamPresence: Object.fromEntries(teamPresenceEntries) }, ubq };
