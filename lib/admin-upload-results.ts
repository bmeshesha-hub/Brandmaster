...
export type AdminUploadResultRow = {
  ...
  status: "SUCCESS" | "FAILED" | "NOT_FOUND" | "ALREADY_EXISTS";
  rawStatus: string;
  errorMessage?: string;
...
export function parseAdminUploadResults(text: string): AdminUploadResultParse {
  ...
  const rows = parsed.slice(1).filter((row) => row[idIndex]?.trim()).map((row) => {
    ...
    const detail = `${rawStatus} ${errorMessage || ""}`;
    const notFound = /DOCUMENT_NOT_FOUND|requested brand or product could not be found/i.test(detail);
    const alreadyExists = /already exists|duplicate(?: brand| mapping| record)?/i.test(detail);
    return {
      ...
      status: success ? "SUCCESS" as const : notFound ? "NOT_FOUND" as const : alreadyExists ? "ALREADY_EXISTS" as const : "FAILED" as const,
      rawStatus,
      errorMessage,
...
export function summarizeAdminUploadResults(attemptedIds: string[], rows: AdminUploadResultRow[]) {
  ...
  return {
    ...
    alreadyExists: matching.filter((row) => row.status === "ALREADY_EXISTS"),
...
export function applyAdminUploadResultsToRecords(records: BrandRecord[], attemptedIds: string[], rows: AdminUploadResultRow[], filename: string, importedAt: string, moveFailuresToReview: boolean, markNotFoundDone = false, confirmedBy?: string) {
  ...
  const updated = records.map((record) => {
    ...
    if (result.status === "SUCCESS") return { ...record, workflowStage: "ADMIN_CONFIRMED" as const, adminUploadStatus: "SUCCESS" as const, adminUploadedAt: importedAt, adminUploadedBy: confirmedBy, adminUploadResultFile: filename, adminUploadMessage: result.rawStatus, createdBrandId: result.createdBrandId };
    if (result.status === "NOT_FOUND" || result.status === "ALREADY_EXISTS") return markNotFoundDone
      ? { ...record, workflowStage: "CLOSED_WITHOUT_MAPPING" as const, excludedFromExport: true, triageResolution: result.status === "ALREADY_EXISTS" ? "ALREADY_DONE" as const : "NOT_FOUND_IN_UBQ" as const, triageResolutionNote: result.errorMessage || result.rawStatus, triageResolvedAt: importedAt, triageResolvedBy: confirmedBy, status: "reviewed" as const, reviewer: confirmedBy, reviewedAt: importedAt, adminUploadStatus: undefined, adminUploadedAt: importedAt, adminUploadedBy: confirmedBy, adminUploadResultFile: filename, adminUploadMessage: result.status === "ALREADY_EXISTS" ? "Closed: Admin reports this brand was already completed" : "Closed: the source brand is no longer present in UBQ" }
    ...
    return { ...record, workflowStage: moveFailuresToReview ? "SECOND_REVIEW" as const : "DOWNLOADED" as const, status: moveFailuresToReview ? "needs-review" as const : record.status, secondReviewRequestedBy: moveFailuresToReview ? confirmedBy : record.secondReviewRequestedBy, secondReviewRequestedAt: moveFailuresToReview ? importedAt : record.secondReviewRequestedAt, secondReviewReason: moveFailuresToReview ? `Admin upload failed: ${result.errorMessage || result.rawStatus}` : record.secondReviewReason, adminUploadStatus: "FAILED" as const, adminUploadedAt: importedAt, adminUploadResultFile: filename, adminUploadMessage: result.errorMessage || result.rawStatus };
  });
  return {
    records: updated,
    ...
    resolved: updated.filter((record) => attempted.has(record.id) && Boolean(record.triageResolution)),
